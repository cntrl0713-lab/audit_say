...
> 2026-09-08 이름 변경 이후 프로필 테이블은 `cpa_users`다. 이전 `user_cpa`는 호환 뷰이므로 이 문서의 테이블 RLS 명령을 이전 이름에 실행하지 않는다. [이름 전환 기록](docs/프로젝트-테이블-cpa-접두어-전환.md)을 먼저 확인한다. 아래 과거 RLS 검증 설명은 테이블 이름을 현재 이름으로 표기했다.
...
이미 `assertAdmin`/`assertAuthenticated`로 권한을 검증하므로, **테이블을
...
그 카운터는 실질적으로 아무것도 막지 못했다. 채점 1회는 외부 AI 호출을 동반하므로
제한이 걸리지 않는다는 것은 곧 외부 AI 호출 비용 상한이 없다는 뜻이다.
...
2026-09-08 보안 수정 이후 `lib/rateLimit.ts`는 RPC·테이블 미존재, 권한 오류,
통신 오류를 모두 fail closed로 처리한다. **DB가 명시적으로 허용한 요청만 통과하며
인스턴스 로컬 폴백은 사용하지 않는다.** RPC를 먼저 적용해야 한다.
사용자별 AI 채점은 60초에 10회, 신규 제출 기록 생성은 빈 답안을 포함해 60초에 30회로
제한한다. 기존 제출의 저장 결과 조회·복원에는 신규 제출 한도를 소비하지 않는다.
...
함수가 사라지면 신규 제출과 AI 채점 요청은 거절된다. 기존 저장 결과 조회는 유지된다.
