// 무호출 사전검사 후 --run이면 운영 채점 경로(Luna)로 대표답안을 실제 채점한다.
//   npx tsx --env-file=.env.local cpa_uploader/drafts/standard-priority-2026-09-14/verify-and-grade.ts [--run --run-id=run-v1] [--only=l01-sub1,...]
...
async function main() {
 ...
 assert.equal(manifest.cases.length,30); assert.equal(local.length,10);
