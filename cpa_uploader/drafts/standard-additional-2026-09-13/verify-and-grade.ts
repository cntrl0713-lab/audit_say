...
const previousIndex={sets:['standard-gap','standard-expansion','standard-followup'].flatMap(name=>read(`cpa_uploader/drafts/${name}-2026-09-13/index.json`).sets)};
const previousSets:QuestionSetV3[]=previousIndex.sets.map((s:any)=>read(s.file));
...
async function main() {
 ...
 errors.push(...draftConflicts(sets,[...bank,...previousSets]));
 errors.push(...validateAuthoringBank([...bank,...previousSets,...sets]).errors);
 assert.deepEqual(errors,[]);
 assert.equal(manifest.cases.length,39); assert.equal(local.length,13);
 ...
 const inputFiles=[...index.sets.flatMap((s:any)=>[s.file,`${s.file}.authoring-plan.json`]),`${batch}/sources/official-excerpts.txt`,`${batch}/qa-representatives.json`,`${batch}/agent-review.json`,`${batch}/content.mjs`,`${batch}/build.mjs`,`${batch}/verify-and-grade.ts`];
 const guards=[...runtimeFiles,...inputFiles].map(file=>({file,sha256:sha(file)}));
 for(const row of guards){const dst=`${dir}/frozen/${row.file}.snapshot`;fs.mkdirSync(path.dirname(dst),{recursive:true});fs.copyFileSync(row.file,dst);}
 ...
 if(!process.env.OPENAI_API_KEY?.trim()) {write(`${dir}/blocked.json`,{reason:'OPENAI_API_KEY 미설정',completed_jobs:0,unexecuted:jobs.length,local_preflight_completed:true});console.log('실제 채점 중단: OPENAI_API_KEY 미설정');return;}
