...
export const batch='cpa_uploader/drafts/standard-additional-2026-09-13';
const raw='cpa_uploader/raw/originals/standard-additional-2026-09-13';
...
const inputs={KGA:'cpa_uploader/drafts/frequency-gap-2026-09-10/sources/kga-2026-pymupdf-pages.txt',ETHICS:'cpa_uploader/raw/originals/standard-expansion-2026-09-13/ethics-2024-complete.txt'};
const urls={KGA:'https://www.kicpa.or.kr/board/fileMngr?cmd=down&boardId=acc0102&bltnNo=11786004332051&fileSeq=1&subId=sub06',ETHICS:'https://www.kicpa.or.kr/board/fileMngr?cmd=down&boardId=ethstd03&bltnNo=11735541549550&fileSeq=2&subId=sub06'};
const editions={KGA:'KICPA 2026년 7월 개정 전문. 해당 기준의 일반 요구를 출제하며 시험 적용연도는 별도로 확정하지 않는다.',ETHICS:'공인회계사윤리기준 2024-12-19 의결 전문. 2026-09-13 개정 공고 및 공개초안 목록 확인; 2026-09-02 공개초안은 확정 본문으로 적용하지 않는다.'};
...
const research={date:'2026-09-13',bank_sha256:sha(bankPath),comparison_inventory_sha256:sha(`${batch}/comparison-inventory.json`),comparison_scope:{canonical_sets:bank.length,canonical_questions:bank.flatMap(s=>s.subquestions).length,draft_and_bank_variants:inv.variants.length,prior_batches:['cpa_uploader/drafts/standard-gap-2026-09-13/index.json','cpa_uploader/drafts/standard-expansion-2026-09-13/index.json','cpa_uploader/drafts/standard-followup-2026-09-13/index.json']},selection_policy:'기존·진행 중 초안의 요구와 직접 비교. 미연결 또는 낮은 문구 유사도를 미출제 확정으로 사용하지 않음. 지침상 학습 순서를 따라 기초·윤리·수임·문서화·특수관계자 순서로 배열.',edition_policy:editions,excluded_candidates:[{topic:'낮은 감사보수',reason:'pilot-01-005/sub3이 위협과 안전장치를 이미 다룸'},{topic:'계약서 내용',reason:'pilot-03-005/sub2와 중복'},{topic:'통제무력화 기본 절차',reason:'pilot-05-002, pilot-05-009가 이미 다룸'},{topic:'계정·거래 경영진주장 및 중요성 지표',reason:'직전 standard-gap 배치와 중복'},{topic:'전문가·내부감사·서비스조직 이용',reason:'pilot-13-002·003·009·010·011에서 해당 요구가 이미 구체적으로 다뤄져 제외'},{topic:'이해상충·제2의견·감사인 교체',reason:'직전 standard-expansion 배치의 15물음과 중복되어 제외'},{topic:'고유한계·특수관계자 계약 검사·그룹감사 중요성',reason:'pilot-02-007, pilot-11-006/sub3, pilot-14-008 등에서 이미 직접 다룸. 기존 사례형 요구의 독립형 재출제를 제외한다.'},{topic:'선행 43물음',reason:'세 배치의 기준서 요구·모범답안·criterion까지 대조하여 새 요구만 선정한다.'},{topic:'개정 윤리기준 공개초안',reason:'확정 규정이 아니므로 배제'}],groups:groups.map(g=>({id:g.id,title:g.title,neighbors:g.neighbors,difference:g.difference,source_unit_ids:g.units,elements:g.elements})),elements:elementData.elements.filter(e=>selectedElements.includes(e.id)),frequency_note:'개별 요소의 기출 고유물음 빈도와 모의 빈도를 별도로 유지한다. 선물·접대와 기준 이탈 문서화의 기출 요구 및 의뢰인 자산 보관의 교재 요구를 각각 원발문과 대조했다. 같은 시험의 재수록은 추가 횟수로 합하지 않는다. 기타 새 기준서 요구의 직접 기출빈도는 확정하지 않았다.'};
write(`${batch}/research.json`,research);
const index=[];const semantics=[];const cases=[];let num=0;
let problems='# 기준서형 추가 물음 13선\n\n각 물음은 다른 물음이나 회사별 사례 없이 독립적으로 풉니다. 물음에서 정한 범위는 모두 답하되, 순서·문장 수는 점수에 영향을 주지 않습니다.\n\n';
let answers='# 기준서형 추가 물음 13선 — 모범답안·배점\n\n각 독립 채점기준은 1점이며 의미가 같은 자연스러운 표현을 인정합니다. 아래 번호는 정답 순서를 강제하지 않습니다.\n\n';
for(const g of groups){
 ...
 const setId=`draft-standard-additional-20260913-${g.id.toLowerCase()}`;
...
if(num!==13)throw Error(`물음 수 ${num}`);
...
write(`${batch}/qa-representatives.json`,{version:1,artifact_type:'draft_grading_cases',selected_before_execution:true,model:'gpt-5.6-luna',policy:{tolerance_points:1,target_rate:0.95,budget_usd:null,budget_note:'사용자 지정 금액 상한 없음. 대표 39건으로 초기 실행을 제한하고 제공자 한도 오류에서 중단한다. 내용 오류·허용범위 밖 점수에만 필요한 추가 검사를 한다.',representative_jobs:cases.length,extra_semantic_api_calls:0},cases});
...
const provenance={version:1,checked_on:'2026-09-13',urls,editions,inputs:Object.entries(inputs).map(([kind,file])=>({kind,file,sha256:sha(file)})),originals:Object.entries(pdfs).map(([kind,file])=>({kind,file,sha256:sha(file),prior_collection:'cpa_uploader/raw/collections/2026-09-11-initial/manifest.json'})),derivation:{KGA:'보존된 PyMuPDF 전문의 지정 행을 LF로 연결',ETHICS:'pypdf의 페이지별 직접 추출본 지정 행; 페이지 제목만 추가',LAW:'공식 HTML 원본에서 Python 표준 HTMLParser로 script/style을 제외한 텍스트 노드를 추출; 법령 링크 경계가 줄바꿈으로 남음'},context_ranges:contextRanges,working_source:source,raw_excerpt:`${raw}/official-excerpts.txt`,excerpt_sha256:sha(source)};
write(`${batch}/sources/provenance.json`,provenance);
const entries=[...Object.entries(inputs).filter(([k])=>k!=='KGA').map(([k,f])=>({original_path:f,category:'verification',role:'official_text_extraction',source_url:urls[k],edition:editions[k]})),{original_path:`${raw}/official-excerpts.txt`,category:'verification',role:'official_text_excerpt'},{original_path:source,category:'verification',role:'working_source_copy',derived_from:`${raw}/official-excerpts.txt`}].map(e=>({...e,sha256:sha(e.original_path),provenance_record:`${batch}/sources/provenance.json`}));
