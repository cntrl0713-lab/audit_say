// r37에서 분리 보존하는 기준서형 세트의 후보 은행·실제 채점 입력 생성기.
// 배치 공용 tools/build-round3.mjs는 모든 물음이 question_style 'case'임을 단언하므로 기준서형에는 쓸 수 없다.
// 이 파일은 r31-comparative-restatement-extension/build-standards-round.mjs의 절차를 그대로 따르되 회차·경로·세트 ID만 바꾼 것이다.
// 후보 검증에서 현재 정본 + 이 회차의 두 초안(사례형·기준서형)을 함께 메모리에서 validateAuthoringBank로 확인한다.
// 기존 tools/의 파일은 고치지 않는다. 기존 출력이 있으면 쓰지 않는다.
//
//   node --import tsx cpa_uploader/drafts/case-review-2026-09-15/r37-small-entity-update/build-standards-round.mjs candidate
//   node --import tsx cpa_uploader/drafts/case-review-2026-09-15/r37-small-entity-update/build-standards-round.mjs execution
...
const CASE_SET_ID = 'case-18-small-entity-20260921';
...
if (mode === 'candidate') {
    ...
    const entries = draft.subquestions.map((question) => {
        ...
        return {
            set_id: SET_ID, subquestion_id: question.id, question_style: 'standard', topic_ids: question.topic_ids,
    ...
    const caseEntries = caseDraft.subquestions.map((question) => {
        ...
        return {
            set_id: CASE_SET_ID, subquestion_id: question.id, question_style: 'case', topic_ids: question.topic_ids,
    ...
    write(`${R}/baseline-standards-${V}.json`, {
        ...
        full_candidate_in_memory: {
            written: false, sha256: hash(serialize(full)), sets: full.length,
    ...
    write(`${R}/catalog-standards-${V}.json`, {
        schema_version: 1, source_file: candidateFile, source_file_sha256: ref(candidateFile).sha256,
        public_content_hash: contentHash([draft].map(compilePublicQuestionSet)),
        review_file: classificationFile, review_file_sha256: ref(classificationFile).sha256,
        topics: catalog.topics, classifications,
    });
    console.log({
        full_candidate: { sets: full.length, questions: checked.subquestionCount, criteria: checked.criterionCount, points: checked.totalPoints },
    ...
    for (const question of set.subquestions) {
        ...
        for (const kind of maximum > 1 ? ['partial', 'wrong'] : ['wrong']) {
            ...
            cases.push({
                id: row.id, set_id: SET_ID, subquestion_id: question.id, kind, answer: row.answer, expected_points: points,
    ...
    const reviews = [{
        set_id: SET_ID, content_hash: reviewedContentHash(set), reviewer_id: rootReview.reviewer_id,
        ...
        questions: set.subquestions.map((question) => {
            ...
            return {
                subquestion_id: question.id, criterion_ids: question.criteria.map((c) => c.id),
    ...
    write(out + '/policy.json', {
        scope: ref(out + '/scope.json'), model: 'gpt-5.6-luna', grading_point_tolerance: 1,
        ...
        reuse_decision: `${V} 기준서형 초안 1세트·2물음을 검토한다. 원 pilot-18-005의 실측 기록은 없고 발문도 다시 썼으므로 재사용하지 않으며 분모에 넣지 않는다. 기준서형은 물음마다 학습 단위가 따로 생겨 한 요청이 한 물음만 담으므로, 모범·부분·오답 역할을 물음마다 실측해 모두 6요청이 된다(r31의 기준서형 세트와 같다). 모범답안 요청이 사례 지문 없는 독립 물음으로 만점이 되는지를 함께 확인한다. 채점 은행은 이 세트만 담은 부분 은행이다.`,
    });
    ...
    // 기준서형은 물음마다 학습 단위가 따로 생긴다(<세트ID>--<물음ID>--standard).
    ...
    const entries = projections.flatMap(({ unit, projection, projectedFile }) => ['model', 'partial', 'wrong'].map((kind) => {
        ...
        for (const question of projection.subquestions) {
            ...
            selection.push({
                ...ref(kind === 'model' ? bankFile : out + '/representatives.json'), subquestion_id: question.id,
                case_id: kind === 'model' ? null : representative.id, kind,
                reason: '원문과 현재 criterion에서 실행 전에 확정한 대표 답안. 기준서형 독립 물음 하나를 한 요청으로 채점한다.',
            });
        }
        return {
            id: `${unit}--${kind}`, worker: 'a', learning_unit_id: unit, source_set_id: SET_ID,
    ...
    console.log(JSON.stringify({
        ...
        learning_units: units, manifest: ref(out + '/grading-manifest.json'), actual_sdk_calls: 0,
    }, null, 2));
}
