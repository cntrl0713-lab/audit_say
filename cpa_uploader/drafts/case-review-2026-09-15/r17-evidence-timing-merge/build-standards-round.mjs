// r17에서 분리 보존하는 기준서형 세트의 후보 은행·실제 채점 입력 생성기.
...
//
//   node --import tsx cpa_uploader/drafts/case-review-2026-09-15/r17-evidence-timing-merge/build-standards-round.mjs candidate
//   node --import tsx cpa_uploader/drafts/case-review-2026-09-15/r17-evidence-timing-merge/build-standards-round.mjs execution
...
const SET_ID = 'pilot-07-006-standards-20260920';
const CASE_SET_ID = 'case-07-evidence-timing-20260920';
...
if (mode === 'candidate') {
    ...
    write(out + '/policy.json', { scope: ref(out + '/scope.json'), model: 'gpt-5.6-luna', grading_point_tolerance: 1,
        ...
        reuse_decision: `${V} 기준서형 초안 1세트·1물음만 검토한다. 원 pilot-07-006의 실측은 발문이 달라 재사용하지 않고 분모에 넣지 않는다. 모범·부분·오답 역할을 3요청으로 실측하며, 모범답안 요청이 사례 지문 없는 독립 물음으로 만점이 되는지를 함께 확인한다. 채점 은행은 이 세트만 담은 부분 은행이다.` });
