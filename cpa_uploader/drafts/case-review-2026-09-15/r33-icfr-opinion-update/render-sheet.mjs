...
//   node cpa_uploader/drafts/case-review-2026-09-15/r33-icfr-opinion-update/render-sheet.mjs
...
const out = [
    `# ${set.title}`,
    '',
    `단독 갱신 초안 \`${set.id}\` · 사례형 ${set.subquestions.length}물음 · ${total}점. 원 세트는 \`pilot-17-005\`(수정 요청서 22번)이다. 상태는 \`${set.status}\`이며 정본·운영 DB에는 아직 반영하지 않았다.`,
    '',
];
...
const labels = {
    model: '저장 모범답안',
    partial: '대표 부분정답(물음 1은 함정 ⑥을 옳지 않다고 고르고 ⑤를 빠뜨림, 물음 2는 ⑩을 빠뜨림, 물음 3은 ⑬의 이유를 빠뜨림)',
    wrong: '대표 오답(옳은 항목·함정만 고르고 옳지 않은 항목은 하나도 고르지 않음)',
};
