# r06: KGA 320 발췌(2025 전문 PDF 306·310·311쪽)의 원본 대조용 쪽 텍스트와 이미지를 raw originals에 새로 쓴다.
# 기존 PyMuPDF 페이지 텍스트(kga-2025-pymupdf-pages.txt)의 같은 쪽과 비교해 결과를 출력하며, 기존 파일이 있으면 쓰지 않는다.
#   uv run --offline --no-project --with pymupdf python cpa_uploader/drafts/case-review-2026-09-15/r06-materiality-merge/sources/render-kga320-pages.py
import hashlib
import json
import pathlib
...
results = []
for number in PAGES:
    text_file, image_file = RAW / f'kga320-2025-page-{number}.txt', RAW / f'kga320-2025-page-{number}.png'
