// r38에서 분리 보존하는 기준서형 세트의 후보 은행·실제 채점 입력 생성기.
...
//
//   node --import tsx cpa_uploader/drafts/case-review-2026-09-15/r38-interim-review-update/build-standards-round.mjs candidate
//   node --import tsx cpa_uploader/drafts/case-review-2026-09-15/r38-interim-review-update/build-standards-round.mjs execution
...
const SET_ID = 'pilot-19-005-standards-20260921';
...
if (mode === 'candidate') {
    ...
    write(out + '/policy.json', { scope: ref(out + '/scope.json'), model: 'gpt-5.6-luna', grading_point_tolerance: 1,
        ...
        reuse_decision: `${V} 기준서형 초안 1세트·1물음을 검토한다. 원 pilot-19-005 sub1의 실측 기록은 없고 발문도 새로 썼으므로 재사용하지 않으며 분모에 넣지 않는다. 기준서형은 물음마다 학습 단위가 따로 생기는데 이 세트는 물음이 하나여서 단위도 하나이고, 모범·부분·오답 역할을 그 물음에 통합하여 3요청으로 실측한다(r17·r21의 기준서형 세트와 같은 형상이다). 모범답안 요청이 사례 지문 없는 독립 물음으로 만점이 되는지를 함께 확인한다. 채점 은행은 이 세트만 담은 부분 은행이다.` });
