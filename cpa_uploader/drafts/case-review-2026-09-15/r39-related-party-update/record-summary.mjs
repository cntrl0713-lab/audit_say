// r39 실행 요약 기록기. 실제 채점이 끝난 뒤 한 번만 실행한다.
//   node cpa_uploader/drafts/case-review-2026-09-15/r39-related-party-update/record-summary.mjs
import fs from 'node:fs';
import { createHash } from 'node:crypto';
const R = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/r39';
const D = 'cpa_uploader/drafts/case-review-2026-09-15/r39-related-party-update';
const SET = 'case-11-related-party-20260921';
...
const summary = {
    version: 1, round: 'r39', draft_version: 'v1', set_id: SET, created_at: new Date().toISOString(),
    route: 'case_single_update',
    updated_set: 'case-11-undisclosed-related-party-20260913',
    edit_note_number: 43,
    ...
    grading: {
        ...
        rows,
        verdict_state_note: '기대 판정과 실제 판정의 상태 차이는 rows에 남긴다. 이 실행에서는 아홉 답안의 모든 criterion 판정이 기대와 같아 상태 차이가 0건이다. 재채점하지 않았다.',
        deviation_detail: '허용 범위(±1점) 밖 결과가 없고 delta가 모두 0이다. 점수 편차를 없애기 위한 반복 호출을 하지 않았다.',
        usd: main.usd,
        provider_token_totals: main.provider_token_totals,
    },
    supplementary: {
        run: false,
        reason: '대표 9답안이 모두 허용 범위 안이고(9/9 = 100%, 목표 95%) 점수·판정이 모두 기대와 같아 보조 요청을 추가하지 않았다. 비용 통제 지침에 따라 허용한 ±1점 편차를 없애기 위한 반복 호출도 하지 않았다. qa.json의 supplementary_cases 세 건은 run=false로 남겨 두었다.',
    },
    ...
    not_done: [
        '정본·공개본·암호화본 반영',
        '운영 DB 등록',
        '원 세트 case-11-undisclosed-related-party-20260913의 활성 릴리스 제외(퇴역)',
