...
//   node cpa_uploader/drafts/case-review-2026-09-15/r01-group-audit-merge/render-sheet.mjs
...
const out = [`# ${set.title}`, '', `병합 초안 \`${set.id}\` · 사례형 ${set.subquestions.length}물음 · ${set.subquestions.reduce((n, q) => n + points(q), 0)}점. 상태는 \`${set.status}\`이며 정본·운영 DB에는 아직 반영하지 않았다.`, ''];
...
for (const kind of ['model', 'partial', 'wrong']) {
  ...
  out.push(`| ${{ model: '저장 모범답안', partial: '대표 부분정답', wrong: '대표 오답' }[kind]} | ${s('sub1').expected_points} / ${s('sub1').actual_points} | ${s('sub2').expected_points} / ${s('sub2').actual_points} |`);
}
...
out.push('', '대표 부분정답·오답 원문과 기대 판정은 [qa.json](qa.json), 원응답·사용량은 [검증 장부](../../../analysis/reviews/case-review-2026-09-15/README.md)에 있다.', '');
