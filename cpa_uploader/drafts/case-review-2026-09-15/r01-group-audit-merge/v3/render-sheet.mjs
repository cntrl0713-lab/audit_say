// v3 sets.json과 execution-v3·supplementary-v3 실제 채점 결과에서 사람이 읽는 문제지를 만든다.
//   node cpa_uploader/drafts/case-review-2026-09-15/r01-group-audit-merge/v3/render-sheet.mjs
...
const obs = (kind) => JSON.parse(fs.readFileSync(`${R}/execution-v3/actual-a/${set.id}--case--${kind}/observation.json`, 'utf8'));
const supp = JSON.parse(fs.readFileSync(`${R}/supplementary-v3/summary.json`, 'utf8'));
const points = (q) => q.criteria.reduce((n, c) => n + c.max_points, 0);
const out = [`# ${set.title}`, '', `병합 초안 \`${set.id}\` v3 · 사례형 ${set.subquestions.length}물음 · ${set.subquestions.reduce((n, q) => n + points(q), 0)}점. 상태는 \`${set.status}\`이며 정본·운영 DB에는 아직 반영하지 않았다.`, '',
  'v2 대비: 사실관계와 출처는 같고, 발문을 “옳지 않은 이유나 수행하였어야 할 절차를 간략히 서술”로 바꾸었으며 옳지 않은 항목마다 이유 또는 보완절차 중 하나에 1점을 준다.', ''];
...
for (const row of supp.rows) {
  ...
  out.push(`| 보조: ${qa.supplementary_cases.find((c) => c.id === row.id).purpose.replace(/\.$/, '')} | ${s('sub1').expected_points} / ${s('sub1').actual_points} | ${s('sub2').expected_points} / ${s('sub2').actual_points} |`);
}
