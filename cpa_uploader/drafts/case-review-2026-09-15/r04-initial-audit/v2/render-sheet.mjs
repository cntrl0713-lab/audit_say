...
//   node cpa_uploader/drafts/case-review-2026-09-15/r04-initial-audit/v2/render-sheet.mjs
import fs from 'node:fs';
const D = 'cpa_uploader/drafts/case-review-2026-09-15/r04-initial-audit/v2';
...
// v2 대표 실측은 execution-v2/actual-a, 보조 실측은 문항 내용이 같은 v1의 supplementary-v1을 읽는다.
...
const out = [`# ${set.title}`, '', `재구성 초안 v2 \`${set.id}\` · 사례형 ${set.subquestions.length}물음 · ${set.subquestions.reduce((n, q) => n + points(q), 0)}점. 상태는 \`${set.status}\`이며 정본·운영 DB에는 아직 반영하지 않았다.`, ''];
...
for (const row of supp.rows) {
  ...
  out.push(`| 보조(v1 문항): ${qa.supplementary_cases.find((c) => c.id === row.id).purpose.replace(/\.$/, '')} | ${ids.map((id) => `${s(id).expected_points} / ${s(id).actual_points}`).join(' | ')} |`);
}
out.push('', '보조 실측은 출처 제목의 줄 표시만 다른 v1 문항으로 실행했다. 대표 답안 원문과 기대 판정은 [qa.json](qa.json), 원응답·사용량은 [검증 장부](../../../../analysis/reviews/case-review-2026-09-15/README.md)에 있다.', '');
