...
//   node cpa_uploader/drafts/case-review-2026-09-15/r34-management-expert-update/render-sheet.mjs
...
const out = [
    `# ${set.title}`,
    '',
    `단독 갱신 초안 \`${set.id}\` · 사례형 ${set.subquestions.length}물음 · ${total}점. 상태는 \`${set.status}\`이며 정본·운영 DB에는 아직 반영하지 않았다.`,
    '',
];
...
const labels = {
    model: '저장 모범답안',
    partial: '대표 부분정답(물음 1은 함정 ④를 옳지 않다고 고르고 ⑤를 빠뜨림, 물음 2는 ⑨의 이유를 빠뜨림, 물음 3은 ⑭를 빠뜨림)',
    wrong: '대표 오답(함정만 옳지 않다고 고르고 옳지 않은 항목은 적절하다고 명시)',
};
