// r34 실행 요약 기록기. 실제 채점이 끝난 뒤 한 번만 실행한다.
//   node cpa_uploader/drafts/case-review-2026-09-15/r34-management-expert-update/record-summary.mjs
import fs from 'node:fs';
import { createHash } from 'node:crypto';
const R = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/r34';
const D = 'cpa_uploader/drafts/case-review-2026-09-15/r34-management-expert-update';
const SET = 'case-08-management-expert-20260921';
...
const summary = {
    version: 1, round: 'r34', draft_version: 'v1', set_id: SET, created_at: new Date().toISOString(),
    route: 'case_single_update',
    updated_set: 'pilot-08-008',
    edit_note_number: 26,
    ...
    grading: {
        ...
        rows,
        verdict_state_note: '기대 판정과 실제 판정의 상태 차이는 rows에 남긴다. 이 실행에서 아홉 답안의 점수는 모두 기대와 정확히 일치했고(delta 0), 상태 차이는 대표 부분정답 물음 1의 crit1 한 건(기대 contradicted, 실제 not_met)이다. 두 상태 모두 0점이므로 비용 통제 지침에 따라 재채점하지 않는다.',
        usd: main.usd,
        provider_token_totals: main.provider_token_totals,
    },
    supplementary: {
        run: false,
        reason: '대표 9답안의 점수가 모두 기대와 정확히 일치했고 허용 범위 밖 결과가 없어 보조 요청을 추가하지 않았다. 비용 통제 지침에 따라 허용한 ±1점 편차를 없애기 위한 반복 호출도 하지 않았다. qa.json의 supplementary_cases 두 건은 run=false로 남겨 두었다.',
    },
    ...
    not_done: [
        '정본·공개본·암호화본 반영',
        '운영 DB 등록',
        '원 세트 pilot-08-008의 활성 릴리스 제외(퇴역)',
