// r21에서 분리 보존하는 기준서형 세트의 후보 은행·실제 채점 입력 생성기.
// 배치 공용 tools/build-round3.mjs는 모든 물음이 question_style 'case'임을 단언하므로 기준서형에는 쓸 수 없다.
// 이 파일은 build-round3.mjs의 절차를 그대로 따르되 기준서형 한 물음에 맞추고, 후보 검증에서
// 현재 정본 + 이 회차의 두 초안(사례형·기준서형)을 함께 메모리에서 validateAuthoringBank로 확인한다.
// 기존 tools/의 파일은 고치지 않는다. 기존 출력이 있으면 쓰지 않는다.
//
//   node --import tsx cpa_uploader/drafts/case-review-2026-09-15/r21-accounting-estimate-merge/build-standards-round.mjs candidate
//   node --import tsx cpa_uploader/drafts/case-review-2026-09-15/r21-accounting-estimate-merge/build-standards-round.mjs execution
...
const SET_ID = 'pilot-11-005-standards-20260921';
const CASE_SET_ID = 'case-11-accounting-estimate-20260921';
...
if (mode === 'candidate') {
    ...
    write(out + '/policy.json', { scope: ref(out + '/scope.json'), model: 'gpt-5.6-luna', grading_point_tolerance: 1,
        ...
        reuse_decision: `${V} 기준서형 초안 1세트·1물음만 검토한다. 원 pilot-11-005의 실측은 발문이 달라 재사용하지 않고 분모에 넣지 않는다. 모범·부분·오답 역할을 3요청으로 실측하며, 모범답안 요청이 사례 지문 없는 독립 물음으로 만점이 되는지를 함께 확인한다. 채점 은행은 이 세트만 담은 부분 은행이다.` });
    ...
    console.log(JSON.stringify({ requests: entries.length,
        ...
        learning_unit: unit, manifest: ref(out + '/grading-manifest.json'), actual_sdk_calls: 0 }, null, 2));
}
