...
for (const spec of specifications) {
  ...
  const set = {
    ...
    subquestions: spec.questions.map(q => ({
      ...
      criteria: q.claims.map((c, i) => ({ id: `${q.id}.c${i + 1}`,
        ...
        claim: c.answer, critical_facts: [{ id: `${q.id}.c${i + 1}.fact`, type: 'conclusion', expected: c.answer },
  ...
  for (const q of spec.questions) {
    ...
    for (const i of all) add(`omit-${i + 1}`, q.claims.filter((_, j) => i !== j).map(c => c.answer).join('\n'),
      ...
      q.omitKeeps?.[i]?.includes(i) ? '결론 문장을 지웠으나 남은 보고 내용이 같은 결론을 명시하므로 해당 판단도 충족한다.' : '다른 독립 명제는 유지하고 해당 요구를 생략한 작성자 기대 사례. 실제 모델 판정은 미실측.');
...
const answers = ['# 모범답안과 배점', '', '독립 채점 명제별 1점이다. 의미가 같은 표현을 인정하며, 정해진 문장 일치 여부로 채점하지 않는다. 공식 근거를 대조한 작성 초안으로, 실제 모델 채점과 사람 검수는 아직 수행하지 않았다.', '', '[문제지](questions.md)', ''];
...
selection.push('', '반복 빈도가 높더라도 회사가 작성한 정보의 적합성(08-003), 미공개 특수관계자 식별 후 조치(11-003)처럼 이미 직접 다루는 요구는 이번에 추가하지 않았다. 기존 9세트 제작 배치도 대조했으며 해당 배치의 재수록은 새 문제로 만들지 않았다. 현재 연결이 없다는 사실만으로 모든 신규 요구를 미출제로 확정한 것은 아니다.', '', '## 근거와 검증 자료', '', edition, '',
  ...
  '작성자 QA는 모범답안·동의 표현·역순·한 문장·명제 누락·반대 의미·조건 경계·빈 답안의 기대 판정과 정수 합산을 확인하는 자료다. 실제 모델의 의미 채점, 정식 검수 receipt, 사람 승인 및 게시·배포는 수행하지 않았다.', '');
