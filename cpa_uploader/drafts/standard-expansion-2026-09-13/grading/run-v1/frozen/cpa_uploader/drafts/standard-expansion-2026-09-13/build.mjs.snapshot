...
export const batch='cpa_uploader/drafts/standard-expansion-2026-09-13';
const raw='cpa_uploader/raw/originals/standard-expansion-2026-09-13';
...
const inputs={KGA:'cpa_uploader/drafts/frequency-gap-2026-09-10/sources/kga-2026-pymupdf-pages.txt',ETHICS:`${raw}/ethics-2024-complete.txt`,LAW:`${raw}/external-audit-act-article9.txt`};
const urls={KGA:'https://www.kicpa.or.kr/board/fileMngr?cmd=down&boardId=acc0102&bltnNo=11786004332051&fileSeq=1&subId=sub06',ETHICS:'https://www.kicpa.or.kr/board/fileMngr?cmd=down&boardId=ethstd03&bltnNo=11735541549550&fileSeq=2&subId=sub06',LAW:'https://www.law.go.kr/lsLinkCommonInfo.do?lsJoLnkSeq=1028002685'};
const editions={KGA:'KICPA 2026년 7월 개정 전문. 해당 기준의 일반 요구를 출제하며 시험 적용연도는 별도로 확정하지 않는다.',ETHICS:'공인회계사윤리기준 2024-12-19 의결 전문. 2026-09-13 개정 공고 및 공개초안 목록 확인; 2026-09-02 공개초안은 확정 본문으로 적용하지 않는다.',LAW:'국가법령정보센터 현행 제9조: 법률 제20896호, 2025-04-01 시행. 2026-09-13 직접 확인. 제9조제5항·제6항에 범위를 한정한다.'};
...
for(const g of groups){
 ...
 const setId=`draft-standard-expansion-20260913-${g.id.toLowerCase()}`;
 const file=`${batch}/${g.id.toLowerCase()}.json`;const refs=[];
 const refFor=span=>{const id=`src-${g.standard.toLowerCase()}-${span.join('-')}`;const q=quote(g.standard,span);if(!text.includes(q))throw Error(`발췌에 없는 인용 ${id}`);if(!refs.some(r=>r.id===id))refs.push({id,file:source,title:`${editions[category(g.standard)]} 원 추출본 L${span[0]}–L${span[1]}`,page:category(g.standard)==='KGA'?`KGA ${g.standard}`:g.standard==='ETHICS'?'공인회계사윤리기준':'외부감사법 제9조',source_quote:q,role:'standard',content_hash:hash(q)});return id;};
...
if(num!==15)throw Error(`물음 수 ${num}`);
...
write(`${batch}/qa-representatives.json`,{version:1,artifact_type:'draft_grading_cases',selected_before_execution:true,model:'gpt-5.6-luna',policy:{tolerance_points:1,target_rate:0.95,budget_usd:null,budget_note:'사용자 지정 금액 상한 없음. 대표 45건으로 초기 실행을 제한하고 제공자 한도 오류에서 중단한다. 내용 오류·허용범위 밖 점수에만 필요한 추가 검사를 한다.',representative_jobs:cases.length,extra_semantic_api_calls:0},cases});
...
const provenance={version:1,checked_on:'2026-09-13',urls,editions,inputs:Object.entries(inputs).map(([kind,file])=>({kind,file,sha256:sha(file)})),originals:Object.entries(pdfs).map(([kind,file])=>({kind,file,sha256:sha(file),prior_collection:'cpa_uploader/raw/collections/2026-09-11-initial/manifest.json'})),law_html:{file:`${raw}/external-audit-act-article9.html`,sha256:sha(`${raw}/external-audit-act-article9.html`)},derivation:{KGA:'보존된 PyMuPDF 전문의 지정 행을 LF로 연결',ETHICS:'pypdf의 페이지별 직접 추출본 지정 행; 페이지 제목만 추가',LAW:'공식 HTML 원본에서 Python 표준 HTMLParser로 script/style을 제외한 텍스트 노드를 추출; 법령 링크 경계가 줄바꿈으로 남음'},context_ranges:contextRanges,working_source:source,raw_excerpt:`${raw}/official-excerpts.txt`,excerpt_sha256:sha(source)};
write(`${batch}/sources/provenance.json`,provenance);
const entries=[...Object.entries(inputs).filter(([k])=>k!=='KGA').map(([k,f])=>({original_path:f,category:'verification',role:'official_text_extraction',source_url:urls[k],edition:editions[k]})),{original_path:`${raw}/external-audit-act-article9.html`,category:'verification',role:'official_html_original',source_url:urls.LAW,edition:editions.LAW},{original_path:`${raw}/ethics-2024-pages.txt`,category:'verification',role:'initial_page_extraction',source_url:urls.ETHICS,edition:editions.ETHICS},{original_path:`${raw}/official-excerpts.txt`,category:'verification',role:'official_text_excerpt'},{original_path:source,category:'verification',role:'working_source_copy',derived_from:`${raw}/official-excerpts.txt`}].map(e=>({...e,sha256:sha(e.original_path),provenance_record:`${batch}/sources/provenance.json`}));
