...
LOCK_FILE = CONTROL / 'runtime-v2-policy/runtime-lock.json'
lock = read(LOCK_FILE)
...
event({'stage': 'start', 'manifest_sha256': sha(MANIFEST), 'sets': len(entries),
       ...
       'policy': 'Policy v2 follow-up; original v1 evidence remains immutable. Full cohort includes all 808 cases; only the two accepted T02-A opposite answers use QA revision 2.'})
completed = []
blocked = None
for row in entries:
    if (HERE / 'HALT').exists():
    ...
    if changed:
        ...
        break
    folder = ROOT / row['output_directory'] / 'phase-two-v2' / row['set_id']
