...
if len(args.skip_plan_id) != len(args.skip_unit):
...
remember(lock_path)
remember(__file__)
helper = CONTROL / 'continue-semantic-partial.ts'
remember(helper)
...
for job in jobs:
    ...
    try:
        ...
        if result.get('status') == 'partial_units_completed':
...
event({'stage': 'finished' if blocked is None else 'paused', 'completed_sets': len(completed),
       'failed_sets': len(failed_sets), 'partial_sets': len(partial_sets), 'excluded_plan_ids': args.exclude_plan_id, 'blocked': blocked})
with summary_file.open('x', encoding='utf-8') as stream:
    json.dump({'finished_at': now(), 'completed': completed, 'partial_sets': partial_sets, 'failed_sets': failed_sets,
               'blocked': blocked, 'excluded_plan_ids': args.exclude_plan_id,
               'sets_without_complete_receipt': len(jobs) - len(completed),
               'sets_not_finished_to_unit_boundary': len(jobs) - len(completed) - len(partial_sets) - len(failed_sets), 'actual_grading_cases': 0,
               'limitation': 'Partial evidence preserves unresolved units and is never a completed receipt. First semantic observations with one exact-cache resume after a transient transport error. A second such error leaves that set incomplete and continues independent assigned sets. Non-pass receipts retain their verdict and need exact-unit repeats; generated-case grading remains separate.'},
