...
for (let i = 2; i < process.argv.length; i++) {
    ...
    if (!['--file', '--cases', '--original-qa', '--runtime-lock', '--output'].includes(key)
        || !process.argv[i + 1] || args[key]) throw Error('지원 인자: --file --cases --original-qa --runtime-lock --output [--validate-only]');
...
if (set.id !== 'pilot-02-006' || original.set_id !== set.id || cases.set_id !== set.id
    ...
    || cases.cases.length !== 2 || new Set(cases.cases.map(test => test.id)).size !== 2) {
    throw Error('N01 기존 넓은 답안 두 건의 별도 회귀 자료여야 함');
}
...
if (args['--validate-only']) {
    console.log(JSON.stringify({
        status: 'case_contract_validated_without_model', api_calls: 0, case_count: cases.cases.length,
...
write('inputs.json', {
    created_at: new Date().toISOString(), model, question_set: set, exploratory_cases: cases,
    hashes, transport: 'production_gradeQuestionSetV3', mock: false,
    policy: '49개 필수 작성자QA에 추가하는 원답안 회귀2개. 각3회 실행. 원기대값 exact와 허용범위 일치를 별도로 기록한다.',
});
void (async () => {
    const records: Array<Record<string, unknown>> = [];
    let stopped: Record<string, unknown> | null = null;
    for (let index = 0; index < cases.cases.length && !stopped; index++) {
    ...
    const changes = changedInputs();
    write('summary.json', {
        finished_at: new Date().toISOString(), model, set_id: set.id, planned_cases: 2,
        planned_attempts: 6, actual_attempts: records.length, records, stopped, changed_inputs: changes,
