...
for option in ['manifest', 'runtime-lock', 'semantic-controller-summary', 'first-recovered-receipt', 'run-name']:
...
semantic = read(args.semantic_controller_summary)
if semantic.get('blocked'):
    raise RuntimeError('Finish or explicitly resolve the semantic controller before grading')
entries = [row for row in manifest['entries'] if row['package'] in OWNED]
if len(entries) != 18:
    raise RuntimeError('Expected the 18 assigned sets')
first = read(args.first_recovered_receipt)
if len(first['reviews']) != 1 or first['reviews'][0]['set_id'] != 'pilot-02-006':
    raise RuntimeError('The explicitly recovered first receipt must be T02-A')
receipts = {'T02-A': {'receipt': args.first_recovered_receipt, 'receipt_sha256': sha(args.first_recovered_receipt)}}
for row in semantic['completed']:
...
for file in [args.runtime_lock, args.semantic_controller_summary, __file__, CONTROL / 'run-generated-qa.ts']:
    remember(file)
