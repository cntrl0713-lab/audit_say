...
LOCK_FILE = CONTROL / 'runtime-v4-stable/runtime-lock.json'
lock = read(LOCK_FILE)
...
for row in entries:
    row['required_qa_cases'] = row['qa_cases']
    row['original_qa_file'] = row['qa_file']
    row['original_qa_sha256'] = row['qa_sha256']
    if row['plan_id'] == 'T02-A':
        row['qa_file'] = 'cpa_uploader/drafts/delegated-authoring-2026-09-11/n01/phase-two-followup/qa-v3/qa-cases-t02-a.json'
        row['qa_sha256'] = 'afa2daf9d4f7394c9de67c06c03d9b16b030c22d1b96bc40287bf4b4cef9287b'
    if row['plan_id'] == 'T01-A':
        row['qa_file'] = 'cpa_uploader/drafts/delegated-authoring-2026-09-11/n01/phase-two-followup/t01-a-qa-v2/qa-cases-t01-a.json'
        row['qa_sha256'] = 'e6399330557edfdea7636e4da89ec512d04722e273cc373e527dd3942d6f37a6'
    if row['plan_id'] == 'T04-B':
        row['qa_file'] = 'cpa_uploader/drafts/delegated-authoring-2026-09-11/n01/phase-two-followup/t04-b-qa-v2/qa-cases-t04-b.json'
        row['qa_sha256'] = '73a731cfe1c4058f266c8b74c8b91e78031bda67ec2bf76a5f248bee3a1b8a93'
    if row['plan_id'] == 'T16-A':
        row['qa_file'] = 'cpa_uploader/drafts/delegated-authoring-2026-09-11/n06/phase-two-v4/qa-cases-t16-a-remaining.json'
        row['qa_sha256'] = '2e84308c4f9262f726751dd505de9f5e014539c719ac806e9d2c8e57e3b15652'
        row['qa_cases'] = 33
if len(entries) != 18:
...
event({'stage': 'start', 'manifest_sha256': sha(MANIFEST), 'sets': len(entries),
       'planned_new_cases': sum(row['qa_cases'] for row in entries), 'required_cases': sum(row['required_qa_cases'] for row in entries), 'already_verified_v4_cases': 8, 'reuse_linkage': 'cpa_uploader/drafts/delegated-authoring-2026-09-11/n06/phase-two-v4/t16-a-reused-cases-linkage.json', 'parallel_sets': 1,
       ...
       'policy': 'Policy v4 full required QA follow-up; original v1 evidence remains immutable. Required cohort808 = new800 + v4 verified8 with recomputed request/schema and code identity. T02-A/T01-A/T04-B use accepted follow-up QA. No v3 grading results count as v4 evidence.'})
completed = []
blocked = None
for row in entries:
    if (HERE / 'HALT').exists():
    ...
    if changed:
        ...
        break
    folder = ROOT / row['output_directory'] / 'phase-two-v4' / row['set_id']
