"""Read-only progress and mismatch details for this worker's v4 author QA."""
import json
import pathlib
import sys
...
for entry in entries:
    folder = root / entry['output_directory'] / 'phase-two-v4' / entry['set_id'] / 'author-qa-01'
    ...
    progress.append({
        ...
        'reused_verified_v4_cases': 8 if entry['plan_id'] == 'T16-A' else 0,
...
if '--compact' in sys.argv:
    payload = {'completed_sets': sum(row['completed'] for row in progress),
               'new_cases_saved': sum(row['cases'] for row in progress),
               'recent_progress': progress[-2:], 'issues': issues}
else:
    payload = {'progress': progress, 'issues': issues}
