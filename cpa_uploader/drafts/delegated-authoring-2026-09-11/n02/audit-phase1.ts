...
const results = manifest.sets.map((m: any, i: number) => {
  ...
  try { const p = prepareSemanticReview(set, { bank: [...initial, ...sets.filter((_: any, j: number) => i !== j)], authoringPlan: plan, maxInputChars: 400000 }); semanticPreparation = { success: true, request_chars: p.requestChars, explicit_max_input_chars: 400000, bank_hash: p.bankHash, units: p.units.length, content_hash: p.contentHash }; }
...
const result = { checked_at: new Date().toISOString(), phase: 'phase1_no_model_calls', catalog_fingerprint: catalog.fingerprint,
  comparison_file: initialFile, comparison_sha256: hash(fs.readFileSync(initialFile)), initial_sets: initial.length, n02_additional_sets: sets.length, comparison_is_final: false,
  results, in_memory_bank: inMemory, conflicts: draftConflicts(sets, initial),
