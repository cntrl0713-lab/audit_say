...
const config=read(taskFile); config.tasks=config.tasks.filter(t=>['T06-B','T05-B'].includes(t.plan_id)); const control=own+'/semantic-exact-diagnostics-refill02-independent-control';
...
if(config.tasks.length!==2||config.tasks.some(t=>t.preflight_exit_code!==0||read(t.preflight_output).status!=='preflight_valid'))throw Error('Two independent original unit preflights required');
...
write(control+'/inputs.json',{created_at:new Date().toISOString(),task_file:taskFile,task_file_sha256:hash(taskFile),single_API_stream:true,exact_attempt_instructions:true,planned_actual_requests:4,checked_hashes:Object.fromEntries(snapshots),no_repromotion_of_original_receipts:true});
let job=0;
try{
 ...
 write(control+'/queue-completed.json',{finished_at:new Date().toISOString(),actual_valid_observations:4,original_valid_observations:2,units_with_three:2,receipt_mutations:0});
