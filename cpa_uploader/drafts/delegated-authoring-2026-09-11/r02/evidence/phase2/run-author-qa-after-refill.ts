...
function safeError(error: unknown) {
    ...
    const chain: Array<{name?:string;status?:number;code?:string;retryable?:boolean;request_id?:string}> = [];
    while(queue.length&&chain.length<12){
        ...
        chain.push({name:typeof item.name==='string'?item.name:undefined,status:typeof item.status==='number'?item.status:undefined,code:typeof item.code==='string'?item.code:undefined,retryable:typeof item.retryable==='boolean'?item.retryable:undefined,request_id:typeof item.request_id==='string'?item.request_id:typeof item.requestID==='string'?item.requestID:undefined});
        for(const key of ['cause','error','body']){const child=item[key];if(child&&typeof child==='object')queue.push(child);else if(key==='body'&&typeof child==='string'){try{queue.push(JSON.parse(child));}catch{ /* Never emit raw bodies. */ }}}
    }
    const top=error as Error&{code?:string;status?:number;retryable?:boolean},credit=chain.find(v=>['credit_balance_exhausted','insufficient_quota'].includes(v.code||''));
    return {name:top.name,message:top.message,code:credit?.code||top.code,status:credit?.status||top.status,retryable:credit?false:top.retryable,credit_or_quota_exhausted:Boolean(credit),safe_cause_chain:chain};
}
...
const hashFiles=[args['--file'],args['--qa'],...codeFiles,...sourceFiles,process.argv[1]];
        ...
        for(let attempt=1;attempt<=repetitions;attempt++){
            ...
            try{
                ...
                record={set_id:set.id,case_id:test.id,attempt,started_at,finished_at:new Date().toISOString(),model,answers,expected:test,trace,
                    transport:'live_model_attempt',matched:false,error:safeError(error)};
                executionError=true;
            }
