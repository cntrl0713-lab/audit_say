...
const safeError=error=>{
 const seen=new Set(),safe=[],queue=[error];
 while(queue.length&&safe.length<12){const value=queue.shift();if(!value||typeof value!=='object'||seen.has(value))continue;seen.add(value);safe.push({name:typeof value.name==='string'?value.name:undefined,status:typeof value.status==='number'?value.status:undefined,code:typeof value.code==='string'?value.code:undefined,retryable:typeof value.retryable==='boolean'?value.retryable:undefined,request_id:typeof value.request_id==='string'?value.request_id:typeof value.requestID==='string'?value.requestID:undefined});for(const key of ['cause','error','body']){const nested=value[key];if(nested&&typeof nested==='object')queue.push(nested);else if(key==='body'&&typeof nested==='string'){try{queue.push(JSON.parse(nested));}catch{ /* Do not log a raw body. */ }}}}
 const credit=safe.find(v=>['credit_balance_exhausted','insufficient_quota'].includes(v.code));
 return {name:error.name,message:error.message,code:credit?.code||error.code,status:credit?.status||error.status,credit_or_quota_exhausted:Boolean(credit),retryable:credit?false:error.retryable,safe_cause_chain:safe};
};
...
}catch(error){record={set_id:set.id,case_id:test.id,attempt:1,started_at,finished_at:new Date().toISOString(),model,transport:'live_model_attempt',answers,expected:test,request_hash,schema_hash,raw_judgment:raw,trace,matched:false,error:safeError(error),reference};}
