...
type RawRow = { set_id: string; unit_id: string; attempt: number; model: string; performed_at: string; input_hash: string; schema_hash: string; content_hash: string; bank_hash: string; source_files: Identity[]; response: unknown; error?: string; error_code?: string; instructions_hash?: string; record_kind?: string };
...
function parseArgs(values: string[]) {
    ...
    for (let i = 0; i < values.length; i++) {
        ...
        if (!['--manifest', '--plan-id', '--runtime-lock', '--resume-log', '--output'].includes(key) || !values[i + 1] || values[i + 1].startsWith('--')) throw Error('지원인자 --manifest --plan-id --runtime-lock --resume-log(반복가능) --output --cache-only 또는 --execute');
    ...
    return { values: args, logs, execute };
}
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
    ...
    const cacheErrors: Array<{ log: string; line: number; unit_id: string; error: string }> = [];
    for (const log of parsed.logs) {
        ...
        for (const [index, row] of rows.entries()) {
            ...
            if (row.error || row.response == null) { cacheErrors.push({ log, line: index + 1, unit_id: row.unit_id, error: row.error || 'null response' }); continue; }
    ...
    const audit = { started_at: new Date().toISOString(), mode: parsed.execute ? 'execute' : 'cache_validation_only', manifest: manifestFile, plan_id: entry.plan_id, set_id: set.id,
        ...
        skipped_error_rows: cacheErrors, missing_units: prepared.units.filter(u => !cached.has(u.id)).map(u => u.id), actual_api_requests: 0,
    ...
    try {
        for (const unit of prepared.units) {
            ...
            for (let attempt = 1; attempt <= 2; attempt++) {
                ...
                const meta: Record<string, unknown> = { set_id: set.id, unit_id: unit.id, content_hash: prepared.contentHash, bank_hash: prepared.bankHash, source_files: prepared.sourceFiles,
                    attempt, model, performed_at: new Date().toISOString(), input_hash: request.input_hash, schema_hash: request.schema_hash, record_kind: 'new_actual_model_request', new_api_request: true };
        ...
        const receipt = completeSemanticReview(prepared, { units: chunks.flatMap(c => c.units), cases: chunks.flatMap(c => c.cases), notes: chunks.flatMap(c => c.notes) },
            { method: 'model_reasoned', model, performed_at: new Date().toISOString(), description: `실제 과거 모델 근거 ${cached.size}단위를 원입력·스키마·모델·공통코드로 확인하고 직접 재ground하였다. 빠진 ${liveSuccessful}단위는 신규 실제 API ${liveRequests}요청으로 검수했다. 출처/코호트와 독립 actual grading은 별도다. cache-audit 및 raw chunks 참조.` });
...
if (process.argv[1] && /resume-semantic\.ts$/i.test(process.argv[1])) {
