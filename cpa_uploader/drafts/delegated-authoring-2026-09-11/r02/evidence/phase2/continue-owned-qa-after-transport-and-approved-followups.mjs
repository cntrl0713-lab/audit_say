...
const lockFile=base+'/runtime-v5-bank-v3/runtime-lock.json',lock=read(lockFile),manifest=read(lock.manifest_file),phase='phase-two-v5-bank-v3-resume-followups',control=own+'/'+phase+'-control';
...
const oneHelper=own+'/run-one-author-case-locked.mjs',fullHelper=base+'/run-author-qa.ts';
const recovered=drafts+'/n02/evidence/phase2/phase-two-v5-bank-v3-resume/pilot-08-007/customer-receipt-missing-third/summary.json';
...
write(control+'/queue-inputs.json',{created_at:new Date().toISOString(),manifest:lock.manifest_file,runtime_lock:lockFile,identities,model:gradingModelName(),single_stream:true,small_executions:24,remaining_required_sets:9,remaining_required_cases:472,required_total:835,already_covered_required_cases:363,T07_A_override:{file:extraQA[2],sha256:hash(extraQA[2]),count:73,changed_expected_cases:2,all_original_answers_preserved:true,old71_unmodified_expected_cases_reused:true},recovered_timeout_case:{file:recovered,sha256:hash(recovered),failed_attempt_counted_as_valid:false},previous_evidence_controls:[own+'/phase-two-v4-bank-v2-control',own+'/phase-two-v5-source-ranges-control',own+'/phase-two-v5-bank-v3-control'],note:'Parent authorized same-current-input limited recovery and continued independent sets. Normal completed cases are not restarted. Single-case helper requests one production grading execution, so prior 2 valid executions require only 1 replacement execution.'});
...
try{
 ...
 write(control+'/queue-completed.json',{finished_at:new Date().toISOString(),required_total:835,required_already_covered:363,newly_completed_required_sets:completed,newly_completed_required_cases:472,small_executions:24,all_original_evidence_preserved:true});
