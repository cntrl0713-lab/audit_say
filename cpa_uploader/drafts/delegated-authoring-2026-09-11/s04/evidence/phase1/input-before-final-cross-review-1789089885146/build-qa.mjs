...
const implied = { 'T10-C/sub2': { 0: [1] }, 'T12-C/sub1': { 0: [1] }, 'T12-C/sub2': { 0: [1] }, 'T12-D/sub1': { 0: [1, 2] } };
const oppositeAlso = {};
const manifests = [];
for (const d of definitions) {
  ...
  for (const [qi, q] of set.subquestions.entries()) {
    ...
    if (key === 'T10-B/sub3') {
      ...
      add('two-paths-compulsory', 'condition_boundary', '경영진 조사·필요한 수정요청과 감사인의 추가절차 성격·시기·범위 조정은 모든 경우 반드시 둘 다 수행하여야만 한다.', [], [], '선택 가능한 경로를 예외 없는 동시 의무로 바꾼다.');
    }
    ...
    if (key === 'T10-C/sub2') {
      ...
      add('partial-reliability-list', 'condition_boundary', '원천이 외부이고 합계가 일치하므로 비교가능성·성격과 관련성·작성통제는 고려하지 않아도 된다.', [], [], '발문에 요구된 종합 고려의 일부를 명시적으로 제외했다.');
      add('expectation-only-no-reliability', 'alternative_valid', q.model_answer[2], [2], [], '기대치 정확성 평가만으로 기초데이터 신뢰성 평가가 이미 수행됐다고 간주하지 않는다.');
    }
    ...
    if (key === 'T12-C/sub3') {
      add('performance-materiality-only', 'condition_boundary', '전체 중요성의 적합성은 확인하지 않고 수행중요성만 다시 계산한다.', [], [], '450.10과수행중요성을교체한답.');
    ...
    if (key === 'T12-D/sub1') {
      ...
      add('suspicion-without-conclusion', 'condition_boundary', '성실성에 대한 우려가 조금이라도 제기되면 책임진술을 믿을 수 없다는 결론이 없어도 무조건 의견거절이다.', [], [], '단순의문과불신결론경계.');
    }
    if (key === 'T12-D/sub2') {
      ...
      add('core-information-without-transactions', 'condition_boundary', '경영진은 합의한 모든 정보를 제공하고 접근을 허용하였다는 진술을 하되 거래의 기록과 재무제표 반영은 확인할 필요가 없다.', [], [], '두번째책임영역의거래완전성부분누락.');
    }
