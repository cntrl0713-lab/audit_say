...
const s02 = read('cpa_uploader/drafts/delegated-authoring-2026-09-11/s02/draft-manifest.json').sets.map((s: any) => read(s.file)[0]);
const r01Folder = 'cpa_uploader/drafts/delegated-authoring-2026-09-11/r01';
const r01 = ['04-320','09-501','09-505','10-530','12-560','12-570'].map(k => { const value = read(`${r01Folder}/draft-${k}-freq01.json`); return Array.isArray(value) ? value[0] : value; });
const comparison = [...initial.filter((s: any) => !r01.some((r: any) => r.id === s.id)), ...r01, ...n02, ...n03, ...s02];
...
const results = manifest.sets.map((m: any, i: number) => {
  ...
  try { const p = prepareSemanticReview(set, { bank: [...comparison, ...sets.filter((_: any, j: number) => i !== j)], authoringPlan: plan, maxInputChars: 500000 }); semanticPreparation = { success: true, request_chars: p.requestChars, explicit_max_input_chars: 500000, bank_hash: p.bankHash, units: p.units.length, content_hash: p.contentHash }; }
...
const result = { checked_at: new Date().toISOString(), phase: 'phase1_no_model_calls', catalog_fingerprint: catalog.fingerprint,
  comparison_file: initialFile, comparison_sha256: hash(fs.readFileSync(initialFile)), initial_sets: initial.length, n02_additional_sets: n02.length, n03_additional_sets: n03.length, s02_additional_sets: s02.length, r01_replaced_sets: r01.length, comparison_sets_before_own_peers: comparison.length, comparison_is_final: false,
