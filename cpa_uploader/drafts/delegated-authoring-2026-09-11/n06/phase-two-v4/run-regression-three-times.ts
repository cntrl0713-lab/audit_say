...
const allowed=path.resolve('cpa_uploader/drafts/delegated-authoring-2026-09-11/n06')+path.sep;
...
const runtimeLockFile='cpa_uploader/analysis/reviews/delegated-authoring-2026-09-11/runtime-v4-stable/runtime-lock.json';
const runtimeLock=read(runtimeLockFile);
for(const row of [...runtimeLock.code_files,...runtimeLock.source_files,runtimeLock.comparison_bank,{file:runtimeLock.manifest_file,sha256:runtimeLock.manifest_sha256}]){
 if(sha(fs.readFileSync(row.file))!==row.sha256)throw Error('v4 runtime lock changed: '+row.file);
}
if(gradingModelName()!==runtimeLock.settings.grading_model)throw Error('v4 model mismatch');
const sourceFiles=[...new Set(set.source_refs.map(ref=>ref.file))];
const hashFiles=[args['--file'],args['--qa'],...codeFiles,...sourceFiles,runtimeLockFile,fileURLToPath(import.meta.url)];
const hashes=Object.fromEntries(hashFiles.map(file=>[file,sha(fs.readFileSync(file))]));
const model=gradingModelName();
fs.mkdirSync(output,{recursive:true});
fs.writeFileSync(path.join(output,'inputs.json'),JSON.stringify({created_at:new Date().toISOString(),model,question_set:set,qa,hashes,selected:args['--only']||null,planned_repetitions_per_case:3,transport:'production_gradeQuestionSetV3',mock:false},null,2)+'\n',{flag:'wx'});
