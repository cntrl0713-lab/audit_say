...
const waveFile='cpa_uploader/analysis/reviews/delegated-authoring-2026-09-11/wave-one-ready/manifest.json';
const wave=read(waveFile);
const active=new Map<string,QuestionSetV3>(read(bankFile).map((s:QuestionSetV3)=>[s.id,s]));
for(const s of wave.validation.sets as QuestionSetV3[]) active.set(s.id,s);
const bank:QuestionSetV3[]=[...active.values()];
...
const report={wave_file:waveFile,wave_sha256:hash(fs.readFileSync(waveFile)),domain_errors:domain.errors,artifact_type:'n06_static_validation',version:1,performed_at:new Date().toISOString(),comparison_file:bankFile,comparison_sha256:hash(fs.readFileSync(bankFile)),comparison_sets:bank.length,own_sets:sets.length,conflicts,outputs,model_calls:0,semantic_review:'not_run',live_grading:'not_run',status:domain.errors.length||conflicts.length||outputs.some(o=>o.errors.length)?'fail':'pass'};
