...
const previous=read('cpa_uploader/drafts/delegated-authoring-2026-09-11/r01/phase-two-v5/author-queue-author5b.json');
const permittedStop=previous.stopped?.reason==='pause_requested_at_set_boundary'||(previous.stopped?.reason==='execution_failure'&&previous.stopped.last_plan_id==='T09-C'&&previous.completed.at(-1)?.execution_failure);
...
const output=path.join(directory,'phase-two-v5-recheck-01');if(fs.existsSync(output))throw Error('Output exists');fs.mkdirSync(output,{recursive:true});
fs.writeFileSync(path.join(output,'inputs.json'),JSON.stringify({started_at:new Date().toISOString(),runtime_lock_file:lockFile,runtime_lock_sha256:sha(lockFile),input_records:records,cases:['sub1/omit-sentence-1','sub1/implicit-judgment'],question_changed:false,original_qa_changed:false,runner_sha256:sha(fileURLToPath(import.meta.url))},null,2)+'\n',{flag:'wx'});
const completed=[];let stopped=null;
for(const id of ['sub1/omit-sentence-1','sub1/implicit-judgment']){
 let count=0,round=0;
 while(count<3){
  guard();round++;
  ...
  const observations=summary.records.map(r=>{const file=path.join(path.dirname(summaryFile),r.file),o=read(file);return{file,sha256:sha(file),case_id:o.case_id,request_hash:o.request_hash,schema_hash:o.schema_hash,model:o.model,transport:o.transport,expected:o.expected,score:o.result.score,judgments:o.raw_judgment.subquestions.find(s=>s.subquestion_id==='sub1').verdicts,matched:o.matched};});
...
const result={finished_at:new Date().toISOString(),runtime_lock:{file:lockFile,sha256:sha(lockFile)},planned_cases:2,recorded_observations:completed.length,cases:['sub1/omit-sentence-1','sub1/implicit-judgment'].map(id=>({id,observations:completed.filter(o=>o.case_id===id).length,scores:completed.filter(o=>o.case_id===id).map(o=>o.score),all_matched:completed.filter(o=>o.case_id===id).every(o=>o.matched)})),stopped,records:completed,original_evidence_preserved:true};
