...
const [planId,phase,label='run1',lockPath='cpa_uploader/analysis/reviews/delegated-authoring-2026-09-11/final-153-v1/runtime-lock.json',priorSemantic]=process.argv.slice(2);
...
const folder=path.join(path.dirname(entry.file),'phase-two-v1',planId.toLowerCase(),label);
...
const cliArgs=phase==='author'?['--file',entry.file,'--qa',entry.qa_file,'--output',output]:['--file',entry.file,'--plan',entry.plan_files[0].file,'--bank',lock.comparison_bank.file,...(phase==='generated'?['--review-input',priorSemantic,'--grade-cases']:[]),'--output',output];
