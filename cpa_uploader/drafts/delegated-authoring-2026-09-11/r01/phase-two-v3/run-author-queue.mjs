...
const lockFile=process.argv[3]||'cpa_uploader/analysis/reviews/delegated-authoring-2026-09-11/runtime-v3-stable/runtime-lock.json';
...
for(const entry of entries){
 ...
 const summaryPath=path.join(path.dirname(entry.file),'phase-two-v3',entry.plan_id.toLowerCase(),label,'author-qa','summary.json');
 ...
 if(!executionFailure)for(const caseId of historicalFailures[entry.plan_id]||[]){
  ...
  while(observed<3){
   ...
   const recheckPath=path.join(path.dirname(entry.file),'phase-two-v3',entry.plan_id.toLowerCase(),recheckLabel,'author-qa/summary.json');
 ...
 record.mismatched_case_ids=[...new Set([...record.mismatched_case_ids,...record.historical_rechecks.flatMap(x=>x.mismatched_case_ids)])];
