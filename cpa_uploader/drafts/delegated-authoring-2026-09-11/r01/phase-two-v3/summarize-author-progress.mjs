...
const sets=entries.map(entry=>{
 const folder=path.join(path.dirname(entry.file),'phase-two-v3',entry.plan_id.toLowerCase());
 const runs=fs.existsSync(folder)?fs.readdirSync(folder).filter(name=>name.startsWith('author3')).map(name=>path.join(folder,name,'author-qa','summary.json')).filter(file=>fs.existsSync(file)):[];
