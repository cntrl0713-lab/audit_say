...
const lockFile=control+'/runtime-v5-bank-v3-after-refill-02/runtime-lock.json',lock=read(lockFile),manifest=read(lock.manifest_file);
const entries=manifest.entries.filter(e=>['R01','N04','N05','S03'].includes(e.package));
const execute=process.argv.includes('--execute'),label=process.argv.find(a=>a.startsWith('--label='))?.slice(8)||'generated-bank-v3-refill02-01';
...
for(const entry of chosen){
 guard();
 const semanticCandidates=['semantic-v5-bank-v3-complete-units-root-01-1','semantic-v5-bank-v3-after-refill-root-01-1','semantic-v5-bank-v3-root-01-1'].map(label=>path.join(entry.output_directory,'phase-two-v5',entry.set_id,label,'review/semantic.json'));
