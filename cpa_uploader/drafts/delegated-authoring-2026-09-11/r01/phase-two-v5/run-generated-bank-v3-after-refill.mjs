...
const execute=process.argv.includes('--execute'),label=process.argv.find(a=>a.startsWith('--label='))?.slice(8)||'generated-bank-v3-after-refill-01';
...
for(const entry of chosen){
 guard();
 const semantic=path.join(entry.output_directory,'phase-two-v5',entry.set_id,'semantic-v5-bank-v3-root-01-1/review/semantic.json');
 if(!fs.existsSync(semantic)){completed.push({plan_id:entry.plan_id,semantic,available:false,api_calls:0});if(execute){stopped={reason:'semantic_receipt_missing',plan_id:entry.plan_id};break;}continue;}
 const receipt=read(semantic).reviews.find(r=>r.set_id===entry.set_id);
 if(receipt?.verdict!=='pass'){completed.push({plan_id:entry.plan_id,semantic,available:true,verdict:receipt?.verdict,api_calls:0});if(execute){stopped={reason:'semantic_receipt_not_pass',plan_id:entry.plan_id};break;}continue;}
...
fs.writeFileSync(outputFile,JSON.stringify({recorded_at:new Date().toISOString(),mode:execute?'actual_model':'preflight_api_0',runtime_lock:{file:lockFile,sha256:sha(lockFile)},runner_sha256:sha(fileURLToPath(import.meta.url)),selected_plan_ids:chosen.map(e=>e.plan_id),completed,stopped,original_semantic_receipts_unchanged:true,older_v2_generated_evidence_preserved:true},null,2)+'\n',{flag:'wx'});
