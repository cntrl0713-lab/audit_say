...
if(sha(lockFile)!==transition.new_lock.sha256)throw Error('Runtime lock changed');
...
const sets=manifest.entries.filter(e=>['R01','N04','N05','S03'].includes(e.package)).map(entry=>{
 ...
 const qa=read(entry.qa_file),observed=new Set(records.map(r=>r.case_id)),missing=qa.cases.filter(c=>!observed.has(c.id)).map(c=>c.id),failures=[...new Set(records.filter(r=>r.matched===false).map(r=>r.case_id))];
 return{plan_id:entry.plan_id,set_id:entry.set_id,package:entry.package,expected_cases:entry.qa_cases,observed_unique_cases:observed.size,full_case_coverage:missing.length===0,missing_case_ids:missing,mismatched_case_ids:failures,recorded_attempts:records.length,live_model_attempts:records.filter(r=>r.transport==='live_model').length,production_no_model_attempts:records.filter(r=>r.transport==='production_empty_answer_no_model').length,reused_model_observations:records.filter(r=>r.reused).length,trace_events:records.reduce((sum,r)=>sum+r.trace_events,0),failure_repetitions:failures.map(id=>({case_id:id,observations:records.filter(r=>r.case_id===id).length,scores:records.filter(r=>r.case_id===id).map(r=>r.points),matches:records.filter(r=>r.case_id===id).map(r=>r.matched)})),summaries,records};
});
const sum=key=>sets.reduce((total,s)=>total+s[key],0);
const report={recorded_at:new Date().toISOString(),scope:'Assigned 16 sets: preserved v4 completed 7 sets plus v5 author5-prefixed completed runs with the same grader, model, question, plan, QA and source inputs. Experimental T12-A v2 and all generated QA excluded.',runtime_lock:{file:lockFile,sha256:sha(lockFile)},manifest:{file:lock.manifest_file,sha256:sha(lock.manifest_file)},prior_snapshot:{file:priorFile,sha256:sha(priorFile)},transition_proof:{file:transitionFile,sha256:sha(transitionFile)},sets:sets.length,completed_sets:sets.filter(s=>s.full_case_coverage).length,expected_unique_cases:sum('expected_cases'),observed_unique_cases:sum('observed_unique_cases'),recorded_attempts:sum('recorded_attempts'),live_model_attempts:sum('live_model_attempts'),production_no_model_attempts:sum('production_no_model_attempts'),reused_model_observations:sum('reused_model_observations'),mismatched_case_count:sets.reduce((total,s)=>total+s.mismatched_case_ids.length,0),trace_events:sum('trace_events'),details:sets};
