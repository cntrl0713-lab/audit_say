...
const selected=['T13-A','T14-A','T14-B','T09-C','T09-D','T05-C','T13-B','T13-C'];
...
const generated=read(path.join(folder,'generated-four-execute.json'));
if(generated.records.length!==4||generated.records.some(r=>r.stopped||!r.summary))throw Error('Generated stream has not completed');
const priorAuthor=read(path.join(folder,'author-queue-author5.json'));
if(priorAuthor.completed.length!==1||priorAuthor.completed[0].plan_id!=='T11-B'||!priorAuthor.completed[0].all_case_coverage||priorAuthor.completed[0].execution_failure)throw Error('T11-B author stage is incomplete');
if(!process.env.OPENAI_API_KEY)throw Error('Credential missing; use --env-file=.env.local');
const label=process.argv.find(a=>a.startsWith('--label='))?.slice(8)||'author5b';
