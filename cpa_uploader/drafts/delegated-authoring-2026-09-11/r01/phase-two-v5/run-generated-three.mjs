// Three coordinator-approved semantic pass receipts; one sequential grader stream.
...
const entries=[['T04-A','draft-04-320-freq01'],['T09-A','draft-09-501-freq01'],['T09-B','draft-09-505-freq01']];
const records=[];
for(const [id,setId]of entries){
 ...
 const stdout=path.join(folder,'generated-three-'+mode+'-'+id.toLowerCase()+'.stdout.log'),stderr=path.join(folder,'generated-three-'+mode+'-'+id.toLowerCase()+'.stderr.log');
...
fs.writeFileSync(path.join(folder,'generated-three-'+mode+'.json'),JSON.stringify({finished_at:new Date().toISOString(),mode,runtime_lock:{file:lock,sha256:sha(lock)},manifest:{file:manifest,sha256:sha(manifest)},records},null,2)+'\n',{flag:'wx'});
if(records.length!==3||records.some(r=>r.exit_code!==0))process.exitCode=1;
