...
const [planId,phase,label='run1',lockPath='cpa_uploader/analysis/reviews/delegated-authoring-2026-09-11/runtime-v4-stable/runtime-lock.json',priorSemantic,onlyCase]=process.argv.slice(2);
...
const folder=path.join(path.dirname(entry.file),'phase-two-v4',planId.toLowerCase(),label);
...
let qaFile=entry.qa_file;
const reuseFile='cpa_uploader/drafts/delegated-authoring-2026-09-11/r01/phase-two-v4/reuse-index.json';
if(phase==='author'&&!onlyCase){
 const reuseIndex=read(reuseFile);
 if(reuseIndex.runtime_lock_sha256!==sha(lockPath))throw Error('Reuse lock differs');
 const reused=reuseIndex.entries.find(item=>item.plan_id===planId);
 if(reused){
  const original=read(entry.qa_file),ids=new Set(reused.cases.map(item=>item.case_id));
  for(const item of reused.proof_files)if(sha(item.file)!==item.sha256)throw Error('Reuse evidence changed');
  qaFile=path.join(folder,'remaining-author-cases.json');
  write('remaining-author-cases.json',{...original,cases:original.cases.filter(item=>!ids.has(item.id))});
  write('per-case-reuse.json',reused);
  inputRecords.push({file:reuseFile,sha256:sha(reuseFile)},...reused.proof_files,{file:qaFile,sha256:sha(qaFile)});
 }
}
...
const cliArgs=phase==='author'?['--file',entry.file,'--qa',qaFile,'--output',output,...(onlyCase?['--only',onlyCase]:[])]:['--file',entry.file,'--plan',entry.plan_files[0].file,'--bank',lock.comparison_bank.file,...(phase==='generated'?['--review-input',priorSemantic,'--grade-cases']:[]),'--output',output];
