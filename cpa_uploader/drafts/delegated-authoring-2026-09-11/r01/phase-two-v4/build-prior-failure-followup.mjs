...
for(const [planId,ids] of Object.entries(selected))for(const id of ids){
 ...
 if(observations.length<3)throw Error('Insufficient v4 repetitions: '+planId+'/'+id);
...
const lock='cpa_uploader/analysis/reviews/delegated-authoring-2026-09-11/runtime-v4-stable/runtime-lock.json';
const output={recorded_at:new Date().toISOString(),scope:'The five historical author QA mismatches, fresh v4 observations only; the original v1/v2 evidence remains unchanged.',predecessor:{file:predecessor,sha256:sha(predecessor)},runtime_lock:{file:lock,sha256:sha(lock)},cases:cases.length,attempts:cases.reduce((sum,item)=>sum+item.observed_attempts,0),all_matched:cases.every(item=>item.all_matched),actual_model_calls_by_this_aggregation_script:0,details:cases};
fs.writeFileSync(path.join(here,'prior-five-failures-v4-followup.json'),JSON.stringify(output,null,2)+'\n',{flag:'wx'});
