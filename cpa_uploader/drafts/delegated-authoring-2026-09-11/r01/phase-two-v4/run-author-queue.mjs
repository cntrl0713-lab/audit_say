...
const lockFile=process.argv[3]||'cpa_uploader/analysis/reviews/delegated-authoring-2026-09-11/runtime-v4-stable/runtime-lock.json';
...
const records=[];
const reuseIndex=JSON.parse(fs.readFileSync(path.join(folder,'reuse-index.json'),'utf8'));
...
for(const entry of entries){
 ...
 const summaryPath=path.join(path.dirname(entry.file),'phase-two-v4',entry.plan_id.toLowerCase(),label,'author-qa','summary.json');
 ...
 record.reused_cases=reuseIndex.entries.find(item=>item.plan_id===entry.plan_id)?.cases??[];
 record.covered_case_count=record.recorded_cases+record.reused_cases.length;
 record.expected_author_cases=entry.qa_cases;
 if(!executionFailure)for(const caseId of historicalFailures[entry.plan_id]||[]){
  ...
  while(observed<3){
   ...
   const recheckPath=path.join(path.dirname(entry.file),'phase-two-v4',entry.plan_id.toLowerCase(),recheckLabel,'author-qa/summary.json');
 ...
 record.mismatched_case_ids=[...new Set([...record.mismatched_case_ids,...record.historical_rechecks.flatMap(x=>x.mismatched_case_ids),...record.reused_cases.filter(item=>item.observations.some(observation=>!observation.matched)).map(item=>item.case_id)])];
