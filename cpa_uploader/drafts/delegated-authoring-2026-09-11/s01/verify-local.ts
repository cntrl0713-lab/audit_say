...
for(const [index,set] of sets.entries()){
    ...
    try{
        ...
        preflight={delivered_source_ref_ids:delivered,status:'pass',request_chars:p.requestChars,required_units:p.units.length,bank_hash:p.bankHash,source_files:p.sourceFiles,content_hash:p.contentHash};
...
const report={active_selection:activeSelection,ambiguous_active_ids:ambiguous,ledger_file:ledgerFile,ledger_sha256:hash(fs.readFileSync(ledgerFile)),domain_errors:domain.errors,artifact_type:'s01_static_validation',version:1,performed_at:new Date().toISOString(),comparison_file:bankFile,comparison_sha256:hash(fs.readFileSync(bankFile)),comparison_sets:bank.length,own_sets:sets.length,conflicts,outputs,model_calls:0,semantic_review:'not_run',live_grading:'not_run',status:ambiguous.length||domain.errors.length||conflicts.length||outputs.some(o=>o.errors.length)?'fail':'pass'};
