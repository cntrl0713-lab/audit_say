/** S03 static checks only. No API calls or permanent bank writes. */
...
try{inventoryIds=readPendingDrafts(process.cwd(),base).filter(s=>sets.some(x=>x.id===s.id)).map(s=>s.id);if(inventoryIds.length!==5||new Set(inventoryIds).size!==5)errors.push('S03 inventory duplicates/missing');}catch(e){errors.push(String(e));}
...
for(const s of lineage){const row=rows.find(x=>x.plan_id===s.plan_id);if(!row||row.set_id!==s.set_id||row.package!=='S03')errors.push(`Ledger mismatch ${s.plan_id}`);}
