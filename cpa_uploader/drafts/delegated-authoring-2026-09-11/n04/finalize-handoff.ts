/** N04 no-call handoff evidence; writes this package only. */
...
write('inventory-exclusions.json',{initial_failure:'readPendingDrafts(data+drafts) rejected n02/draft-manifest.json as a malformed draft/checkpoint. No model call. Metadata files were read and explicitly excluded; no source file renamed or modified.',excluded});
...
const source='cpa_uploader/data/official/delegated-n04-kga-2025.txt';
...
const rows=lineage.sets.map((s:any)=>`| ${s.plan_id} | ${s.set_id} | 공식2025/2026 대조·조건에 따른2025 | \`${s.sha256}\` | 오류0 | 미실행 | 0 / 미실측 | 최종49 고정 후2차 | [초안](${path.basename(s.actual_file)}) · [QA](${path.basename(s.qa_file)}) |`).join('\n');
const chars=check.per_set.map((s:any)=>s.review_preparation.request_chars.toLocaleString('en-US'));
const text=`# N04 1차 인계\n\n**draft_ready: 신규3세트·9물음·53점·작성자 QA214사례.** 실제 의미검수·모델채점·정본 반영은 미실행이다. 사용자 승인된 전체 제작 범위 중1차 인계이며 제작 완료 선언이 아니다.\n\n| 계획 ID | 실제 ID | 출처·판본 | 초안 버전·SHA-256 | 정적검사 | 의미검수 | 실제 채점 사례 수·불일치 | 남은 일 | 증거 경로 |\n|---|---|---|---|---|---|---|---|---|\n${rows}\n\n총괄49행 ID 장부의 N04/T11-A·T11-B·T13-A 배정에 대응한다. [lineage](lineage.json)는 실제 파일·version1계획·QA·수동근거 파일의 최신 해시와9개 계획물음 대응을 보존한다. 문항 상태는needs_review/needs_human_review다.\n\n배점은31→53점(+22), 세트별18/16/19점이다. 잠정 상한 없이 실제 독립 요구를 분리하라는 총괄 지시에 따라 방법·가정·데이터의 선택/적용, 요청/대응평가, 미비점평가/존재시전달, 서로 다른 평가대상과 독립 속성을 분리했다. 권한부여·승인은 A40~A41과 현재 사례에 별도 승인단계/주체가 없으므로 권한 있는 거래승인 증거라는 한 행위로 유지했다. [변경표](design-changes.json)에9물음 전수명제를 기록했다. T13의 지문·발문에서 회상해야 할3특성 목록을 미리 주지 않도록 수정했고, T11의 판단 문장에 다른criterion의 선택·적용 답이 그대로 남아 가짜 누락을 만드는 표현도 제거했다.\n\n[정적 결과](static-check.json), [CLI 실제 결과](cli-validation.json), [전역 탐색·관계제안 검사](inventory-and-relations-check.json)는 오류0이다. 현재정본+N04 메모리 검증, exact quote, registered src ID·계획·정수합계·QA해시·53criterion별 경계범위·비어있지않은not_met, 명시적data+drafts탐색 및 준비비교본과 동일ID/발문 충돌을 확인했다. 전체49의 의미적 중복검수 결과는 아니다.\n\n400,000자 명시 한도에서 준비용111세트+N04로 무호출 의미검수 입력을 준비했다. 순서대로 ${chars.join(' / ')}자이며 공식 등록 메타데이터가 모두 확인됐다. 기본160,000자 예산을 공용으로 바꾸거나 비교본문을 줄이지 않았다. 최종49 비교본을 받으면 다시 준비한다. API 호출은0회이며 의미검수 receipt나 실제채점결과는 만들지 않았다.\n\n공식 등록파일 \`${source}\` SHA-256은 \`${sha(source)}\`다.86등록단위 중 양판본74문단은 공백제거기준같고12개는 표현·각주·품질관리체계가 다르다. [공식 증거](sources/official-investigation.json)와 [차이](sources/edition-differences.json), [등록단위](sources/registered-catalog.json)를 인계한다. 620 정합수정의 별도일괄 시행공고까지 확보했다는 주장을 하지 않고, 개정체계 선제적용을 하지 않는2026보고기간의 종전품질관리 사례로 정답범위를 한정했다. 시스템 동의표현은 허용하며 외부전문가 항상미적용을 추가점수로 두지 않는다.\n\n540.27/29의 정답을 좌우하는265 전달범위와540.A139~A144 문맥은 추가로 공식PDF에서 직접 읽어 [의존문맥](sources/dependency-context.json)에 보존했고 실제plan의scope.exceptions에 원문을 넣었다. 이 추가문맥은 별도 등록source ID를 꾸며내지 않은 수동계획 근거이며 상세 커뮤니케이션 방식·의견·계산을 숨은 요구로 만들지 않는다.\n\n수동제작이므로 각초안의version1계획을--plan으로, 총괄고정비교본을--bank로 전달한다. evidence-packet 파일은 생성기패킷이 아니므로--packet으로 넣지 않는다. 실제 source_refs·requirement·plan이 모델에 들어가는 경로를 준비했으며 독립 장부파일이 자동입력된다고 가정하지 않는다.\n\n[빈도 증거](frequency-evidence.json)는10요소의 실제 기출/모의/연습/OX·원발문·공통지문·재수록을 분리한다. [coverage 제안](coverage-proposal.json)의10관계는 실제draft/criterion/src ID를 사용하고 모두needs_review다. 최종content 의미대조와snapshot·공통links통합은총괄소유다. A302해설까지확인하여범위의기계적상한으로잘못읽지않았고, 선택형기출을완전열거로확대한부분은각명제의직접빈도로세지않았다.\n\n[S02/S03 인계](s02-s03-handoff.md)에 전문가·내부감사·서비스조직의주체와500/620경계를전달한다. [후속메모](phase2-followups.md)는N04실측요건과총괄이전달한R01/N05 QA보완사항을보존한다. 해당패키지원본은수정하지않았다.\n\n키는node --env-file=.env.local에서존재만확인했고값은기록하지않았다. 실제모델은총괄현행기본gpt-5.6-luna정책을이어받는다. [실행입력](execution-inputs.json)은인계당시코드·정책·원문해시이며2차실제실행시다시고정한다. 이단계의쓰기범위는n04/뿐이다. 정본·공개본·기존배치원본·공용source·coverage·wiki수정은하지않았다.\n`;
fs.writeFileSync(path.join(base,'handoff.md'),text);
const readme=path.join(base,'README.md');fs.writeFileSync(readme,fs.readFileSync(readme,'utf8').replace('298,467 / 264,671 / 319,802',chars.join(' / ')));
let links=0;for(const file of fs.readdirSync(base).filter(f=>f.endsWith('.md'))){const t=fs.readFileSync(path.join(base,file),'utf8');for(const m of t.matchAll(/\]\(([^)]+)\)/gu)){let dest=m[1].split('#')[0];if(!dest||/^[a-z]+:/iu.test(dest))continue;links++;if(!fs.existsSync(path.resolve(base,dest)))errors.push('Missing link '+file+' '+dest);}}
...
if(errors.length)throw Error(errors.join('\n'));
console.log(JSON.stringify({stage:lineage.stage,sets:3,questions:9,points:check.points,qa:check.qa_cases,links,errors,hashes:lineage.sets.map((s:any)=>({id:s.set_id,sha256:s.sha256}))}));
