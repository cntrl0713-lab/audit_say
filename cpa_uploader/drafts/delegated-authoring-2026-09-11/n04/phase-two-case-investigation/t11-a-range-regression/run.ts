...
void(async()=>{
 ...
 for(let index=0;index<qa.cases.length&&!stopped;index++)for(let attempt=1;attempt<=3;attempt++){
  ...
  try{
   ...
   Object.assign(record,{transport:'live_model',raw_judgment:rawJudgment,trace,result,exact_verdict_differences:exact,verdict_differences:exact,matched:result.score===test.expected_points&&result.security_flag==='none'&&exact.length===0,boundary_policy:'이 3사례는 condition_boundary가 아니므로 명시반대/누락을 0점 동등으로 치환하지 않는다.',changed_inputs:changed()});
