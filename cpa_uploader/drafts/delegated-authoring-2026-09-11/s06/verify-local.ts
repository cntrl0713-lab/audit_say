...
const ledgerFile='cpa_uploader/analysis/reviews/delegated-authoring-2026-09-11/id-ledger.json';
const ledger=read(ledgerFile);
const active=new Map<string,QuestionSetV3>(read(bankFile).map((s:QuestionSetV3)=>[s.id,s]));
const activeSelection:Array<{set_id:string;file:string;sha256:string}>=[];
const ambiguous:string[]=[];
for(const e of ledger.entries){
 const candidates:Array<{file:string;set:QuestionSetV3}>=[];
 if(fs.existsSync(e.output_directory))for(const name of fs.readdirSync(e.output_directory)){
  if(!name.endsWith('.json'))continue;
  const file=path.join(e.output_directory,name).replaceAll('\\','/');
  try{const raw=read(file);for(const s of Array.isArray(raw)?raw:[raw])if(s?.id===e.set_id&&Array.isArray(s.subquestions)&&Array.isArray(s.source_refs))candidates.push({file,set:s});}catch{}
 }
 if(candidates.length>1)ambiguous.push(e.set_id);
 if(candidates.length===1){const {file,set}=candidates[0];active.set(set.id,set);activeSelection.push({set_id:set.id,file,sha256:hash(fs.readFileSync(file))});}
}
const sets:QuestionSetV3[]=manifest.outputs.map((o:any)=>read(path.join(dir,o.file)));
const ownIds=new Set(sets.map(s=>s.id));
const bank:QuestionSetV3[]=[...active.values()].filter(s=>!ownIds.has(s.id));
...
for(const [index,set] of sets.entries()){
    ...
    for(const id of plan.source_unit_ids)if(!catalog.units.some(u=>u.id===id))errors.push(`Plan source ID does not exist: ${id}`);
    ...
    let preflight:any;
    try{
        const p=prepareSemanticReview(set,{root,bank:[...bank,...sets],authoringPlan:plan,maxInputChars:400000});
        const requestText=JSON.stringify(p.requestContext);
        const delivered=set.source_refs.filter(ref=>requestText.includes(JSON.stringify(ref.source_quote).slice(1,-1))).map(ref=>ref.id);
        if(delivered.length!==set.source_refs.length)errors.push('Necessary registered source quote missing from actual request context');
        const dep=read(path.join(dir,'sources/dependency-context.json'))[output.plan_id];
        const dependencyDelivered=requestText.includes(JSON.stringify(dep).slice(1,-1));
        if(!dependencyDelivered)errors.push('Manual dependency context missing from actual model input');
        preflight={delivered_source_ref_ids:delivered,manual_dependency_delivered:dependencyDelivered,status:'pass',request_chars:p.requestChars,required_units:p.units.length,bank_hash:p.bankHash,source_files:p.sourceFiles,content_hash:p.contentHash};
...
const domain=validateAuthoringBank([...bank,...sets],root);
const report={active_selection:activeSelection,ambiguous_active_ids:ambiguous,ledger_file:ledgerFile,ledger_sha256:hash(fs.readFileSync(ledgerFile)),domain_errors:domain.errors,artifact_type:'s06_static_validation',version:1,performed_at:new Date().toISOString(),comparison_file:bankFile,comparison_sha256:hash(fs.readFileSync(bankFile)),comparison_sets:bank.length,own_sets:sets.length,conflicts,outputs,model_calls:0,semantic_review:'not_run',live_grading:'not_run',status:ambiguous.length||domain.errors.length||conflicts.length||outputs.some(o=>o.errors.length)?'fail':'pass'};
...
fs.writeFileSync(path.join(dir,reportFile),JSON.stringify({...report,max_input_chars:400000},null,2)+'\n');
