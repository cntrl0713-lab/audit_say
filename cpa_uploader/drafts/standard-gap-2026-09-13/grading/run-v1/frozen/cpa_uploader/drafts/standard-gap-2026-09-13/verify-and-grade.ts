...
async function main() {
 ...
 const run={version:1,artifact_type:'draft_actual_grading_manifest',started_at:new Date().toISOString(),model:gradingModelName(),transport:'model',selected_cases:manifest.cases.map((c:any)=>c.id),policy:manifest.policy,inputs:guards,sdk_retries:0,concurrency:2,production_publication:false,synthetic_data_only:true};
 ...
 async function worker() {
  while(!stopped&&next<manifest.cases.length) {
   const c=manifest.cases[next++];const set=sets.find(s=>s.id===c.set_id)!;const projected=projection(set,c.subquestion_id);
   ...
   try {
    ...
    console.log(JSON.stringify({completed:results.length,total:manifest.cases.length,id:c.id,expected:c.expected_score,actual:sub.score,status:record.status}));
   } catch(error:any) {
    ...
    write(`${prefix}.result.json`,record);results.push(record);
    console.log(JSON.stringify({completed:results.length,total:manifest.cases.length,id:c.id,status:'execution_error',error:record.error}));
   }
  }
 }
