...
const D = 'cpa_uploader/drafts/case-trio-next-2026-09-14', O = `${D}/a`;
const read = p => JSON.parse(fs.readFileSync(p, 'utf8'));
const sha = x => crypto.createHash('sha256').update(x).digest('hex');
const sets = read(`${O}/sets.json`), design = read(`${O}/design.json`), qa = read(`${O}/qa.json`), review = read(`${O}/review.json`), bank = read(`${D}/bank-before.json`), catalog = read(`${D}/source-catalog.json`);
const rows = [];
for (const set of sets) {
  const d = design.find(x => x.set_id === set.id), errors = validateQuestionAuthoringPlan(d.plan);
  for (const id of d.plan.source_unit_ids) if (!catalog.units.some(u => u.id === id)) errors.push(`missing source ${id}`);
  ...
  for (const q of set.subquestions) {
    ...
    if (q.criteria.some(c => c.max_points !== 1 || c.scores.met !== 1 || c.scores.not_met !== 0 || c.scores.contradicted !== 0)) errors.push(`integer points ${q.id}`);
  }
  const factsChars = Array.from(set.shared_context.facts.map(f => f.text).join('\n')).length;
  if (factsChars < 400 || factsChars !== d.facts_chars) errors.push('facts codepoint count');
...
const result = { checked_at: new Date().toISOString(), method: 'Static draft shape, exact source bytes/hashes, plan/catalog linkage, integer QA roles and frozen bank plus one draft in memory. No model calls or canonical changes.', rows, bank_check: bankCheck, hashes };
