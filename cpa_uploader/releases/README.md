...
| 파일 | 단계 | 내용 |
| --- | --- | --- |
| `inspection.json` | inspect(운영 읽기) | active 릴리스 ID·원문 해시·세트 수, importer 경로 함수 정의 해시·ACL, 누적 릴리스 수와 저장 원문 크기 |
...
| `apply-response.json` 또는 `apply-failure.json` | apply(운영 쓰기, 1회) | importer 영수증 또는 실패(결과 불명, 자동 재시도 없음) |
| `verification.json`, `completion.json` | verify(운영 읽기) | 공개본·물음 분류 왕복, 바뀐 세트만 새 버전인지, 저장 원문 해시 |
