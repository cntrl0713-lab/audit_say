...
import {
    loadPromotionLedger, publicationPaths, reviewedContentHash, snapshotFile, snapshotSources,
...
try {
    ...
    if (args['--status']) {
        ...
        console.log(`세트 ${sets.length}개:`, sets.reduce<Record<string, number>>((counts, set) => {
            counts[set.status] = (counts[set.status] || 0) + 1; return counts;
        }, {}));
        const ledger = loadPromotionLedger(paths.ledger);
        console.log(`장부 항목 ${ledger.entries.length}건`);
        for (const entry of ledger.entries) console.log(`- ${entry.date} ${entry.set_id}: ${entry.from_status} → ${entry.to_status} (${entry.evidence})`);
    } else withPublicationLock(paths.authoring, () => {
        const evidence = args['--evidence']?.trim();
        if (!evidence) throw new Error('승급 또는 소급 기록에는 --evidence(실제 검수 근거)가 필요합니다.');
        ...
        if (result.errors.length) throw new Error(`승급 전 전체 정본 검증 실패:\n${result.errors.join('\n')}`);
        const ledger = loadPromotionLedger(paths.ledger);
        ...
        if (args['--backfill-verified']) {
            ...
            if (sets.some((set) => !ledger.entries.some((entry) => entry.set_id === set.id && entry.to_status === 'verified'))) throw new Error('새 소급 검수 기록은 지원하지 않습니다. needs_review 초안을 의미검수한 뒤 정상 승급하십시오.');
        ...
        if (target !== 'verified' && target !== 'published') throw new Error('--to는 verified 또는 published만 지원합니다.');
        const ids = (args['--sets'] || '').split(',').map((id) => id.trim()).filter(Boolean);
        if (!ids.length) throw new Error('승급할 세트 id가 필요합니다. --sets ID,...');
        if (new Set(ids).size !== ids.length) throw new Error('승급 대상 ID가 중복됩니다.');
        const byId = new Map(sets.map((set) => [set.id, set]));
        const missing = ids.filter((id) => !byId.has(id));
        if (missing.length) throw new Error(`존재하지 않는 세트 id: ${missing.join(', ')}`);
        const reverify = Boolean(args['--reverify']);
        if (reverify) {
            for (const id of ids) {
                const set = byId.get(id)!;
                if (!['verified', 'published'].includes(set.status)) throw new Error(`[${id}] --reverify는 기존 verified/published 문항에만 사용합니다. needs_review는 일반 검수 승급을 사용하십시오.`);
            }
        }
        const priorErrors = validatePromotionLedger(sets, ledger, false,
            reverify ? { pendingReverificationIds: new Set(ids) } : {});
        if (priorErrors.length) throw new Error(`승급 전 검수 장부 검증 실패:\n${priorErrors.join('\n')}`);
        const reviewPath = args['--review'] ? path.resolve(args['--review']) : null;
        if (reviewPath) guards.push(snapshotFile(reviewPath));
        const reviewDocument = reviewPath ? readSemanticReviewDocument(reviewPath) : null;
        const efficientPath = args['--efficient-review'] ? path.resolve(args['--efficient-review']) : null;
        ...
        for (const id of ids) {
            const set = byId.get(id)!;
            if (set.status === target && !reverify) continue;
            const expected = target === 'verified' ? 'needs_review' : 'verified';
            if (!reverify && set.status !== expected) throw new Error(`[${id}] ${target} 승급에는 ${expected} 상태가 필요합니다.`);
            if (target === 'published' && set.verification.review_status !== 'verified') throw new Error(`[${id}] published 승급에는 review_status=verified가 필요합니다.`);
        ...
        const errors = validatePromotionLedger(sets, ledger, target === 'published', { efficientContext });
        if (errors.length) throw new Error(`승급 결과 검증 실패:\n${errors.join('\n')}`);
        if (!promoted) { console.log('모든 대상이 이미 요청 상태입니다. 변경 없음.'); return; }
        assertEfficientEvidenceUnchanged(efficientContext);
        for (const [file, hash] of efficientContext.files) guards.push({ file, hash });
        ...
        if (reverify) console.log('재검수된 대상은 verified 상태입니다. 재게시 승급과 컴파일이 필요합니다.');
        if (target === 'published') console.log('다음 단계: compile-question-bank-v3.ts 실행 후 기본 전체 은행 검증.');
    });
} catch (error) {
