...
const output=`${root}/qa-prepared-v4`;
...
const predecessor=`${root}/qa-prepared-v3/manifest.json`,manifest=read(predecessor);
const bankFile=`${root}/prepared-reviewed-v6/candidate-authoring.json`,bank=read(bankFile);
const oldBank=read(manifest.bank_file),correctionFile=`${root}/prepared-reviewed-v6/official-source-corrections.json`,corrections=read(correctionFile);
...
for(const entry of manifest.entries){
    ...
    if(set.id==='pilot-03-001'){
        ...
        if(sub.prompt!==scope.before_prompt&&sub.prompt!==scope.after_prompt)throw Error('Scope clarification predecessor mismatch.');
