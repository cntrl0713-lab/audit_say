...
const review = path.resolve(owned, '..'), prepared = path.join(review, 'prepared-reviewed-v4');
const output = path.join(owned, 'learning-unit-smoke-v2');
...
assert.equal(frozen(bankFile).sha256, 'ab2986dad1d82f4f4e24e7d50d9058a17ec324af84c706c7e7f51b92bf8f745b', 'Approved v4 bank identity');
const predecessorFile = path.join(owned, 'learning-unit-smoke-v1', 'manifest.json');
const predecessor = read(predecessorFile);
assert.equal(frozen(predecessorFile).sha256, '8699840893cab7482b9ac5dbfb53ef3363c2eb74149701c297b0fee0b0808679', 'Preserve original v3 smoke evidence');
for (const row of predecessor.entries) assert.equal(frozen(row.file).sha256, row.sha256, `Preserve prior artifact: ${row.file}`);
...
const deltas = rows.map(row => {
    ...
    return { learning_unit_id: row.learning_unit_id, source_set_id: row.source_set_id,
        before_points: prior.points, after_points: row.points,
...
assert(substantiveDeltas.every(row => row.source_set_id === 'pilot-08-007'), 'Only the independently reviewed pilot-08-007 scope correction is expected');
...
const manifest = { version: 2, created_at: new Date().toISOString(), status: 'app_projection_and_empty_path_checked_nonempty_model_smoke_not_run',
...
fs.writeFileSync(path.join(output, 'v3-v4-comparison.json'), serialize({ version: 1, api_calls: 0, source_bank_sha256: frozen(bankFile).sha256, deltas }), { flag: 'wx' });
