...
function semanticProvenanceInput(provenance, currentManifest, job) {
    ...
    // Receipt source_files covers direct question refs. The job also freezes plan-only
    // source units; these must remain guarded without becoming invented receipt refs.
    const expectedSetSources = [...new Map(currentSet.source_refs.map(ref => [resolve(ref.file).toLowerCase(), ref.file])).values()]
        .map(file => ({ file, sha256: fileHash(resolve(file)) }));
    for (const row of expectedSetSources) required(job.source_files.some(frozen => samePath(resolve(frozen.file), resolve(row.file)) && frozen.sha256 === row.sha256),
        `${job.set_id}: question source identity is not frozen in the job`);
    required(Array.isArray(receipt.source_files) && jsonHash(fileIdentities(receipt.source_files)) === jsonHash(fileIdentities(expectedSetSources)), `${job.set_id}: receipt question source files differ`);
...
async function provenanceFixtureTests() {
    const fixture = fs.mkdtempSync(path.join(path.dirname(SELF), '.worker-v3-fixture-'));
    const events = [];
    ...
    try {
        ...
        const planOnlyFile = path.join(fixture, 'plan-only-source.txt');
        fs.writeFileSync(planOnlyFile, 'Local synthetic plan-only source. No model execution.');
        const setSources = [...new Set(set.source_refs.map(ref => ref.file))].map(file => ({ file, sha256: fileHash(resolve(file)) }));
        ...
        const oldJob = { set_id: set.id, worker: 'a', file: sourceFile, sha256: fileHash(sourceFile), plan_file: planFile, plan_sha256: fileHash(planFile),
            source_files: [...setSources, { file: planOnlyFile, sha256: fileHash(planOnlyFile) }] };
        ...
        const receipt = { schema_version: '1.0', set_id: set.id, content_hash: reviewedHash(set), bank_hash: jsonHash([]),
            source_files: setSources, context, context_hash: jsonHash(context),
        ...
        events.push('valid_cross_run_with_guarded_plan_only_source_and_direct_question_receipt');
        ...
        const changeReceipt = changed => {
            delete changed.receipt_hash; changed.receipt_hash = jsonHash(changed);
        ...
        const restoreReceipt = () => { write(receiptFile, { schema_version: '1.0', reviews: [receipt] }); write(summaryFile, summary); write(currentFile, current); };
        negative('missing_direct_question_source_rejected_with_refreshed_receipt_hashes', () => changeReceipt({ ...receipt, source_files: setSources.slice(1) }), restoreReceipt);
        negative('plan_only_source_added_to_receipt_rejected_with_refreshed_receipt_hashes', () => changeReceipt({ ...receipt, source_files: oldJob.source_files }), restoreReceipt);
        negative('wrong_direct_source_hash_rejected_with_refreshed_receipt_hashes', () => changeReceipt({ ...receipt, source_files: setSources.map((row, i) => i ? row : { ...row, sha256: '0'.repeat(64) }) }), restoreReceipt);
        negative('plan_only_file_byte_change_still_rejected', () => fs.appendFileSync(planOnlyFile, 'changed'), () => fs.writeFileSync(planOnlyFile, 'Local synthetic plan-only source. No model execution.'));
        negative('plan_only_file_removed_from_current_job_still_rejected', () => write(currentFile, { ...current, jobs: [{ ...currentJob, source_files: setSources }] }), () => write(currentFile, current));
        ...
        assert.equal(path.dirname(fixture), path.dirname(SELF)); assert(path.basename(fixture).startsWith('.worker-v3-fixture-'));
...
async function main() {
    ...
    const run = { version: 3, started_at: now(), worker, phase, manifest_file: relative(manifestFile), manifest_sha256: manifestHash, worker_file: relative(SELF), worker_sha256: selfHash, model: manifest.model, review_model: manifest.review_model, max_input_chars: manifest.max_input_chars, set_ids: jobs.map(job => job.set_id), output: relative(output), transport: 'production_cli_subprocess', mock: false, credentials_logged: false, stop_file: stopFile ? relative(stopFile) : null, receipt_provenance: args['--semantic-directory'] ? relative(resolve(args['--semantic-directory'])) : null };
