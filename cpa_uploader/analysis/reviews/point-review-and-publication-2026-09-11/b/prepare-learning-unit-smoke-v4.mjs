...
const output = path.join(owned, 'learning-unit-smoke-v4');
...
const predecessorFile = path.join(review, 'a', 'learning-unit-smoke-v3', 'manifest.json');
const predecessor = read(predecessorFile);
assert.equal(frozen(predecessorFile).sha256, '33b03f712e8fea314318fd1b75851dafcdef02fabf0b014aa059c0089b77b273', 'Preserve v6 smoke-v3 evidence');
...
frozen(path.join(review, 'a', 'run-learning-unit-smoke-v3.ts'));
frozen(path.join(review, 'a', 'run-learning-unit-smoke-v3.test.ts'));
const inheritedGenerator = frozen(path.join(review, 'a', 'prepare-learning-unit-smoke-v3.mjs'));
...
assert(studentDeltas.every(row => row.learning_unit_id === 'pilot-03-001--sub1--standard'), 'Any student-input followup must be limited to the root-approved acceptance-scope clarification');
assert(deltas.filter(row => row.prompt_changed).every(row => row.learning_unit_id === 'pilot-03-001--sub1--standard'), 'Any grader-prompt followup must be limited to the root-approved acceptance-scope clarification');
...
const manifest = { version: 4, created_at: new Date().toISOString(), status: 'app_projection_and_empty_path_checked_nonempty_model_smoke_not_run',
    ...
    predecessor_comparison: { compared_learning_units: deltas.length, substantive_changed_units: substantiveDeltas, all_classification_changed_units: deltas.filter(row => row.classifications_changed).length,
        ...
        interpretation: 'The v6 smoke-v3 bundle is preserved byte for byte. The actual v7 projection, source and classification identities, student input, grader prompts and schemas are compared below. Notes-only hash changes are distinct from request changes. Any accepted scope clarification is recorded as an actual field difference. No old model observation is promoted to a new result.' },
...
fs.writeFileSync(path.join(output, 'v6-v7-comparison.json'), serialize({ version: 1, api_calls: 0, source_bank_sha256: frozen(bankFile).sha256, deltas }), { flag: 'wx' });
fs.writeFileSync(path.join(output, 'README.md'), `# v7 실제 학습 단위의 smoke 준비

변경 기존 ${changedIds.length}세트와 신규 ${newIds.length}세트에서 실제 앱 함수로 ${rows.length}학습 단위(${counts.standard_units} 기준서형, ${counts.case_units} 사례형)를 투영했다. 사례형은 부모의 모든 사례형 물음을 함께 담으며 기준서형은 한 물음·부모 사실 없음·독립 발문을 확인했다.

빈 답안 ${emptyCalls}개는 명시적인 빈 키로 생산 채점기의 모델 미호출 경로를 실행해 0점과 정상 보안 처리를 확인했다. 비빈 저장 모범답안 ${rows.length}개는 요청과 완전 criterion 기대값만 준비했으며 모델 실행은 0회다.

이전 a/learning-unit-smoke-v3의 전체 파일은 원본 해시로 보존했다. v6-v7-comparison.json에 249단위별 실제 차이를 기록한다. 학생 입력 변경 ${studentDeltas.length}개, 채점 prompt 변경 ${deltas.filter(row => row.prompt_changed).length}개, schema 변경 ${deltas.filter(row => row.schema_changed).length}개이며 답안·criterion·기대점수는 불변을 검사했다. 원 실측을 새로운 판정으로 승계하지 않았다.

전체 원세트의 formal QA와 비교하면 ${counts.formal_request_changed_units}단위에서 prompt, ${counts.formal_schema_changed_units}단위에서 schema가 다르다. 서명 토큰·DB·XP·UI 상호작용·실제 모델의 모범답안 만점은 이 준비로 검증한 것이 아니다. 원문·공통 코드·현재 실행 manifest는 수정하지 않았다.
`, { flag: 'wx' });
assertFrozen();
