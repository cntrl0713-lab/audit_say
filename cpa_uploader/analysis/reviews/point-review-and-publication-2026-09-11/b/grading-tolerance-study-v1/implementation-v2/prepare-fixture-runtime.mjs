...
// Exact private production grounding functions, extracted without rewriting
// their bodies; used only as a local behavioral oracle (no model request code).
const gradingFile = path.join(root, 'lib/questionV3Grading.ts');
const gradingSource = ts.createSourceFile(gradingFile, fs.readFileSync(gradingFile, 'utf8'), ts.ScriptTarget.Latest, true);
const oracleNames = ['record', 'groundJudgment', 'validateJudgmentCoverage'];
const oracleFunctions = oracleNames.map(name => gradingSource.statements.find(statement => ts.isFunctionDeclaration(statement) && statement.name?.text === name).getText(gradingSource));
const oracle = `import { resolveAnswerEvidence } from ${JSON.stringify(pathToFileURL(path.join(root, 'lib/questionV3Evidence.ts')).href)};\n${oracleFunctions.join('\n')}\nexport { groundJudgment };\n`;
fs.writeFileSync(path.join(target, 'grounding-oracle.mjs'), ts.transpileModule(oracle, { compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.ESNext } }).outputText);
...
const report = { relocation_only: ['relative imports resolve to real original dependencies or sibling proposed modules', 'graderHash repository root points at real unchanged grading files', 'private validation/read functions exported only in fixture runtime', 'actual production groundJudgment and its private helpers extracted unchanged as local oracle'],
