...
const fixtureChecks = JSON.parse(fs.readFileSync(path.join(own, 'fixture-results-v2.json'), 'utf8')).tests.length;
const predecessor = JSON.parse(fs.readFileSync(path.join(own, 'predecessor.json'), 'utf8'));
for (const item of predecessor.preserved) if (sha(path.join(root, item.file)) !== item.sha256) throw Error(`PREDECESSOR_CHANGED:${item.file}`);
const output = { version: 2, created_at: new Date().toISOString(), production_edits: 0, api_calls: 0,
  ...
  predecessor: { file: 'predecessor.json', sha256: sha(path.join(own, 'predecessor.json')), preserved_files: predecessor.preserved.length },
  changes_from_v1: ['Mirror all groundJudgment negative evidence/partial conditions', 'Enforce exactly one first-attempt judgment trace for the supported no-recovery producer path', 'Capture acceptance path snapshot before reading and verify/parse the same Buffer', 'Hash and parse artifact JSON/JSONL from the same captured Buffer'],
  fixture: { file: 'fixture-results-v2.json', sha256: sha(path.join(own, 'fixture-results-v2.json')), checks: fixtureChecks },
...
console.log(JSON.stringify({ snapshots: entries.length, patch_sha256: output.patch.sha256, checks: fixtureChecks, lint_errors: lint.filter(x => x.exit_code !== 0).length }));
