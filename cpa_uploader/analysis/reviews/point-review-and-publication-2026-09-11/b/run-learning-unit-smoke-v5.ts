...
export const FROZEN_MANIFEST_SHA256 = 'a75cc9f9a1c54ecee0b20498478ff9340742b96a447cf5094e3c6b9093817eb7';
...
export function prepare(options: Options) {
    ...
    assert.equal(manifest.version, 5); assert.equal(manifest.entries.length, 249); assert.equal(manifest.counts.source_sets, 119);
    freeze(SELF);
    ...
    for (const { row, index } of manifest.entries.map((row, index) => ({ row, index }))) {
        ...
        const catalogFile = manifest.inputs.find(row => row.file.endsWith('/prepared-reviewed-v7/learning-question-classifications.json'));
        assert(catalogFile);
