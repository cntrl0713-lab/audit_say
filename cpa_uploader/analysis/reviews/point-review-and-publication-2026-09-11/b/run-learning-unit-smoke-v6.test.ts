...
import { parseArgs, partition, safeError, failureKind, rawSecurityFindings, fullCreditFindings, runSequential, verifyFrozenFile, GuardError, RecordingError } from './run-learning-unit-smoke-v6.ts';
...
const runner = path.join(owned, 'run-learning-unit-smoke-v6.ts');
const manifest = path.join(owned, 'learning-unit-smoke-v6/manifest.json');
const fixtureRoot = fs.mkdtempSync(path.join(owned, 'learning-unit-smoke-v6-fixtures-'));
