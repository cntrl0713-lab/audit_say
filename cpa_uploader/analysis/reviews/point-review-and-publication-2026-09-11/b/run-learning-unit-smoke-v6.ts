...
export const FROZEN_MANIFEST_SHA256 = '6a49e407e3a85e2a2f44ba3c434f2ca5fc75ffd54b4b4b8b918bb952a3c28b2c';
...
export function prepare(options: Options) {
    ...
    assert.equal(manifest.version, 6); assert.equal(manifest.entries.length, 249); assert.equal(manifest.counts.source_sets, 119);
    freeze(SELF);
    ...
    for (const { row, index } of manifest.entries.map((row, index) => ({ row, index }))) {
        ...
        const catalogFile = manifest.inputs.find(row => row.file.endsWith('/c/prepared-reviewed-v8/learning-question-classifications.json'));
        assert(catalogFile);
