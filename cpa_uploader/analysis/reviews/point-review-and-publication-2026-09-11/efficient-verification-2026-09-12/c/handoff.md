...
- `handoff-static-checks.json`:64계획의version1 ready 형상·실제 카탈로그 ID, 선택·보충 QA3,387개의 형상·정수합산, 원QA ID/답안/대상 물음 보존, 기준은행 불변 검사를 완료했다. 이3,387개 정적 검사를 전수 의미 판정 또는 실제 채점으로 세지 않는다.
...
효율실행의 세 역할은 모범 만점·양의 부분점수·오답0점이다. 이전 인계의 `wrong_or_boundary_answer`36개는 명제 일부를 맞힌1~9점 경계 답안이어서 이 역할 계약에 맞지 않았다. 해당 답안이 실제 부분정답이라는 판단이나 기대값을 바꾸지 않고 `handoff-v2-preserved/`에 전부 보존했다.

36개 물음의 기존QA에는 비어 있지 않은0점 사례가 없어, 각각 해당 주제의 명시적인 반대를 포함한 새 답안을 직접 작성·판정했다.27세트의QA 사본에36개를 추가했으며 원QA 객체는 모두 완전히 보존했다. 문항·배점·계획 파일은 변하지 않았다. 모범149개·부분144개·이미0점인 오답113개의 대표 객체406개도 완전히 같다. 새 오답36개만 교체하여 전체442개, 오답149개 전부0점을 확인했다. 원 부분점수를0으로 변환하거나 빈답안·단어 나열을 사용하지 않았다.

`wrong-zero-selection-v3/selection.json`은 원 대표와 새 답안·판정의 대응이고, `integrity-check.json`은 보존·선정·합계 검사다. 실제 모델 호출이나 추가 호출 수는 발생하지 않았다.
