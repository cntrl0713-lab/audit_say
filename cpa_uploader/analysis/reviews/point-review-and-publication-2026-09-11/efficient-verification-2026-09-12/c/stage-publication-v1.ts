...
const PREPARATION = `${C}/publication-workflow-v1/preparation-lock.json`;
...
interface Options { batch: string; batchSha: string; readiness: string; sealInputs: string; output: string; execute: boolean }
...
export function parseOptions(args: string[]): Options {
    ...
    const names: Record<string, string> = { '--batch': 'batch', '--batch-sha256': 'batchSha', '--readiness': 'readiness', '--seal-inputs': 'sealInputs', '--output': 'output' };
...
export function main(args = process.argv.slice(2)) {
    ...
    const lock = read<{ inputs: FileRef[]; allowed_unselected_note_changes: NoteCorrection[] }>(PREPARATION);
    ...
    if (!opts.execute) {
        console.log(JSON.stringify({ status: 'validated_dry_run_no_stage_writes', cohorts, steps, offline_receipt_preflight_ms: offlineReceiptPreflightMs, canonical_writes: 0, db_calls: 0, api_calls: 0 }));
        return;
    }
    ...
    write('execution-plan.json', { status: 'local_staging_started_not_installed', created_at: workflowStartedAt, offline_receipt_preflight_ms: offlineReceiptPreflightMs, cohorts, steps,
    ...
    try {
        ...
        write('completion.json', { status: 'isolated_published_stage_validated_not_installed_not_db_applied', completed_at: new Date().toISOString(), elapsed_ms: Date.now() - workflowStartedMs,
            cohorts, set_count: final.length, question_count: final.reduce((n, s) => n + s.subquestions.length, 0),
