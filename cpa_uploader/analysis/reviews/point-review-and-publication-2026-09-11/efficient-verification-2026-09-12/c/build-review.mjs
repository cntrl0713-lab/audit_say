...
const follow=['content-followups-v1','scope-followups-v1','kga800-page-metadata-v2'].flatMap(d=>read(`${E}/${d}/index.json`).entries);
...
const zeroQa=read(`${E}/wrong-zero-selection-v3/index.json`).entries,zeroSelection=read(`${E}/wrong-zero-selection-v3/selection.json`).entries;
for(const j of jobs){const z=zeroQa.find(z=>z.set_id===j.set_id);if(z){j.qa_file=z.qa_file;j.qa_sha256=z.qa_sha256;}}
const preservedRepresentatives=read(`${E}/handoff-v2-preserved/representative-cases.json`).entries;
...
for(const job of jobs){
 ...
 for(let qi=0;qi<s.subquestions.length;qi++){
  ...
  const zero=zeroSelection.find(z=>z.set_id===s.id&&z.subquestion_id===q.id);if(zero)picks.wrong=subset.find(x=>x.id===zero.new_case.id);
  ...
  for(const [kind,cc] of Object.entries(picks))if(cc){
   ...
   if(kind==='model')assert.equal(cc.expected_points,max);if(kind==='partial')assert(cc.expected_points>0&&cc.expected_points<max);if(kind==='wrong')assert.equal(cc.expected_points,0);
   const previous=preservedRepresentatives.find(r=>r.set_id===s.id&&r.subquestion_id===q.id&&r.role===roles[kind]);
   const qaFile=previous&&(kind!=='wrong'||!zero)?previous.qa_file:supplement.includes(cc)?`${E}/representative-supplements/${s.id}.json`:job.qa_file;
...
const aSelectionFile=`${D}/efficient-verification-2026-09-12/a/selected-files.json`,aSelection=read(aSelectionFile).entries;
const fullBankMap=new Map(bank.map(s=>[s.id,s]));
for(const j of aSelection){assert.equal(fh(j.file),j.sha256);const raw=read(j.file),s=(Array.isArray(raw)?raw:[raw]).find(s=>s.id===j.set_id);assert(s);fullBankMap.set(s.id,s);}
for(const s of sets)fullBankMap.set(s.id,s);
const fullBank=[...fullBankMap.values()],validation=validateAuthoringBank(fullBank);
write(`${E}/full-bank-validation-v2.json`,{version:2,checked_at:new Date().toISOString(),source_bank:{file:master.bank_file,sha256:fh(master.bank_file)},a_selection:{file:aSelectionFile,sha256:fh(aSelectionFile)},c_set_ids:sets.map(s=>s.id),memory_bank_sha256:hash(JSON.stringify(fullBank)),sets:fullBank.length,subquestions:validation.subquestionCount,criteria:validation.criterionCount,points:validation.totalPoints,errors:validation.errors,api_calls:0,note:'완전한 메모리 은행에서 validateAuthoringBank의 반환 errors를 검사한다. 초기 C 집계는 반환 errors를 강제하지 않은 검사 결함이 있어 보존한 뒤 이 검사로 보완했다.'});
assert.deepEqual(validation.errors,[],'full validateAuthoringBank returned errors');
