...
export interface SubquestionV3 {
    id: string;
    type: QuestionTypeV3;
    // Optional only for immutable legacy v3 snapshots, classified through a versioned sidecar.
...
export function compilePublicQuestionSet(questionSet: QuestionSetV3): PublicQuestionSetV3 {
    return {
        ...
        subquestions: questionSet.subquestions.map((subquestion) => ({
            ...
            ...(subquestion.question_style === undefined ? {} : { question_style: subquestion.question_style }),
            ...(subquestion.topic_ids === undefined ? {} : { topic_ids: [...subquestion.topic_ids] }),
...
export function validateQuestionSetV3(
    ...
    const hasLearningMetadata = questionSet.subquestions.some(sub => sub.question_style !== undefined || sub.topic_ids !== undefined);
    if (hasLearningMetadata && new Set(questionSet.subquestions.map(sub => sub.question_style)).size !== 1) {
        errors.push('사례형과 기준서형은 별도 학습 단위로 작성해야 합니다.');
    }
    for (const subquestion of questionSet.subquestions) {
        ...
        if (hasLearningMetadata) {
            if (!['case', 'standard'].includes(subquestion.question_style ?? '')) errors.push(`[${subquestion.id}] question_style이 필요합니다.`);
            if (!Array.isArray(subquestion.topic_ids) || subquestion.topic_ids.length === 0
                || new Set(subquestion.topic_ids).size !== subquestion.topic_ids.length
                || subquestion.topic_ids.some(id => typeof id !== 'string' || !/^(0[1-9]|1[0-9])$/.test(id))) {
                errors.push(`[${subquestion.id}] 등록된 주제의 topic_ids가 필요합니다.`);
            }
            if (subquestion.question_style === 'case' && (!Array.isArray(sharedFacts) || sharedFacts.length === 0)) {
                errors.push(`[${subquestion.id}] 사례형은 부모 문제의 사실관계가 필요합니다.`);
            }
            if (subquestion.question_style === 'standard' && Array.isArray(sharedFacts) && sharedFacts.length !== 0) {
                errors.push(`[${subquestion.id}] 기준서형은 사실관계 없이 단독 풀이 가능한 발문이어야 합니다.`);
            }
        }
