...
const review = path.resolve(owned, '..'), prepared = path.join(review, 'prepared-reviewed-v6');
const output = path.join(owned, 'learning-unit-smoke-v3');
...
assert.equal(frozen(bankFile).sha256, 'e43cd491c08faebd70c02eb67bfe15198e669b66a6372b723ed539da229fd94a', 'Approved v6 bank identity');
const predecessorFile = path.join(review, 'b', 'learning-unit-smoke-v2', 'manifest.json');
const predecessor = read(predecessorFile);
assert.equal(frozen(predecessorFile).sha256, '9f9ba50c3ce15b028985d6f8f68e234877e060b256d1e431f063af336e4df117', 'Preserve v4 smoke-v2 evidence');
...
const inheritedGenerator = frozen(path.join(review, 'b', 'prepare-learning-unit-smoke-v2.mjs'));
...
assert.deepEqual(studentDeltas.map(row => row.learning_unit_id), ['pilot-03-001--sub1--standard'], 'Only the approved acceptance-scope prompt changes student input');
assert.deepEqual(deltas.filter(row => row.prompt_changed).map(row => row.learning_unit_id), ['pilot-03-001--sub1--standard'], 'Only this grading prompt changes');
...
const manifest = { version: 3, created_at: new Date().toISOString(), status: 'app_projection_and_empty_path_checked_nonempty_model_smoke_not_run',
    predecessor: { file: relative(predecessorFile), sha256: frozen(predecessorFile).sha256, artifact_bytes_preserved: true },
    predecessor_comparison: { compared_learning_units: deltas.length, substantive_changed_units: substantiveDeltas, all_classification_changed_units: deltas.filter(row => row.classifications_changed).length,
        ...
        interpretation: 'Source and classification identities change after official-source replacement, while actual student-visible content and grader prompt change only for pilot-03-001/sub1. No old model observation is promoted to a new result.' },
