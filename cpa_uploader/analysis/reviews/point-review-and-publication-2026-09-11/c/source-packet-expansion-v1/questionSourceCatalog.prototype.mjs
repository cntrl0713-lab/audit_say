...
function standardUnits(data, source, registry, metadata19) {
  ...
  for (let index = 0; index < data.lines.length; index++) {
    ...
    if (pageHeading) { page = Number(pageHeading[1]); continue; }
    if (heading) {
      flush(index);
...
export function buildSourceCatalog({ repoDir = DEFAULT_ROOT } = {}) {
  ...
  const registry = {...JSON.parse(registryText), referenceFootnotes: JSON.parse(fs.readFileSync('cpa_uploader/analysis/reviews/point-review-and-publication-2026-09-11/c/source-packet-expansion-v1/reference-footnotes-proposal.json', 'utf8'))};
