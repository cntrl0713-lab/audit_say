...
const output=`${root}/execution-${mode==='--all'?'all':mode==='--new-only'?'new':'existing'}-v5`;
const bankFile=`${root}/prepared-reviewed-v6/candidate-authoring.json`;
...
const qaManifestFile=`${root}/qa-prepared-v4/manifest.json`;
...
const summary=read(`${root}/prepared-reviewed-v6/summary.json`);
const overrideFile=`${root}/prepared-reviewed-v6/plan-overrides.json`;
...
const codeFiles=read(`${root}/execution-runtime-v4.json`).code_files;
...
for(const set of bank.filter(set=>selected.has(set.id))){
    try{
        ...
        const corrections=read(root+'/prepared-reviewed-v6/official-source-corrections.json');
