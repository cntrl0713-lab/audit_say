...
const output=`${root}/execution-${mode==='--all'?'all':mode==='--new-only'?'new':'existing'}-v2`;
const bankFile=`${root}/prepared-reviewed-v4/candidate-authoring.json`;
...
const before:QuestionSetV3[]=read(`${root}/prepared-reviewed-v3/candidate-authoring.json`);
const qaManifestFile=`${root}/qa-prepared-v2/manifest.json`;
...
const summary=read(`${root}/prepared-reviewed-v4/summary.json`);
const overrideFile=`${root}/prepared-reviewed-v4/plan-overrides.json`;
...
const codeFiles=read(`${root}/execution-runtime-v2.json`).code_files;
...
for(const set of bank.filter(set=>selected.has(set.id))){
    try{
        // Quote/locator corrections must not silently alter any grading contract.
        const original=before.find(row=>row.id===set.id)!;
        const contract=(value:QuestionSetV3)=>({context:value.shared_context,subquestions:value.subquestions.map(sub=>({
            ...sub,requirements:sub.requirements.map(req=>({...req,source_span:undefined}))}))});
        if(set.id==='pilot-08-007'){
            const correctedFile=`${root}/a/pilot-08-007-remediation-v1/question.json`;
            if(JSON.stringify(set)!==JSON.stringify(read(correctedFile)))throw Error('Reviewed duplicate-score correction differs from its source.');
        }else if(JSON.stringify(contract(original))!==JSON.stringify(contract(set)))throw Error('Unreviewed question/answer/criterion change.');
