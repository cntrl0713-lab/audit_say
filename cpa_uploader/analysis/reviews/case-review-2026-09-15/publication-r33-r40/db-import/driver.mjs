// 사례형 지정 검토 r33~r40(D등급 단독 갱신)의 운영 DB 반영: 원 8세트 퇴역과 새 10세트 추가를 한 릴리스로 적용한다. publication-r28-r32/db-import/driver.mjs를 옮겼다.
// 새 10세트에는 분리 보존한 기준서형 두 세트가 포함된다(퇴역 없이 추가만 한다). pilot-18-005-standards-20260921은 물음이 둘이라 단위도 둘이고 pilot-19-005-standards-20260921은 하나다.
// 학습 단위 확인은 r20~r27에서 고친 유형별 판정을 그대로 가져왔다. 이번에도 물음마다 단위가 따로 생기므로
// 물음마다 순회하는 이 판이 반드시 필요하다. 세트당 한 단위를 단정하는 옛 판으로 되돌리면 apply가 커밋된 뒤 멈춘다.
//   --prepare : 사례형 퇴역 함수 설치 결과(r01~r04 반영의 publication/db-migration/after.json, 2026-09-16 적용)와 직전 운영 반영(사례형 r28~r32, release 04052784)을 읽기 전용으로 참조해
...
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r33-r40/db-import/driver.mjs <mode> [--expected-preparation-sha256 SHA]
// 기록(준비·probe·반영 결과)은 이 폴더에, 전송 pack·SQL 사본은 커밋하지 않는 tmp/case-review-publication-r33-r40/db-import/에 둔다.
...
if (mode === '--prepare') {
    ...
    // 학습 단위 확인: 사례형은 세트당 한 단위(<세트ID>--case), 기준서형은 물음마다 한 단위(<세트ID>--<물음ID>--standard)다(lib/learningUnits.ts learningUnitId).
    const unitChecks = [];
    ...
    if (!fs.existsSync(O + '/roundtrip.json')) write(O + '/roundtrip.json', { status: 'passed', checked_at: new Date().toISOString(), source_bytes_identical: true, public_round_trip: true, learning_classification_round_trip: true, learning_topic_round_trip: true,
        transaction_guarded_retained_versions_and_classifications_preserved: true, retired_set_ids: built.retiredIds, new_set_ids: built.newIds, retirement_authorization_stored: true, classification_count: rows.length, learning_unit_count: units.length, new_unit_checks: unitChecks,
