...
if encode(sha256(convert_to(pg_get_functiondef('public.cpa_assert_reviewed_question_retirements(jsonb)'::regprocedure),'UTF8')),'hex') <> '64a2f3b0e5c0a8414b389a154d816d6bbd8bebd79703a52bd61856478008f510' then raise exception 'Protected function changed before memo DDL: cpa_assert_reviewed_question_retirements(jsonb)'; end if;
if encode(sha256(convert_to(pg_get_functiondef('public.cpa_get_question_version(uuid)'::regprocedure),'UTF8')),'hex') <> 'b0ac20592fa6603420a4c9fc129b7af4a15ae2f94a44bf0ab3be2925fa80b2f4' then raise exception 'Protected function changed before memo DDL: cpa_get_question_version(uuid)'; end if;
if encode(sha256(convert_to(pg_get_functiondef('public.cpa_get_question_version_with_source_metadata(uuid)'::regprocedure),'UTF8')),'hex') <> '74697d51accd5f6e9c8cd03e9b0d2ffbd2e3df5467c24e6d1565104ba47e9401' then raise exception 'Protected function changed before memo DDL: cpa_get_question_version_with_source_metadata(uuid)'; end if;
if encode(sha256(convert_to(pg_get_functiondef('public.cpa_import_learning_question_bank(jsonb)'::regprocedure),'UTF8')),'hex') <> '00ea299c46a0d4c4fd7e8e28f60e3de1dd2c9c5e3341bd2f5187286d73739983' then raise exception 'Protected function changed before memo DDL: cpa_import_learning_question_bank(jsonb)'; end if;
if encode(sha256(convert_to(pg_get_functiondef('public.cpa_import_learning_question_bank_with_retirements(jsonb)'::regprocedure),'UTF8')),'hex') <> 'c79b43b6be43070c4b392c22aa4c8a6164350f898b3fef7e4c22a22075741445' then raise exception 'Protected function changed before memo DDL: cpa_import_learning_question_bank_with_retirements(jsonb)'; end if;
if encode(sha256(convert_to(pg_get_functiondef('public.cpa_import_question_bank(jsonb)'::regprocedure),'UTF8')),'hex') <> 'a95ae21fe483c2eb3eac9ec8198803028a8e206c468ce50850472bb8e54a6b42' then raise exception 'Protected function changed before memo DDL: cpa_import_question_bank(jsonb)'; end if;
if encode(sha256(convert_to(pg_get_functiondef('public.cpa_import_question_bank_with_retirements(jsonb)'::regprocedure),'UTF8')),'hex') <> 'a80ecdea245f0b78ac9fc08d4361391f3abad54b8ca416277866dd204784dda5' then raise exception 'Protected function changed before memo DDL: cpa_import_question_bank_with_retirements(jsonb)'; end if;
if not exists(select 1 from public.cpa_question_bank_releases where status='active' and id='92948c50-7ec5-4a88-a4c4-a0208bc13f63'::uuid and source_file_hash='89da3e6ab717551d500cb27295fb3cd0a5cbee9f7164fa0627874dda6d145b6d') then raise exception 'Active release changed before memo DDL'; end if;
if to_regclass('public.cpa_question_set_version_source') is not null or to_regclass('public.cpa_question_bank_release_item_source') is not null then raise exception 'Memo tables already exist'; end if;
if to_regprocedure('public.cpa_backfill_release_item_source(uuid)') is not null then raise exception 'Backfill function already exists'; end if;
if exists(select 1 from supabase_migrations.schema_migrations where version='20260921120000') then raise exception 'Migration 20260921120000 already recorded'; end if;
end $memo_preflight$;
...
insert into supabase_migrations.schema_migrations(version,name,statements) values('20260921120000','cpa_question_version_source_memo',array['-- cpa_get_question_version의 비용을 릴리스 누적과 무관하게 만든다.
--
-- 배경: 이 함수는 판본이 묶인 릴리스를 모두 순회하며 릴리스마다 저장 원문(현재 6.2MB)을
-- 통째로 sha256 해시하고 jsonb로 파싱한 뒤 배열을 훑어 세트 하나를 찾았다. 비용이 O(R × D)라
-- 릴리스가 쌓일수록 커졌고, 2026-09-21 릴리스 22개 시점에 가장 오래 유지된 판본 셋에서
