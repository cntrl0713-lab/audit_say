// 사례형 지정 검토 r01~r04의 정본·공개본 반영.
//   --baseline : 현재 정본 5종의 해시와 은행·장부 사본을 기록한다(쓰기 대상은 이 폴더뿐).
//   --stage    : 격리 stage에서 원 8세트를 빼고 새 4세트를 붙인 뒤 검수 승급(verified)·게시(published), 공개본·암호화본·분류 카탈로그, 전체 검증, DB 준비 검사를 수행한다.
//   --install  : stage 결과를 정본에 원자적으로 기록하고 정본 검사를 수행한다.
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication/publish.mjs --baseline|--stage|--install
...
const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication', S = P + '/stage', L = P + '/stage-logs';
...
assert.equal(newIds.length, 4); assert.equal(retiredIds.length, 8); assert.equal(new Set([...newIds, ...retiredIds]).size, 12);
