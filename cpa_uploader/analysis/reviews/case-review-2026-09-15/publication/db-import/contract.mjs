...
// fix-v4 db-incremental-v1/replace-contract.mjs의 제자리 교체 계약을 퇴역·추가 계약으로 옮겼다. 최종 원문 바이트와 정식 importer 전체 payload가
...
// stage·baseline 사본은 저장소 밖으로 옮겼다(copy-archive-2026-09-15.json). 최종 원문은 설치 기록으로 stage와 같음을 확인한 정본에서, 기준 원문은 반영 전 커밋의 정본에서 해시를 대조해 읽는다.
...
export const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication', N = P + '/db-import';
...
// baseline.json을 기록할 때의 정본은 이 커밋(origin/main)과 같았다. 기록한 SHA-256으로 대조한다.
export const BASELINE_COMMIT = '76e8da7546c994b9a982f29e41a3e93f9c6908de';

// 검증된 stage 파일과 설치된 정본 파일이 같은 바이트이고 설치 뒤 바뀌지 않았는지 확인한다.
export function installedFiles() {
    ...
    guard(installed.files); guard([readinessRow]);
...
export function buildLocal(retirementDocument) {
    ...
    const document = fs.readFileSync(CANONICAL.authoring, 'utf8'), sets = JSON.parse(document);
    ...
    const readiness = read(readinessRef.file), compiled = sets.map(compilePublicQuestionSet), catalog = learningCatalogForBank(sets, read(CANONICAL.catalog));
    ...
    assert.equal(canonicalJson(read(CANONICAL.public)), canonicalJson(compiled));
