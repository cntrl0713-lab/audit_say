// 사례형 지정 검토 r01~r04의 운영 DB 반영: 원 8세트 퇴역과 새 4세트 추가를 한 릴리스로 적용한다.
//   --prepare : 사례형 퇴역 함수 설치 결과(db-migration/after.json)의 active release·함수 정의로 퇴역 manifest, 로컬 복원 증명, guarded SQL을 만든다(네트워크 없음).
...
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication/db-import/driver.mjs <mode> [--expected-preparation-sha256 SHA]
...
import { P, N, PROJECT, RPC, CANONICAL, read, sha, ref, write, guard, baselineSource, buildLocal, buildSql, localProof } from './contract.mjs';

const R = N + '/preparation-v1', O = N + '/publication-v1', M = P + '/db-migration';
...
const mode = process.argv[2]; assert(['--prepare', '--probe', '--apply'].includes(mode));
...
if (mode === '--prepare') {
    assert(!fs.existsSync(R), 'Preserve the existing preparation'); assert(!fs.existsSync(O), 'A DB attempt already exists; inspect it before preparing another');
    const installed = read(P + '/install-completion.json'); assert.equal(installed.status, 'canonical_installed_and_validated'); guard(installed.files);
    ...
    const expected = { release_id: after.active.release_id, source_file_hash: sha(baselineDocument), set_count: baselineSets.length };
    assert.equal(after.active.source_file_hash, expected.source_file_hash, 'Active release does not hold the reviewed baseline source');
    const stageDocument = fs.readFileSync(CANONICAL.authoring, 'utf8'), sets = JSON.parse(stageDocument);
    ...
    const manifest = { version: 1, artifact_type: 'question_bank_retirement_manifest',
        authorization: `2026-09-15 사용자 지시 “불필요한 파일은 아카이브에 넣고 지적했던 문제들 대체해서 운영Db적용해줘”; ${P}/authorization.md sha256 ${ref(P + '/authorization.md').sha256}; 수락 ${P}/acceptance-completion.json sha256 ${ref(P + '/acceptance-completion.json').sha256}`,
    ...
    fs.mkdirSync(R); fs.writeFileSync(R + '/retirement-manifest.json', document, { flag: 'wx' });
    ...
    const inputs = [...built.installed.files, built.readinessRef, ...[P + '/stage-completion.json', P + '/install-completion.json', P + '/baseline.json', P + '/plan.json', P + '/authorization.md', P + '/acceptance-completion.json', M + '/completion.json', M + '/after.json', R + '/retirement-manifest.json', ...code, ...runtime].map(ref)]; guard(inputs);
    ...
    const inputs = [...prep.inputs, prep.pack, prep.sql, prep.probe_sql, prep.proof, prep.retirement_manifest, ref(R + '/preparation.json'), ref(R + '/probe-result.json'), ...installed.files]; guard(inputs);
