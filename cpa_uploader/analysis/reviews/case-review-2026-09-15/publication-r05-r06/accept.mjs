// 지정 검토 r05~r06의 최신 판본별 agent 내용 검토와 실제 Luna 대표 실측을 비용 통제 수락 batch로 묶고, 승급 도구와 같은 검증으로 receipt를 만든다.
//   node --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r05-r06/accept.mjs
...
const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r05-r06';
...
const rounds = plan.rounds.map((round) => {
    const [set] = read(round.draft); assert.equal(set.id, round.set_id);
...
const completion = write(`${P}/acceptance-completion.json`, { status: 'accepted_for_publication', created_at: new Date().toISOString(), authorization, rounds,
    ...
    strict_mismatch_note: 'r05의 부분정답 식별 기준 2건은 예상 contradicted·실제 not_met처럼 0점 상태끼리의 차이다. 기대값을 바꾸거나 재채점하지 않았다.',
