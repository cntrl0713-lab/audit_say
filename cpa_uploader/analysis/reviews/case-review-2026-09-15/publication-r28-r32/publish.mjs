// 사례형 지정 검토 r28~r32(C등급 기존 갱신본 확장)의 정본·공개본 반영. publication-r20-r27/publish.mjs를 이 묶음(새 6세트·퇴역 11세트)에 맞춰 옮겼다. 검증 방식은 같다.
// 새 6세트에는 r31에서 분리 보존한 기준서형 세트 pilot-16-008-standards-20260921이 포함된다(퇴역 없이 추가만 하며 물음이 둘이라 학습 단위도 둘이다).
// 운영 DB 반영은 이 스크립트가 하지 않는다. 사용자가 승인한 운영 반영은 설치 뒤 별도 단계로 기록한다(authorization.md).
//   - 기준·후보·stage 사본은 커밋하지 않는 tmp/case-review-publication-r28-r32/에 두고, 이 폴더의 기록에는 해시만 남긴다(docs/문항-수정-패치-운영.md).
...
//   --stage    : tmp의 격리 stage에서 원 11세트를 빼고 새 6세트를 붙인 뒤 검수 승급(verified)·게시(published), 공개본·암호화본·분류 카탈로그, 전체 검증, DB 준비 검사를 수행한다.
//   --install  : stage 결과를 정본에 원자적으로 기록하고 정본 검사를 수행한다.
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r28-r32/publish.mjs --baseline|--stage|--install
...
const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r28-r32', T = 'tmp/case-review-publication-r28-r32', S = T + '/stage', L = P + '/stage-logs';
...
assert.equal(newIds.length, 6); assert.equal(retiredIds.length, 11); assert.equal(new Set([...newIds, ...retiredIds]).size, 17);
...
if (mode === '--baseline') {
    ...
    // 현재 정본은 직전 운영 반영(r20~r27 묶음, release c04d6bdd)의 원문과 같은 바이트여야 한다. 그 뒤 다른 작업의 변경이 없음을 확인한다.
    const release = read('cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r20-r27/db-import/publication-v1/completion.json');
    ...
    write(P + '/baseline.json', { recorded_at: new Date().toISOString(), files, copies: [ref(T + '/baseline/authoring.json'), ref(T + '/baseline/ledger.json')], copies_committed: false,
        production_release: { record: ref('cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r20-r27/db-import/publication-v1/completion.json'), release_id: release.release_id,
