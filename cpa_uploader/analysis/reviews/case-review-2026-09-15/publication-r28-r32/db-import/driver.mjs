// 사례형 지정 검토 r28~r32(C등급 기존 갱신본 확장)의 운영 DB 반영: 원 11세트 퇴역과 새 6세트 추가를 한 릴리스로 적용한다. publication-r20-r27/db-import/driver.mjs를 옮겼다.
// 새 6세트에는 r31에서 분리 보존한 기준서형 세트 pilot-16-008-standards-20260921이 포함된다(퇴역 없이 추가만 한다).
// 학습 단위 확인은 r20~r27에서 고친 유형별 판정을 그대로 가져왔다. 이번 기준서형 세트는 물음이 둘이라 단위도 둘(--sub2--standard, --sub3--standard)이므로
// 물음마다 순회하는 이 판이 반드시 필요하다. 세트당 한 단위를 단정하는 옛 판으로 되돌리면 apply가 커밋된 뒤 멈춘다.
//   --prepare : 사례형 퇴역 함수 설치 결과(r01~r04 반영의 publication/db-migration/after.json, 2026-09-16 적용)와 직전 운영 반영(사례형 r20~r27, release c04d6bdd)을 읽기 전용으로 참조해
...
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r28-r32/db-import/driver.mjs <mode> [--expected-preparation-sha256 SHA]
// 기록(준비·probe·반영 결과)은 이 폴더에, 전송 pack·SQL 사본은 커밋하지 않는 tmp/case-review-publication-r28-r32/db-import/에 둔다.
