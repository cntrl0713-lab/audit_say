...
// publication-r20-r27/db-import/contract.mjs의 퇴역·추가 계약(복원 CTE, 남는 세트·순서 조건, guarded SQL, PGlite 증명)을 그대로 이어받아 r28~r32 묶음에 적용한다.
// 바꾼 점은 입력의 위치뿐이다. 이번에도 정본이 직전 운영 반영(release c04d6bdd)과 같은 바이트에서 시작하므로 기준 원문은 이 묶음의 설치 전 baseline 사본을 쓴다.
//   - 최종 원문: 이 묶음의 stage 산출물(tmp/case-review-publication-r28-r32/stage). 설치 기록으로 정본과 같은 바이트였음을 확인한다.
//   - 기준 원문: 운영 active release(사례형 r20~r27 반영, c04d6bdd)의 저장 원문과 같은 바이트인 설치 전 정본 사본(tmp/case-review-publication-r28-r32/baseline/authoring.json).
...
export const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r28-r32', N = P + '/db-import', T = 'tmp/case-review-publication-r28-r32';
export const PROJECT = 'xvifzicrjmbfqaepcfpp', RPC = 'cpa_import_learning_question_bank_with_retirements';
// 이 묶음의 기준이 되는 운영 반영(사례형 r20~r27, release c04d6bdd). 읽기 전용으로 참조한다.
export const PRIOR_RELEASE = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r20-r27/db-import/publication-v1';
