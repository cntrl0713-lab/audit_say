// 사례형 지정 검토 r07~r11의 정본·공개본 반영. publication-r05-r06/publish.mjs를 옮기되 두 가지를 바꿨다.
//   - 기준·후보·stage 사본은 커밋하지 않는 tmp/case-review-publication-r07-r11/에 두고, 이 폴더의 기록에는 해시만 남긴다(docs/문항-수정-패치-운영.md).
//   - 학습 분류 입력은 정본 카탈로그가 가리키는 cpa_uploader/data/learning-question-classification-review.json을 제자리에서 갱신한다
//     (2026-09-19 correct_cpa_v3.ts가 정한 위치. 퇴역 세트 행을 빼고 새 세트 행을 회차 순서로 붙인다).
//   --baseline : 현재 정본 6종(은행·장부·공개본·암호화본·분류 카탈로그·분류 입력)의 해시를 기록하고 은행·장부를 tmp에 복사한다.
//   --stage    : tmp의 격리 stage에서 원 10세트를 빼고 새 5세트를 붙인 뒤 검수 승급(verified)·게시(published), 공개본·암호화본·분류 카탈로그, 전체 검증, DB 준비 검사를 수행한다.
//   --install  : stage 결과를 정본에 원자적으로 기록하고 정본 검사를 수행한다.
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11/publish.mjs --baseline|--stage|--install
...
const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11', T = 'tmp/case-review-publication-r07-r11', S = T + '/stage', L = P + '/stage-logs';
...
assert.equal(newIds.length, 5); assert.equal(retiredIds.length, 10); assert.equal(new Set([...newIds, ...retiredIds]).size, 15);
...
if (mode === '--baseline') {
    ...
    // 운영 active release(기준서형 검토 반영, c43c59e4)의 원문과 같은 바이트인지 대조한다. DB 반영의 기준 원문이 된다.
    const release = read('cpa_uploader/releases/20260919-standard-spoiler-compressed/completion.json');
    assert.equal(release.status, 'production_published_and_verified'); assert.equal(release.source_file_hash, files.find((f) => f.name === 'authoring').sha256, 'Canonical bank differs from the active production release');
    write(P + '/baseline.json', { recorded_at: new Date().toISOString(), files, copies: [ref(T + '/baseline/authoring.json'), ref(T + '/baseline/ledger.json')], copies_committed: false,
        production_release: { record: ref('cpa_uploader/releases/20260919-standard-spoiler-compressed/completion.json'), release_id: release.release_id, source_file_hash: release.source_file_hash },
