// 지정 검토 r07~r11의 최신 판본별 agent 내용 검토와 실제 Luna 대표 실측을 비용 통제 수락 batch로 묶고, 승급 도구와 같은 검증으로 receipt를 만든다.
// publication-r05-r06/accept.mjs를 이 묶음의 계획(plan.json)에 맞춰 옮겼다. 검증 방식은 같다.
//   node --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11/accept.mjs
...
const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11';
...
const rounds = plan.rounds.map((round) => {
    ...
    const E = round.execution, manifest = ref(`${E}/grading-manifest.json`), summary = read(`${E}/actual-a/summary.json`);
...
const completion = write(`${P}/acceptance-completion.json`, { status: 'accepted_for_publication', created_at: new Date().toISOString(), authorization, rounds,
    ...
    strict_mismatch_note: 'r07·r08·r09·r11의 부분정답·오답 식별 기준에서 예상 contradicted·실제 not_met처럼 0점 상태끼리의 차이가 있다(회차 README 기재: r07 2건, r08 3건, r09 1건, r11 4건, r10 v3 0건). 점수는 모두 일치했고 기대값을 바꾸거나 재채점하지 않았다.',
