// 사례형 지정 검토 r07~r11의 운영 DB 반영: 원 10세트 퇴역과 새 5세트 추가를 한 릴리스로 적용한다. publication-r05-r06/db-import/driver.mjs(v2)를 옮겼다.
//   --prepare : 사례형 퇴역 함수 설치 결과(r01~r04 반영의 publication/db-migration/after.json, 2026-09-16 적용)와 직전 운영 반영(기준서형 검토, release c43c59e4)을 읽기 전용으로 참조해
...
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11/db-import/driver.mjs <mode> [--expected-preparation-sha256 SHA]
// 기록(준비·probe·반영 결과)은 이 폴더에, 전송 pack·SQL 사본은 커밋하지 않는 tmp/case-review-publication-r07-r11/db-import/에 둔다.
...
import { P, N, T, PROJECT, RPC, STAGE, PRIOR_RELEASE, read, sha, ref, write, guard, baselineSource, buildLocal, buildSql, localProof } from './contract.mjs';
...
if (mode === '--prepare') {
    ...
    assert.equal(prior.status, 'production_published_and_verified'); assert.equal(priorVerification.passed, true); assert.equal(priorVerification.release_id, prior.release_id);
    ...
    assert.equal(prior.source_file_hash, expected.source_file_hash, 'Prior release source differs from this round\'s baseline source');
    assert.equal(priorVerification.set_count, baselineSets.length, 'Prior release set count differs from this round\'s baseline source');
    ...
    const manifest = { version: 1, artifact_type: 'question_bank_retirement_manifest',
        authorization: `2026-09-19 사용자 지시 “두 초안을 그대로 두고 전체 운영DB 대체 및 커밋 푸시 진행해줘”; ${P}/authorization.md sha256 ${ref(P + '/authorization.md').sha256}; 수락 ${P}/acceptance-completion.json sha256 ${ref(P + '/acceptance-completion.json').sha256}`,
