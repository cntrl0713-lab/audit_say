...
// publication-r05-r06/db-import/contract.mjs의 퇴역·추가 계약(복원 CTE, 남는 세트·순서 조건, guarded SQL, PGlite 증명)을 그대로 이어받아 r07~r11 묶음에 적용한다.
// 바꾼 점은 입력의 위치뿐이다.
//   - 최종 원문: 이 묶음의 stage 산출물(tmp/case-review-publication-r07-r11/stage). 설치 기록으로 정본과 같은 바이트였음을 확인한다.
//   - 기준 원문: 운영 active release(기준서형 검토 반영, c43c59e4)의 원문과 같은 바이트인 설치 전 정본 사본(tmp/…/baseline/authoring.json).
//     커밋에 없으므로 git show 대신 이 사본을 읽고, baseline.json과 운영 반영 기록의 SHA-256으로 대조한다. 운영 SQL도 이 해시를 다시 확인한다.
...
export const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11', N = P + '/db-import', T = 'tmp/case-review-publication-r07-r11';
export const PROJECT = 'xvifzicrjmbfqaepcfpp', RPC = 'cpa_import_learning_question_bank_with_retirements';
// 이 묶음의 기준이 되는 운영 반영(기준서형 검토, release c43c59e4). 읽기 전용으로 참조한다.
export const PRIOR_RELEASE = 'cpa_uploader/releases/20260919-standard-spoiler-compressed';
...
export function baselineSource() {
    ...
    const prior = read(PRIOR_RELEASE + '/completion.json'); assert.equal(prior.status, 'production_published_and_verified');
    assert.equal(prior.source_file_hash, expected, 'Baseline differs from the active production release source');
