// 사례형 지정 검토 r13~r15의 정본·공개본 반영. publication-r12/publish.mjs를 이 묶음(새 3세트·퇴역 8세트)에 맞춰 옮겼다. 검증 방식은 같다.
// 운영 DB 반영은 이 스크립트가 하지 않는다. 사용자가 승인한 운영 반영(r12 포함)은 설치 뒤 별도 단계로 기록한다(authorization.md).
//   - 기준·후보·stage 사본은 커밋하지 않는 tmp/case-review-publication-r13-r15/에 두고, 이 폴더의 기록에는 해시만 남긴다(docs/문항-수정-패치-운영.md).
...
//   --stage    : tmp의 격리 stage에서 원 8세트를 빼고 새 3세트를 붙인 뒤 검수 승급(verified)·게시(published), 공개본·암호화본·분류 카탈로그, 전체 검증, DB 준비 검사를 수행한다.
//   --install  : stage 결과를 정본에 원자적으로 기록하고 정본 검사를 수행한다.
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r13-r15/publish.mjs --baseline|--stage|--install
...
const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r13-r15', T = 'tmp/case-review-publication-r13-r15', S = T + '/stage', L = P + '/stage-logs';
...
assert.equal(newIds.length, 3); assert.equal(retiredIds.length, 8); assert.equal(new Set([...newIds, ...retiredIds]).size, 11);
...
if (mode === '--baseline') {
    ...
    // 현재 정본은 다른 세션이 설치한 r12를 이미 담고 있어 운영 active release(2ed1a151)보다 앞선다. 두 기준을 각각 대조한다.
    //   - r12 설치 기록과 바이트가 같은지: 그 설치 뒤 정본이 다른 작업으로 바뀌지 않았음을 확인한다.
    //   - active release 원문: 이번 묶음의 운영 반영이 r12 변경까지 포함한다는 사실을 기록에 남긴다(사용자 지시).
    const priorInstall = read('cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r12/install-completion.json');
    assert.equal(priorInstall.status, 'canonical_installed_and_validated');
    const priorBank = priorInstall.files.find((f) => f.file === 'cpa_uploader/data/cpa_question_sets_v3.authoring.json');
    assert.equal(priorBank.sha256, files.find((f) => f.name === 'authoring').sha256, 'Canonical bank differs from the r12 install record');
    const release = read('cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11/db-import/publication-v1/completion.json');
    ...
    assert.notEqual(releaseBank.sha256, files.find((f) => f.name === 'authoring').sha256, 'Canonical bank is expected to be ahead of the active release by the r12 change');
    write(P + '/baseline.json', { recorded_at: new Date().toISOString(), files, copies: [ref(T + '/baseline/authoring.json'), ref(T + '/baseline/ledger.json')], copies_committed: false,
        prior_install: { record: ref('cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r12/install-completion.json'), new_sets: priorInstall.new_sets, retired_sets: priorInstall.retired_sets,
            note: '다른 세션이 2026-09-20에 설치한 r12다. 이번 baseline의 정본 바이트는 그 설치 결과와 같다.' },
        production_release: { record: ref('cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11/db-import/publication-v1/completion.json'), release_id: release.release_id,
            source_file_hash: releaseBank.sha256, note: '설치 전에도 정본은 r12만큼 이 active release보다 앞서 있다. 사용자 지시에 따라 이번 증분 운영 반영은 r12 변경까지 포함한다.' },
