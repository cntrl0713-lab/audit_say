// 지정 검토 r13~r15의 agent 내용 검토와 실제 Luna 대표 실측을 비용 통제 수락 batch로 묶고, 승급 도구와 같은 검증으로 receipt를 만든다.
// publication-r12/accept.mjs를 이 묶음의 계획(plan.json)에 맞춰 옮겼다. 검증 방식은 같다.
//   node --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r13-r15/accept.mjs
...
const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r13-r15';
...
const completion = write(`${P}/acceptance-completion.json`, { status: 'accepted_for_publication', created_at: new Date().toISOString(), authorization, rounds,
    ...
    strict_mismatch_note: 'r13·r14 부분정답의 식별 기준에서 예상 contradicted·실제 not_met처럼 0점 상태끼리의 차이가 각각 3건 있다(r15는 0건). 세 회차 대표 24답안의 점수는 모두 기대와 정확히 일치했고(delta 0) 기대값을 바꾸거나 재채점하지 않았다.',
