// 사례형 지정 검토 r13~r15의 운영 DB 반영: 원 10세트 퇴역과 새 4세트 추가를 한 릴리스로 적용한다. publication-r07-r11/db-import/driver.mjs를 옮겼다.
// 이 릴리스의 범위는 r13~r15(8세트 퇴역·3세트 추가)에 더해, 다른 세션이 2026-09-20 정본에만 설치한 r12(2세트 퇴역·1세트 추가)를 사용자 지시로 함께 올린다(db-import/release-plan.json).
//   --prepare : 사례형 퇴역 함수 설치 결과(r01~r04 반영의 publication/db-migration/after.json, 2026-09-16 적용)와 직전 운영 반영(사례형 r07~r11, release 2ed1a151)을 읽기 전용으로 참조해
...
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r13-r15/db-import/driver.mjs <mode> [--expected-preparation-sha256 SHA]
// 기록(준비·probe·반영 결과)은 이 폴더에, 전송 pack·SQL 사본은 커밋하지 않는 tmp/case-review-publication-r13-r15/db-import/에 둔다.
...
if (mode === '--prepare') {
    ...
    const manifest = { version: 1, artifact_type: 'question_bank_retirement_manifest',
        authorization: `2026-09-20 사용자 지시 “각각 합쳐서 새 문제로 대체해줘”·“r12까지 같이 운영 반영하는 쪽으로 진행해줘”; ${P}/authorization.md sha256 ${ref(P + '/authorization.md').sha256}; 릴리스 범위 ${N}/release-plan.json sha256 ${ref(N + '/release-plan.json').sha256}; 수락 ${P}/acceptance-completion.json sha256 ${ref(P + '/acceptance-completion.json').sha256}`,
    ...
    const inputs = [...Object.values(STAGE).map(ref), built.readinessRef, ...[P + '/stage-completion.json', P + '/install-completion.json', P + '/baseline.json', P + '/plan.json', P + '/authorization.md', P + '/acceptance-completion.json',
        M + '/completion.json', M + '/after.json', PRIOR_RELEASE + '/completion.json', PRIOR_RELEASE + '/verification.json', T + '/release-baseline/authoring.json', N + '/release-plan.json',
        'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r12/install-completion.json', R + '/retirement-manifest.json', ...code, ...runtime].map(ref)]; guard(inputs);
