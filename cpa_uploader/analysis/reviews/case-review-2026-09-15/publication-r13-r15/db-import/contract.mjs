...
// publication-r07-r11/db-import/contract.mjs의 퇴역·추가 계약(복원 CTE, 남는 세트·순서 조건, guarded SQL, PGlite 증명)을 그대로 이어받아 r13~r15 묶음에 적용한다.
// 바꾼 점은 입력의 위치와, 이 릴리스가 r12까지 함께 올린다는 범위뿐이다(db-import/release-plan.json).
//   - 최종 원문: 이 묶음의 stage 산출물(tmp/case-review-publication-r13-r15/stage). 설치 기록으로 정본과 같은 바이트였음을 확인한다.
//   - 기준 원문: 운영 active release(사례형 r07~r11 반영, 2ed1a151)의 저장 원문과 같은 바이트인 HEAD 커밋의 정본 사본(tmp/…/release-baseline/authoring.json).
//     워킹트리 정본은 다른 세션이 설치한 r12 때문에 이미 앞서 있어 기준으로 쓰지 않는다. 운영 반영 기록의 SHA-256으로 대조하고 운영 SQL도 이 해시를 다시 확인한다.
...
export const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r13-r15', N = P + '/db-import', T = 'tmp/case-review-publication-r13-r15';
export const PROJECT = 'xvifzicrjmbfqaepcfpp', RPC = 'cpa_import_learning_question_bank_with_retirements';
// 이 묶음의 기준이 되는 운영 반영(사례형 r07~r11, release 2ed1a151). 읽기 전용으로 참조한다.
export const PRIOR_RELEASE = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11/db-import/publication-v1';
...
// 운영 반영 범위는 이 배치의 세 회차에 r12를 더한 릴리스 계획에서 읽는다(정본 설치 계획 plan.json과 구분한다).
export const plan = () => { const p = read(N + '/release-plan.json'); return { retiredIds: p.rounds.flatMap((r) => r.retires), newIds: p.rounds.map((r) => r.set_id) }; };
...
export function baselineSource() {
    // 기준 원문은 운영 active release의 저장 원문과 같은 바이트인 HEAD 커밋 정본 사본이다. 설치 전 워킹트리 정본(P/baseline.json)에는 다른 세션이 설치한 r12가 이미 들어 있어 기준이 아니다.
    const file = T + '/release-baseline/authoring.json';
    ...
    const bytes = fs.readFileSync(file), expected = priorBank.sha256;
    assert.equal(sha(bytes), expected, 'Release baseline differs from the active production release source');
    const recorded = read(P + '/baseline.json');
    assert.equal(recorded.production_release.release_id, prior.release_id);
    assert.equal(recorded.production_release.source_file_hash, expected);
    assert.notEqual(recorded.files.find((r) => r.name === 'authoring').sha256, expected, 'Install baseline is expected to be ahead of the active release by the r12 change');
    return { file, release_id: prior.release_id, release_record: ref(PRIOR_RELEASE + '/completion.json'), sha256: expected, document: bytes.toString('utf8') };
}
