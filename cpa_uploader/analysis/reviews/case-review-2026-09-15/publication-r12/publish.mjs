// 사례형 지정 검토 r12의 정본·공개본 반영. publication-r07-r11/publish.mjs를 이 묶음(새 1세트·퇴역 2세트)에 맞춰 옮겼다. 검증 방식은 같다.
// 운영 DB 반영은 이 승인에 포함되지 않으므로 하지 않는다(authorization.md).
//   - 기준·후보·stage 사본은 커밋하지 않는 tmp/case-review-publication-r12/에 두고, 이 폴더의 기록에는 해시만 남긴다(docs/문항-수정-패치-운영.md).
...
//   --stage    : tmp의 격리 stage에서 원 2세트를 빼고 새 1세트를 붙인 뒤 검수 승급(verified)·게시(published), 공개본·암호화본·분류 카탈로그, 전체 검증, DB 준비 검사를 수행한다.
//   --install  : stage 결과를 정본에 원자적으로 기록하고 정본 검사를 수행한다.
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r12/publish.mjs --baseline|--stage|--install
...
const P = 'cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r12', T = 'tmp/case-review-publication-r12', S = T + '/stage', L = P + '/stage-logs';
...
assert.equal(newIds.length, 1); assert.equal(retiredIds.length, 2); assert.equal(new Set([...newIds, ...retiredIds]).size, 3);
...
if (mode === '--baseline') {
    ...
    // 현재 운영 active release(r07~r11 반영, 2ed1a151)의 원문과 같은 바이트인지 대조한다. 이 반영은 운영 DB를 바꾸지 않으므로,
    // 설치 뒤에는 정본이 active release보다 앞서게 된다는 사실을 기록에 남기기 위한 대조다.
    ...
    write(P + '/baseline.json', { recorded_at: new Date().toISOString(), files, copies: [ref(T + '/baseline/authoring.json'), ref(T + '/baseline/ledger.json')], copies_committed: false,
        production_release: { record: ref('cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r07-r11/db-import/publication-v1/completion.json'), release_id: release.release_id,
            source_file_hash: releaseBank.sha256, note: '이 반영은 운영 DB를 바꾸지 않는다. 설치 뒤 정본은 이 active release보다 앞선 내용이 된다.' },
    ...
    write(P + '/install-completion.json', { status: 'canonical_installed_and_validated', completed_at: new Date().toISOString(), files: Object.values(canonical).map((f) => ref(rel(f))),
        ...
        production_release_unchanged: baseline().production_release.release_id,
        db_note: '사용자 지시 범위가 정본 설치와 원 세트 퇴역까지이므로 운영 DB에 반영하지 않았다. 운영 active release는 그대로이며 이 시점부터 정본이 운영보다 앞선다.' });
