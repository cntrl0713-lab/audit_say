// 사례형 지정 검토 r20~r27의 운영 DB 반영: 원 17세트 퇴역과 새 9세트 추가를 한 릴리스로 적용한다. publication-r16-r19/db-import/driver.mjs를 옮겼다.
// 새 9세트에는 r21에서 분리 보존한 기준서형 세트 pilot-11-005-standards-20260921이 포함된다(퇴역 없이 추가만 한다).
// 고친 곳은 하나다: 앞 회차 드라이버는 새 세트의 학습 단위를 <세트ID>--case로만 찾아 기준서형 분리본에서 멈췄고(r16~r19에서 apply 커밋 뒤 실제로 발생),
// 그때는 verify-v1.mjs로 재개했다. 여기서는 유형별 확인을 드라이버 안에 넣어 같은 지점에서 멈추지 않게 했다.
//   --prepare : 사례형 퇴역 함수 설치 결과(r01~r04 반영의 publication/db-migration/after.json, 2026-09-16 적용)와 직전 운영 반영(사례형 r16~r19, release 77fc4607)을 읽기 전용으로 참조해
...
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/publication-r20-r27/db-import/driver.mjs <mode> [--expected-preparation-sha256 SHA]
// 기록(준비·probe·반영 결과)은 이 폴더에, 전송 pack·SQL 사본은 커밋하지 않는 tmp/case-review-publication-r20-r27/db-import/에 둔다.
