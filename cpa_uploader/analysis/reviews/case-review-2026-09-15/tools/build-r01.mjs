// r01(55·18·19 병합) 실제 채점 입력 생성. 원 manifest 형식은 case-trio-next-2026-09-14/helpers/build-execution.mjs를 따른다.
//   node --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/tools/build-r01.mjs candidate
//   npx tsx scripts/build-learning-unit-catalog.ts --review <R>/classification-v1.json --output <R>/catalog-v1.json
//   node --import tsx cpa_uploader/analysis/reviews/case-review-2026-09-15/tools/build-r01.mjs execution
...
const SET_ID = 'case-14-group-procedures-20260915';
...
assert(['candidate', 'execution'].includes(mode) && process.argv.length === 3, 'Usage: build-r01.mjs candidate|execution');

if (mode === 'candidate') {
  const bank = read(BANK), catalog = read(CATALOG), [draft] = read(D + '/sets.json');
  ...
  assert.equal(draft.id, SET_ID); assert(!bank.some((set) => set.id === SET_ID));
  const rootReview = read(R + '/root-content-review.json');
  ...
  write(R + '/baseline.json', { bank: ref(BANK), catalog: ref(CATALOG), classification_review: ref(catalog.review_file),
    sets: bank.length, questions: bank.reduce((n, s) => n + s.subquestions.length, 0), draft: ref(D + '/sets.json') });
  write(R + '/candidate-v1.json', candidate);
  ...
  const entries = [...review.entries, ...draft.subquestions.map((question) => {
    ...
    return { set_id: SET_ID, subquestion_id: question.id, question_style: 'case', topic_ids: question.topic_ids,
      standalone_prompt: null, case_fact_ids: row.case_fact_ids, reason: `${row.topic_reason} 사례 절차의 옳고 그름을 사실에 적용해 판단해야 하므로 사례형이다.` };
  })];
  write(R + '/classification-v1.json', { source_file: R + '/candidate-v1.json', source_file_sha256: ref(R + '/candidate-v1.json').sha256, entries });
  ...
  assert(set); assert.deepEqual(validateAuthoringBank(bank).errors, []);
  ...
  assert.equal(rootReview.target.reviewed_content_sha256, reviewedContentHash(set)); assert.deepEqual(rootReview.unresolved_content_findings, []);
  assert.deepEqual(sorted(rootReview.questions.map((q) => q.subquestion_id)), sorted(set.subquestions.map((q) => q.id)));
  for (const q of rootReview.questions) for (const check of CONTENT_CHECKS) assert.equal(q.checks[check], 'pass', check);
  const qaFile = D + '/qa.json', qa = read(qaFile);
  const cases = [];
  ...
  const evidenceFiles = [B + '/authorization.md', B + '/policy-input.json', rootFile, qaFile, D + '/design.json', D + '/lineage.json', D + '/sets.json', D + '/build-draft.mjs', R + '/baseline.json', R + '/classification-v1.json'];
  const evidence = evidenceFiles.map(ref);
  const reviews = [{ set_id: SET_ID, content_hash: reviewedContentHash(set), reviewer_id: rootReview.reviewer_id, reviewed_at: rootReview.reviewed_at,
    method: 'agent_content_review', human_review_performed: false, evidence,
  ...
  write(out + '/policy.json', { scope: ref(out + '/scope.json'), model: 'gpt-5.6-luna', grading_point_tolerance: 1,
    ...
    reuse_decision: '병합한 새 사례 1개·2물음만 검토한다. 원 세 세트의 과거 실측은 계약이 달라 재사용하지 않고 분모에 넣지 않는다. 모범·부분·오답 역할을 사례 단위로 통합하여 3요청으로 실측한다.' });
  ...
  assert.equal(metadata.length, 2); assert(metadata.every((c) => c.question_style === 'case'));
  ...
  const inputFiles = [...new Set([...set.source_refs.map((s) => s.file), ...evidenceFiles, B + '/tools/build-r01.mjs', bankFile, catalogFile,
