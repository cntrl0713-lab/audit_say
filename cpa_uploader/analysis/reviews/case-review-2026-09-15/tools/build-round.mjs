// 지정 사례 검토 회차의 실제 채점 입력 생성기(build-r01.mjs의 일반화). 기존 출력이 있으면 쓰지 않는다.
//   node --import tsx <B>/tools/build-round.mjs --round r01 --version v2 --draft-dir <D> --set-id <ID> candidate
//   npx tsx scripts/build-learning-unit-catalog.ts --review <R>/classification-<version>.json --output <R>/catalog-<version>.json
//   node --import tsx <B>/tools/build-round.mjs --round r01 --version v2 --draft-dir <D> --set-id <ID> execution
...
if (mode === 'candidate') {
  ...
  const inputFiles = [...new Set([...set.source_refs.map((s) => s.file), ...evidenceFiles, `${B}/tools/build-round.mjs`, bankFile, catalogFile,
