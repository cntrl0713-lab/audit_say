// 회차 판본의 대표 실측·보조 실측 결과를 요약 파일로 남긴다(기존 파일이 있으면 쓰지 않는다).
//   node cpa_uploader/analysis/reviews/case-review-2026-09-15/tools/summarize-round.mjs --round r02 --version v1 --draft-dir <D> --set-id <ID> [--supplementary supplementary-v1]
...
while (args.length) {
  ...
  assert(['--round', '--version', '--draft-dir', '--set-id', '--supplementary'].includes(flag) && value && !options[flag], 'Unknown/duplicate argument: ' + flag);
  options[flag] = value;
}
...
const main = read(`${R}/execution-${V}/actual-a/summary.json`);
...
const summary = {
  ...
  grading: { model: 'gpt-5.6-luna', manifest: ref(`${R}/execution-${V}/grading-manifest.json`), summary: ref(`${R}/execution-${V}/actual-a/summary.json`),
