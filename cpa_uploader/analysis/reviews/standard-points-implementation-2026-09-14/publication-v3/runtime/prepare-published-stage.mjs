...
assert(paths.authoring.replaceAll('\\','/').includes('/'+R+'/publication-v3/stage/'),'Only isolated publication stage may be changed');
const routing=read(R+'/publication-review-routing-v2.json');
...
const inputs=[ref(paths.authoring),ref(paths.ledger),ref(R+'/publication-review-routing-v2.json'),...new Map(routing.entries.map(r=>[r.evidence.file,r.evidence])).values()];
...
withPublicationLock(paths.authoring,()=>{
 ...
 assert.deepEqual(validatePromotionLedger(sets,ledger,false,{efficientContext:context}),[]);
 ...
 fs.writeFileSync(R+'/publication-v3/stage/receipt-transaction.json',json({status:'all_receipts_validated_before_atomic_stage_write',sets:receipts.length,entries_appended:receipts.length*2,original_entries_preserved:true,receipts}),{flag:'wx'});
