...
const D='cpa_uploader/analysis/reviews/standard-points-implementation-2026-09-14';
const C='cpa_uploader/analysis/reviews/point-review-and-publication-2026-09-11/efficient-verification-2026-09-12/c';
const out=D+'/publication-v3',stage=out+'/stage',backup=out+'/backup';
...
const ids=read(D+'/publication-review-routing-v2.json').entries.map(r=>r.set_id),candidate=read(D+'/candidate-v3/bank.json');
const baseline=read(D+'/concurrent-additions-v1/baseline.json'),baselineHash=name=>baseline.inputs.find(i=>i.snapshot===name).sha256;
...
if(mode==='--stage'){
 assert(!fs.existsSync(out));for(const i of baseline.inputs)assert.equal(ref(i.file).sha256,i.sha256,'Canonical baseline changed: '+i.file);assert.equal(ref(canonical.authoring).sha256,baselineHash('bank.snapshot.json'),'Canonical changed during review');
 assert.equal(ref(canonical.catalog).sha256,baselineHash('catalog.snapshot.json'),'Canonical classifications changed during review');
 ...
 assert.equal(originals.find(r=>r.name==='authoring').sha256,baselineHash('bank.snapshot.json'),'Canonical changed while creating backups');
 assert.equal(originals.find(r=>r.name==='catalog').sha256,baselineHash('catalog.snapshot.json'),'Catalog changed while creating backups');
 for(const r of originals)assert.equal(ref(r.backup).sha256,r.sha256,'Backup changed while copying: '+r.file);
 const runtime=[D+'/publish.mjs',D+'/prepare-published-stage.mjs','cpa_uploader/questionSubsetReview.ts','cpa_uploader/questionBankPublication.ts','scripts/import-question-bank-v3.ts',...EFFICIENT_RUNTIME_FILES];
 const immutable=[...runtime,D+'/candidate-v3/bank.json',D+'/candidate-v3/classification-review.json',D+'/sealed-v1/batch.json',D+'/sealed-v1/readiness.json',D+'/publication-review-routing-v2.json',...read(D+'/publication-review-routing-v2.json').entries.map(r=>r.evidence.file)].map(ref);
 fs.mkdirSync(out+'/runtime');for(const file of runtime)fs.copyFileSync(file,out+'/runtime/'+path.basename(file),fs.constants.COPYFILE_EXCL);
 ...
 fs.copyFileSync(D+'/candidate-v3/bank.json',stage+'/authoring.json');fs.copyFileSync(canonical.ledger,stage+'/promotions.json');
 run('01-verify-publish-transaction',[D+'/prepare-published-stage.mjs'],stagedEnv,guard);
 run('04-compile',['scripts/compile-question-bank-v3.ts'],stagedEnv,guard);
 run('05-catalog',[C+'/rebind-final-catalog.ts','--bank',stage+'/authoring.json','--review',D+'/candidate-v3/classification-review.json','--output',stage+'/classification-review.json','--catalog-output',stage+'/catalog.json'],stagedEnv,guard);
 ...
 withPublicationLock(canonical.authoring,()=>{
  ...
  try{
   ...
   run('08-canonical-catalog',[C+'/rebind-final-catalog.ts','--bank',canonical.authoring,'--review',D+'/candidate-v3/classification-review.json','--output',out+'/canonical-classification-review.json','--catalog-output',out+'/canonical-catalog.json'],env,guard);
