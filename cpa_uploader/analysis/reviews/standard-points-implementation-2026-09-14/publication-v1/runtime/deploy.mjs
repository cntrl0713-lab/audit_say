...
const base='cpa_uploader/analysis/reviews/standard-points-implementation-2026-09-14';
...
const url=process.env.NEXT_PUBLIC_SUPABASE_URL;assert.equal(new URL(url).hostname,'xvifzicrjmbfqaepcfpp.supabase.co');
const db=createClient(url,process.env.SUPABASE_SERVICE_ROLE_KEY,{auth:{persistSession:false,autoRefreshToken:false}});
const active=await db.rpc('cpa_get_active_question_bank');assert(!active.error&&active.data?.length,'Cannot verify current DB release');
assert.deepEqual([...new Set(active.data.map(s=>s.release_id))],[read(base+'/concurrent-additions-v1/db-before/summary.json').release_id],'Another task published a DB release during this review');
write(out+'/concurrency-preflight.json',{checked_at:new Date().toISOString(),original_release_id:read(base+'/concurrent-additions-v1/db-before/summary.json').release_id,unchanged:true});
