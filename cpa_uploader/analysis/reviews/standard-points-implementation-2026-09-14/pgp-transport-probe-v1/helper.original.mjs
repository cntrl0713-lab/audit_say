...
const output = `${R}/pgp-transport-probe-v1`;
...
fs.mkdirSync(output);
const homedir = path.resolve(output, 'gnupg'); fs.mkdirSync(homedir);
...
function encrypt(file, destination) {
    const r = spawnSync(gpg, ['--no-options', '--homedir', homedir, '--batch', '--pinentry-mode', 'loopback', '--passphrase', passphrase,
        '--symmetric', '--compress-algo', 'ZLIB', '--cipher-algo', 'AES256', '--output', path.resolve(destination), path.resolve(file)],
...
const decrypted = spawnSync(gpg, ['--no-options', '--homedir', homedir, '--batch', '--pinentry-mode', 'loopback', '--passphrase', passphrase,
    '--decrypt', path.resolve(encryptedFile)], { windowsHide: true, timeout: 60000, maxBuffer: original.length + 1024 * 1024 });
