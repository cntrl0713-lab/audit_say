...
for (let index = 2; index < process.argv.length; index += 2) {
  ...
  assert(['--execution', '--output', '--prior-executions', '--findings'].includes(key) && !args.has(key) && value && !value.startsWith('--'), 'Unknown/duplicate/incomplete argument');
  args.set(key, value);
}
...
const findingsName = args.get('--findings') ?? 'residual-findings-v1.json';
assert(/^residual-findings-v\d+\.json$/.test(findingsName));
const findingsFile = R + '/' + findingsName, findingsDocument = fs.existsSync(findingsFile) ? read(findingsFile) : [];
const findings = (Array.isArray(findingsDocument) ? findingsDocument : findingsDocument.findings).map(finding => ({
  ...finding,
  subquestion_id: finding.subquestion_id ?? finding.question_key.split('/').at(-1),
  rationale: finding.rationale ?? finding.actual_error + ' ' + finding.distinction,
}));
