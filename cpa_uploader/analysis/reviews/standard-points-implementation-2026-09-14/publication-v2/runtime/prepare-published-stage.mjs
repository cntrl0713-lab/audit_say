...
assert(paths.authoring.replaceAll('\\','/').includes('/'+R+'/publication-v2/stage/'),'Only isolated publication stage may be changed');
...
withPublicationLock(paths.authoring,()=>{
 ...
 fs.writeFileSync(R+'/publication-v2/stage/receipt-transaction.json',json({status:'all_receipts_validated_before_atomic_stage_write',sets:receipts.length,entries_appended:receipts.length*2,original_entries_preserved:true,receipts}),{flag:'wx'});
