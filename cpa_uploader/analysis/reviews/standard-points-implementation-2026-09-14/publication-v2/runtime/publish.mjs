...
const ids=read(D+'/publication-review-routing.json').entries.map(r=>r.set_id),candidate=read(D+'/candidate-v3/bank.json');
...
if(mode==='--stage'){
 ...
 const immutable=[...runtime,D+'/candidate-v3/bank.json',D+'/candidate-v3/classification-review.json',D+'/sealed-v1/batch.json',D+'/sealed-v1/readiness.json',D+'/publication-review-routing.json',...read(D+'/publication-review-routing.json').entries.map(r=>r.evidence.file)].map(ref);
