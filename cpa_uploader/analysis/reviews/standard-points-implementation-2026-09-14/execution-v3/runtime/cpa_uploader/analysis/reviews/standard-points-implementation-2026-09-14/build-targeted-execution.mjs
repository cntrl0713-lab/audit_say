...
const candidate=process.argv[2]??'candidate-v3',execution=process.argv[3]??'execution-v3';
...
const targetReview=read(R+'/targeted-clarification.json');
const targets=[bank.find(s=>s.id===targetReview.set_id)];assert(targets[0]);
assert.deepEqual(validateAuthoringBank(bank).errors,[]);
const reviews=read(C+'/content-reviews.json').filter(r=>r.set_id===targetReview.set_id),qa=read(C+'/representatives.json').filter(r=>r.set_id===targetReview.set_id);
...
const peer=read(R+'/targeted-integration-check.json');
...
const evidence=[R+'/baseline.json',R+'/authorization.md',R+'/targeted-integration-check.json',R+'/targeted-clarification.json',C+'/integration.json',C+'/content-reviews.json',C+'/lineage.json',C+'/representatives.json',...checks.plan_files.map(p=>p.file)];
const cases=[];
for(const row of qa){
...
const code=[...EFFICIENT_RUNTIME_FILES,R+'/run-efficient-grading.ts',R+'/accounting.ts',R+'/contract.ts',R+'/build-targeted-execution.mjs'];
