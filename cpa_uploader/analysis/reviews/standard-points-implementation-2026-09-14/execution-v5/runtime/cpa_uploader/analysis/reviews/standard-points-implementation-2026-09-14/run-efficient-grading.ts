...
export function prepare(options: Options) {
    ...
    if (manifest.budget_enforcement === 'not_specified') assert.equal(manifest.budget_usd, null, 'Unspecified budget must be null');
        ...
        assert(typeof manifest.budget_usd === 'number' && Number.isFinite(manifest.budget_usd) && manifest.budget_usd > 0,
            'Provider monetary limit must be explicitly recorded and positive');
    }
...
export async function main(args = process.argv.slice(2)) {
    ...
    write('preflight.json', { version: 1, manifest: { file: relative(options.manifest), sha256: options.manifestHash }, worker: options.worker,
        ...
        pricing: PRICING, budget_usd: prepared.manifest.budget_usd, budget_enforcement: prepared.manifest.budget_enforcement, pre_request_token_estimation: false,
