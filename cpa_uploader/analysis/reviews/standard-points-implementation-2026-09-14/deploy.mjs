...
const installed=read(base+'/publication-v3/install-completion.json');
assert.equal(installed.status,'canonical_installed_and_validated');
const retirementFile=base+'/retirement-manifest.json';
const retirement=read(retirementFile);
assert.equal(retirement.candidate_source_file_hash,sha(bank));
assert.equal(read(base+'/db-retirement-migration-v1/completion.json').status,'retirement_rpc_installed_and_verified');
assert(!fs.existsSync(out),'Publication attempt already exists; inspect its receipt before any retry');
const inputs=[...installed.files,...[retirementFile,retirement.source_to_replacement_lineage,base+'/publication-review-routing-v2.json',base+'/db-retirement-migration-v1/completion.json',base+'/deploy.mjs','scripts/import-question-bank-v3.ts'].map(ref)];
...
run('01-import',['scripts/import-question-bank-v3.ts','--apply','--expected-hash',sha(bank),'--project-host','xvifzicrjmbfqaepcfpp.supabase.co','--learning-catalog',catalog,'--evidence',base+'/publication-review-routing-v2.json','--retirement-manifest',retirementFile,'--retirement-manifest-sha256',sha(retirementFile),'--report',out+'/readiness.json','--receipt',out+'/receipt.json']);
...
assert.equal(receipt.retirement_manifest_sha256,sha(retirementFile));
assert.deepEqual(receipt.retirement_manifest,retirement);
