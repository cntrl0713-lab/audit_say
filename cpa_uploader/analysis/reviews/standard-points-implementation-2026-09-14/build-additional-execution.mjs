...
const candidate=process.argv[2]??'candidate-v1',execution=process.argv[3]??'execution-v2';
...
const retainedReview=read(R+'/subset-additional-representatives.json');
const retainedIds=read(R+'/subset-regrade-additional-required.json').sets.map(s=>s.set_id);
const targets=[...retainedIds.map(id=>{const s=bank.find(s=>s.id===id);assert(s);return s;})];
...
for(const row of retainedReview.questions){
 ...
 reviews.push({set_id:s.id,subquestion_id:q.id,rationale:'기준서형 일부 삭제 후 잔존한 물음이며, 과거 의미검수 기록이 현행 독립 학습 입력을 결속하지 못하여 검수 근거를 그대로 계승하지 않고 원발문·저장 모범답안·배점·출처를 새로 대조하고 실측한다.',source_review:row.source_review});
...
const evidence=[R+'/baseline.json',R+'/authorization.md',R+'/implementation-content-checks.json',R+'/subset-regrade-additional-required.json',R+'/subset-additional-representatives.json',C+'/content-reviews.json',C+'/lineage.json',C+'/representatives.json',...checks.plan_files.map(p=>p.file)];
const cases=[];
for(const row of qa){
...
const code=[...EFFICIENT_RUNTIME_FILES,R+'/run-efficient-grading.ts',R+'/accounting.ts',R+'/contract.ts',R+'/build-additional-execution.mjs'];
