// 기준서형 수정·스포일러 전수 검토(2026-09-19) 후속(v2)의 실제 채점 입력(execution-v<N>)을 만든다. 기존 출력이 있으면 쓰지 않는다.
//   node --import tsx cpa_uploader/analysis/reviews/standard-spoiler-2026-09-19/tools/build-execution-v2.mts --version v2
// v1 도구(build-execution.mts)는 v1 manifest에 고정되어 있어 고치지 않고, 입력만 v2로 바꾼 사본이다.
// 입력: evidence-v2(correct_cpa_v3 evidence가 만든 부분 은행·카탈로그·검사 기록), plan-v2.json(물음별 agent 내용 대조),
// representatives-reused-v2.json(v1 실행 전에 확정한 대표 답안의 재사용 선택과 바뀐 명제에 대한 기대 판정 재대조).
...
const plan = read<{ entries: { set: string; sub: string; tier: string; issue?: string; review: string }[] }>(`${B}/plan-v2.json`).entries;
const reused = read<{ selections: { set_id: string; subquestion_id: string; kind: 'partial' | 'wrong'; source: { file: string; sha256: string }; source_index: number; source_case_id: string | null; expectation_recheck: string }[] }>(`${B}/representatives-reused-v2.json`).selections;
const corrections = read<{ files: string[] }>(`${B}/corrections-v2.json`).files;
...
void reasonText; // v2는 새로 쓴 대표 답안이 없다.
for (const r of reused) {
    ...
    finish(r.set_id, r.subquestion_id, r.kind, row.answer as string, verdicts,
        { file: r.source.file, sha256: r.source.sha256, index: r.source_index, case_id: r.source_case_id, reason: String(row.reason ?? '원 배치의 대표 답안'),
          reused_by: `${B}/representatives-reused-v2.json`, expectation_recheck: r.expectation_recheck });
}

// ---------------------------------------------------------------- scope, reviews, runtime
fs.mkdirSync(OUT); fs.mkdirSync(`${OUT}/projections`); fs.mkdirSync(`${OUT}/runtime`);
write(`${OUT}/representatives.json`, { version: 1, purpose: '기준서형 수정·스포일러 전수 검토 후속(v2)의 대표 부분정답·오답(v1 실행 전 확정 답안 재사용, 기대 판정 재대조)', cases });
...
write(`${OUT}/policy.json`, { scope: ref(`${OUT}/scope.json`), model: 'gpt-5.6-luna', grading_point_tolerance: 1,
    ...
    reuse_decision: `v1 실측에서 드러난 채점 입력 오류를 고친 ${bank.length}세트의 모든 물음을 학습 단위로 다시 실측한다. 명제·모범답안이 바뀌어 v1 관측(다른 은행)은 재사용할 수 없다. 대표 부분정답·오답 문장은 v1 실행 전에 확정한 것을 재사용하고 기대 판정을 바뀐 명제와 다시 대조했다. 부분 은행에는 수정한 세트만 담는다.` });
const reviewer = 'agent:claude-opus-5 (author agent; no independent peer review)';
const reviewedAt = new Date().toISOString();
const commonEvidence = [`${B}/authorization.md`, `${B}/policy-input.json`, `${B}/plan-v2.json`, `${E}/correction-check.json`,
    `${B}/representatives-reused-v2.json`, `${B}/tools/build-corrections-v2.mts`, `${B}/batch-v1-validation.json`, `${B}/residual-findings-v1.json`];
...
const inputFiles = [...new Set([...bank.flatMap((s) => s.source_refs.map((r) => r.file)), ...evidenceFiles, ...reusedSources,
    `${B}/tools/build-execution-v2.mts`, `${E}/correction-classification-review.json`, bankFile, catalogFile,
