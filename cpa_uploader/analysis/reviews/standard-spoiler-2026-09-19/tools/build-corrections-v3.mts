// 기준서형 수정·스포일러 전수 검토(2026-09-19)의 둘째 후속 계획(plan-v3.json)을 세트별 correction 파일로 옮긴다.
//   node --import tsx cpa_uploader/analysis/reviews/standard-spoiler-2026-09-19/tools/build-corrections-v3.mts [--write]
// build-corrections-v2.mts는 v2 manifest에 고정되어 있어 입력·이름만 바꾼 사본이다. 명제·critical_facts·모범답안만 고치고
// 발문·배점·criterion 구성은 바꾸지 않는다.
...
const DATE = '20260919';
const SLUG = 'standard-spoiler-followup2';
...
const plan = read<{ entries: Entry[] }>(`${B}/plan-v3.json`).entries;
for (const e of plan) for (const key of Object.keys(e)) assert(allowed.has(key), `${e.set}/${e.sub}: plan-v3이 다루지 않는 필드 ${key}`);
...
for (const setId of setIds) {
    ...
    for (const sub of set.subquestions) assert(plan.some((e) => e.set === setId && e.sub === sub.id), `${setId}/${sub.id}: 대상 세트의 모든 물음을 plan-v3에서 다시 대조한다`);
    ...
    const summary = tidy(`기준서형 검토 후속: 명제의 기준서 번호 요건 정리(${entries.map((e) => e.sub).join('·')})`);
    const correction = {
        version: 1 as const, artifact_type: 'question_set_correction' as const,
        correction_id: `${DATE}-${setId}--${SLUG}`, set_id: setId, summary,
        note: `기준서형 수정·스포일러 전수 검토의 v1 실측(${B}/batch-v1.json)에서 기준서 번호만 생략한 답안이 감점된 사례에 따라, 발문이 묻지 않는 기준서 번호를 득점 요건으로 읽히지 않게 명제·critical_facts를 고친다(${B}/plan-v3.json). 발문·학습 유형·주제·배점은 바꾸지 않는다.`,
...
if (write) {
    ...
    fs.writeFileSync(`${B}/corrections-v3.json`, `${JSON.stringify({ version: 1, created_by: `${B}/tools/build-corrections-v3.mts`,
        plan: `${B}/plan-v3.json`, files: corrections.map((c) => `${CORRECTIONS_DIRECTORY}/${c.correction_id}.json`) }, null, 2)}\n`, { flag: 'wx' });
