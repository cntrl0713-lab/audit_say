// 기준서형 수정·스포일러 전수 검토(2026-09-19) 둘째 후속(v3)의 실제 채점 입력(execution-v<N>)을 만든다. 기존 출력이 있으면 쓰지 않는다.
//   node --import tsx cpa_uploader/analysis/reviews/standard-spoiler-2026-09-19/tools/build-execution-v3.mts --version v3
// v1 도구(build-execution.mts)는 v1 manifest에 고정되어 있어 고치지 않고, 입력만 v3으로 바꾼 사본이다.
// 입력: evidence-v3(correct_cpa_v3 evidence가 만든 부분 은행·카탈로그·검사 기록), plan-v3.json(물음별 agent 내용 대조),
// representatives-authored-v3.json(새로 쓴 부분정답·오답), representatives-reused-v3.json(v1 대표 답안 재사용과 기대 판정 재대조).
// 모범답안은 저장된 model_answer를 그대로 쓴다. 학습 단위마다 모범·부분·오답을 한 요청씩 만든다.
...
const plan = read<{ entries: { set: string; sub: string; tier: string; issue?: string; review: string }[] }>(`${B}/plan-v3.json`).entries;
const authored = read<{ cases: { set_id: string; subquestion_id: string; kind: 'partial' | 'wrong'; answer: string; verdicts: Record<string, Verdict>; reason: string }[] }>(`${B}/representatives-authored-v3.json`).cases;
const reused = read<{ selections: { set_id: string; subquestion_id: string; kind: 'partial' | 'wrong'; source: { file: string; sha256: string }; source_index: number; source_case_id: string | null; expectation_recheck: string }[] }>(`${B}/representatives-reused-v3.json`).selections;
const corrections = read<{ files: string[] }>(`${B}/corrections-v3.json`).files;
...
for (const a of authored) {
    finish(a.set_id, a.subquestion_id, a.kind, a.answer,
        ...
        { file: `${B}/representatives-authored-v3.json`, authored_in_this_batch: true, reason: a.reason });
}
for (const r of reused) {
    assert(r.expectation_recheck?.trim(), `${r.set_id}/${r.subquestion_id}/${r.kind}: 기대 판정 재대조 기록 없음`);
    ...
    finish(r.set_id, r.subquestion_id, r.kind, row.answer as string, verdicts,
        { file: r.source.file, sha256: r.source.sha256, index: r.source_index, case_id: r.source_case_id, reason: String(row.reason ?? '원 배치의 대표 답안'),
          reused_by: `${B}/representatives-reused-v3.json`, expectation_recheck: r.expectation_recheck });
}

// ---------------------------------------------------------------- scope, reviews, runtime
fs.mkdirSync(OUT); fs.mkdirSync(`${OUT}/projections`); fs.mkdirSync(`${OUT}/runtime`);
write(`${OUT}/representatives.json`, { version: 1, purpose: '기준서형 수정·스포일러 전수 검토 둘째 후속(v3)의 대표 부분정답·오답(실행 전 확정·재대조)', cases });
...
write(`${OUT}/policy.json`, { scope: ref(`${OUT}/scope.json`), model: 'gpt-5.6-luna', grading_point_tolerance: 1,
    ...
    reuse_decision: `명제의 기준서 번호 요건을 정리한 ${bank.length}세트의 모든 물음을 학습 단위로 다시 실측한다. 명제가 바뀌어 이전 관측(다른 은행)은 재사용할 수 없다. v1 대표 답안은 기대 판정을 바뀐 명제와 다시 대조해 재사용하고, 번호만 생략한 답안을 확인할 대표가 없는 물음은 새로 썼다. 부분 은행에는 수정한 세트만 담는다.` });
const reviewer = 'agent:claude-opus-5 (author agent; no independent peer review)';
const reviewedAt = new Date().toISOString();
const commonEvidence = [`${B}/authorization.md`, `${B}/policy-input.json`, `${B}/plan-v3.json`, `${E}/correction-check.json`,
    `${B}/representatives-authored-v3.json`, `${B}/representatives-reused-v3.json`, `${B}/tools/build-corrections-v3.mts`, `${B}/batch-v1-validation.json`];
...
const inputFiles = [...new Set([...bank.flatMap((s) => s.source_refs.map((r) => r.file)), ...evidenceFiles, ...reusedSources,
    `${B}/tools/build-execution-v3.mts`, `${E}/correction-classification-review.json`, bankFile, catalogFile,
