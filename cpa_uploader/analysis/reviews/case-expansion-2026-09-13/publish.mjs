...
const D='cpa_uploader/analysis/reviews/case-expansion-2026-09-13';
...
if(mode==='--stage'){
 assert(!fs.existsSync(out));assert.equal(ref(canonical.authoring).sha256,read(D+'/baseline.json').bank.sha256,'Canonical changed during review');
 assert.equal(ref(canonical.catalog).sha256,read(D+'/baseline.json').catalog.sha256,'Canonical classifications changed during review');
 ...
 const oldIds=ids.filter(id=>read('cpa_uploader/drafts/case-expansion-2026-09-13/bank-before.json').some(s=>s.id===id)),newIds=ids.filter(id=>!oldIds.includes(id));
 const evidence=D+'/sealed-v1/batch.json';
 run('01-reverify',['cpa_uploader/promote_cpa_v3.ts','--to','verified','--sets',oldIds.join(','),'--efficient-review',evidence,'--evidence',evidence,'--reverify'],stagedEnv,guard);
 ...
 const guardOriginal=()=>{for(const r of [...prep.originals,...prep.immutable,...completion.files])assert.equal(ref(r.file).sha256,r.sha256,'Concurrent change: '+r.file);};
 guardOriginal();
 ...
 withPublicationLock(canonical.authoring,()=>{
  ...
  try{
   ...
   if(prep.originals.every(r=>allowed.get(r.file).has(ref(r.file).sha256))){writePublicationFiles(prep.originals.map(r=>({file:r.file,content:fs.readFileSync(r.backup,'utf8')})),prep.originals.map(r=>({file:r.file,hash:ref(r.file).sha256})));rollback='restored_exact_original_bytes';}
