...
test('369 plus three synthetic Unicode cases roundtrip to exact source bytes and full payload',async()=>{
 const baseline=Array.from({length:369},(_,index)=>({id:'prior-'+index,text:'보존할 원문\n공백·따옴표 "확인" 🧮'}));
 ...
 for(const set_count of ['369',0,-1,1.5,'1; commit;'])assert.throws(()=>buildSql(proof.pack,{release_id:'1'.repeat(36),source_file_hash:sha(built.baselineDocument),set_count},[],proof.payloadHash),/Positive integer/);
});
