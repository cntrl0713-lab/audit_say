...
직전 `case-trio-2026-09-14/helpers`와 그 배치의 최종 `prepare-coverage-v2.mjs`, source 수집·문서 링크 후속 코드, `incremental-db-publication-v1`을 실제 읽고 새 경로로 분리했다. `reuse-origins.json`·`adaptation-origins.json`과 `reuse-source/`는 원 코드 바이트·해시이며 `provenance.json`은 새 코드와 검사 근거다. 과거 실행·실측·검토 통과는 새 배치의 성공으로 복사하지 않는다. source catalogue 계약은 D/source-catalog-final.json이다. 준비 시 읽은 정본은 372개 문제·555개 물음·1,889점이며 실제 통합 직전 수치는 다시 baseline에서 확인한다.

아래에서 D는 `cpa_uploader/drafts/case-trio-next-2026-09-14`, R은 `cpa_uploader/analysis/reviews/case-trio-next-2026-09-14`, H는 R/helpers, N은 R/incremental-db-publication-v1이다. 모든 명령은 저장소 루트에서 실행한다. 표의 순서는 사용법이며 실제 완료 기록이 아니다.
...
| 5 | 아래 source 수집과 root의 source-evidence-plan 완료 후 `node --import tsx H/record-root-review.mjs`, `node --import tsx H/complete-draft-evidence.mjs` | peer·공식 원자료·raw 보존본과 실제 root 판정을 결속 |
...
| 9 | `node --env-file=.env.local --import tsx H/publish.mjs --stage` | stage/install 공통 env 파일·암호화키 검사를 쓰기 전에 수행. 격리 검증·full DB readiness |
| 10 | `node --env-file=.env.local --import tsx H/publish.mjs --install` | 동일 env 인자가 필수다. 승인 범위 내 root 실행. 백업·원본 해시·잠금·실패 롤백과 정본 검증 |
...
| 16 | D/R README·review-preview를 실제 결과로 정리한 뒤 `node H/final-artifact-check.mjs --final-checks final-checks-v1` | 공백을 포함한 `<절대경로>` 링크·최종 사실/발문/답안/criterion 전문·고정 manifest·DB·최종 검사 확인. 점수는 현재 9개 물음에서 계산 |
...
원문 수집은 root가 `node H/collect-source-evidence.mjs --extra-sources R/source-inventory-extras.json`으로 실행한다. extras 인자는 생략할 수 있으며, 지정하면 `[{file,sha256}]` 배열에 공식 목록·원PDF·추출/렌더 계보 등 작성자 목록 외 실제 사용 증거를 넣는다. a/b/c source-files와 source_refs는 자동 포함된다. `raw/materials` 안에 이미 보존된 파일은 다시 수집하지 않고 source-inventory.json의 excluded 및 source-collection.json의 existing_raw_materials로 기록한다. root는 이 파일과 원 렌더/추출 계보도 plan.additional_files에 결속한다. 새 수집 경로는 `cpa_uploader/raw/collections/2026-09-14-case-trio-next`다. 이 도구는 출처 의미검수나 source-evidence-plan을 자동 작성하지 않으며 기존 수집 출력이 있으면 중단한다.
...
작성자 coverage-proposals.json은 각 한 행을 담은 배열이다. 로컬 source_ref ID와 catalogue unit ID가 다르면 명시 `source_unit_ids`를 제공해야 한다. 각 선택 criterion의 인용은 동일 file의 실제 catalogue unit.quote에 공백 정규화 후 포함되어야 한다. 새 관계 ID는 `coverage-trio-next-20260914-a/b/c`이며 이전 배치 ID를 재사용하지 않는다.

도구 자체 검사는 `node H/validate-static.mjs`, `node N/validate-local.mjs`다. syntax/상대 import/lint/targeted TypeScript와 최종 검사·부분점수·coverage·환경·raw 제외·링크 fixture 및 로컬 PGlite commit/rollback을 확인한다. 그 뒤 `node H/finalize-provenance.mjs`로 원본과 새 바이트의 diff·해시를 봉인한다. N/independent-review.json은 이와 별도로 독립 검토자가 실제 현재 파일을 확인해 작성해야 한다. 정적 검사 후 도구를 바꾸면 원 결과를 덮어쓰지 않고 후속 증거와 새 provenance 버전을 사용한다.
