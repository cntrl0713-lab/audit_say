...
const alreadyArchived=[...paths].filter(file=>file.startsWith('cpa_uploader/raw/materials/'));
const entries=[...paths].filter(file=>!alreadyArchived.includes(file)).sort().map(original_path=>({original_path,sha256:hash(original_path),category:original_path.includes('/data/official/')?'official':original_path.includes('/회계감사_통합학습자료/')?'learning':'verification',role:'2026-09-14 신규 세 사례의 원문·판본·발문·시각대조 및 기존 취득계보 바이트 보존'}));
write(R+'/source-inventory-v2.json',{version:1,created_at:new Date().toISOString(),entries,excluded:alreadyArchived.map(file=>({...ref(file),reason:'원PDF 파생 시각대조 자료는 이미 raw 안에 직접 보존되어 있다. 다시 수집하지 않으며 기존 render-provenance와 새 plan additional_files에 결속한다.'})),provenance_notes:['첫 수집 시 raw 내부 시각자료의 재수집을 preflight가 거부했다. 출처 원문·보존본에 쓰기 전 실패했고 기존 입력과 코드를 보존한다.']});
for(const args of [['cpa_uploader/raw/collect.mjs','--input',R+'/source-inventory-v2.json','--output',collection],['cpa_uploader/raw/collect.mjs','--check','--against-originals','--output',collection]]){
...
write(R+'/source-evidence-plan.json',{version:1,status:'complete',raw_collection_manifests:[ref(collection+'/manifest.json')],source_peer_reviews:[ref(D+'/source-peer-review.json')],additional_files:[...alreadyArchived.map(ref),ref(R+'/source-inventory-v2.json'),ref(R+'/collect-source-evidence-v2.mjs'),ref(R+'/c-correction.json'),ref(R+'/c-source-range-correction.json')]});
