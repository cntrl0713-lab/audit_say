...
직전 성공 릴리스 `a4fb1719-b3fa-43bb-975f-5e1c481e67bd`의 DB roundtrip 함수 정의·권한 스냅샷은 sources/prior-db-roundtrip.json.txt에 바이트 보존했다. 이는 함수 출처 계보이며 현재 active ID는 실행 시 별도로 확인한다. 새 source-provenance.json은 원 코드와 사본 및 여기서 추출한 installed-functions-after.json의 해시를 연결한다. 새 독립 code review는 independent-review.json에 status=passed_static_review, human_review_performed=false, unresolved_findings=[], files=[{file,sha256}]로 기록하며 driver·append-contract·source-provenance의 현재 해시를 반드시 포함해야 한다. 이 문서는 검토 완료를 대신하지 않는다.
...
```text
node --import tsx cpa_uploader/analysis/reviews/case-trio-next-2026-09-14/incremental-db-publication-v2/driver.mjs --prepare --expected-active-release-id ROOT_VERIFIED_ACTIVE_UUID
```
...
```text
node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/case-trio-next-2026-09-14/incremental-db-publication-v2/driver.mjs --apply --expected-preparation-sha256 PREPARATION_SHA256
```
...
Management SQL의 HTTP 성공은 `response.ok`로 확인하여 정상 201도 허용한다. 성공 HTTP만으로 게시 완료를 인정하지 않고 JSON 응답 형상·receipt·원문·분류·독립 roundtrip까지 검증한다. 전체 은행 문서를 REST payload로 보내지 않으며 저장된 baseline 원문과 추가 3개를 서버의 동일 트랜잭션 안에서 복원한다.
...
tests/source-reconstruction.test.mjs는 372개 baseline과 새 3개 합성 Unicode 문서의 정확한 source bytes·정식 전체 payload 동일성 및 SQL count 입력 거부를 확인한다. 실제 배치 prepare에서는 숫자를 하드코딩하지 않고 integration-baseline과 stage를 읽는다. 도구 검사는 `node N/validate-local.mjs`로 실행하고 원 검사 로그·결과를 보존한다.
