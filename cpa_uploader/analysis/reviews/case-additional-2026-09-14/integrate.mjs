...
const sourceCatalog=read(D+'/source-catalog.json');
...
for(const agent of ['a','b']){
 ...
 assert.equal(sets.length,3);assert.equal(rs.length,9);assert.equal(cases.length,18);
...
assert.equal(changes.length,6);assert.equal(new Set(changes.map(s=>s.id)).size,6);
...
for(const s of changes){
 ...
 for(const q of s.subquestions){
  ...
  const matched=qa.filter(r=>r.set_id===s.id&&r.subquestion_id===q.id);assert.equal(matched.length,2);assert.deepEqual(matched.map(r=>r.kind).sort(),['partial','wrong']);
  for(const row of matched){const ids=row.met_criterion_ids;assert.equal(new Set(ids).size,ids.length);assert(ids.every(id=>q.criteria.some(c=>c.id===id)));assert.equal(row.expected_points,q.criteria.filter(c=>ids.includes(c.id)).reduce((n,c)=>n+c.scores.met,0));assert(row.kind==='wrong'?row.expected_points===0:row.expected_points>0&&row.expected_points<q.criteria.reduce((n,c)=>n+c.max_points,0));}
...
write('candidate-v1.json',bank);write('classification-v1.json',{source_file:R+'/candidate-v1.json',source_file_sha256:hash(fs.readFileSync(R+'/candidate-v1.json')),entries});
...
write('shape-check.json',{new_cases:6,new_questions:18,minimum_facts_characters:Math.min(...shapes.map(s=>s.facts_characters)),rows:shapes});
...
console.log({sets:bank.length,questions:checked.subquestionCount,new_cases:6,new_questions:18});
