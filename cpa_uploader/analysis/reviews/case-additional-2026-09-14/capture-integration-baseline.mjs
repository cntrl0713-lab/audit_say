...
const record={created_at:new Date().toISOString(),reason:'제작 중 다른 작업이 게시한 기준서 27세트를 보존하기 위해 실제 채점 전 최신 정본·분류를 별도 통합 입력으로 동결한다. 제작 시작 시점 원본과 기존 실행 증거는 수정하지 않는다.',prior_snapshot:D+'/bank-before.json',original_sets_preserved:old.length,concurrent_added_set_ids:added.map(s=>s.id),...Object.fromEntries(refs.map(r=>[r.name,r]))};
