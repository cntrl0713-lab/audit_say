...
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/standard-backlog-publication-2026-09-14/db-baseline.mjs
...
const out={checked_at:new Date().toISOString(),project_host:new URL(url).hostname,release,canonical:{file:bankFile,sha256:createHash('sha256').update(bytes).digest('hex')},sets:data.length,questions:data.flatMap(s=>s.subquestions).length,public_matches_canonical:true,db_writes:0};
