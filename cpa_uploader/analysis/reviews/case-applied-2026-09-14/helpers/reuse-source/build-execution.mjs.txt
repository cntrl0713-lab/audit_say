...
assert.equal(new Set(ids).size, 6, 'User requested exactly six new case sets');
assert.equal(ids.length, 6); assert(target.every(Boolean));
...
assert.equal(bank.length, baseline.length + target.length, 'Candidate must contain baseline plus six new cases');
...
assert.deepEqual(sorted(rootReview.questions.map(question => question.set_id + '/' + question.subquestion_id)), sorted(requiredKeys), 'Root review must explicitly cover all 18 questions once');
assert.deepEqual(sorted(authorReviews.map(question => question.set_id + '/' + question.subquestion_id)), sorted(requiredKeys), 'Author review must cover all 18 questions once');
...
const evidenceFiles = [...new Set([R + '/designs.json', R + '/draft-evidence.json', R + '/case-reviews.json', rootFile,
  qaFile, R + '/authorization.md', R + '/policy-input.json', D + '/bank-before.json', D + '/catalog-before.json',
  D + '/classification-before.json', D + '/source-catalog.json', D + '/source-catalog-v2.json', D + '/source-catalog-v3.json', R + '/integration-baseline.json',
...
write(out + '/policy.json', {scope: ref(out + '/scope.json'), model: 'gpt-5.6-luna', grading_point_tolerance: 1,
  ...
  reuse_decision: '신규 6사례·18물음만 검토하고 각 사례의 모범·대표 부분·오답을 전체 사례 단위로 통합하여 실측한다. 기존 은행 문항·과거 실측을 새 검증 분모에 넣지 않는다.'});
write(out + '/agent-reviews.json', reviews);
const codeFiles = [...new Set([...EFFICIENT_RUNTIME_FILES, ...['run-efficient-grading.ts', 'contract.ts', 'accounting.ts'].map(file => R + '/' + file)])];
...
for (const [index, set] of target.entries()) {
  ...
  for (const kind of ['model', 'partial', 'wrong']) {
    ...
    entries.push({id: `${unit}--${kind}`, worker: ['a', 'b'][index % 2], learning_unit_id: unit, source_set_id: set.id,
...
const inputFiles = [...new Set([...target.flatMap(set => set.source_refs.map(source => source.file)), ...evidenceFiles,
  R + '/build-execution.mjs', R + '/recorder-provenance.json', bankFile, catalogFile, R + '/changed-sets-v1.json',
