// 관찰 O1–O3 수정 10세트의 운영 DB 반영. fix-v3 driver를 v3 수락본 경로로 옮겼다.
...
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/standard-new-verification-2026-09-14/fix-v4/db-incremental-v1/driver.mjs <mode> [--expected-preparation-sha256 SHA]
...
import {F,W,N,PROJECT,RPC,read,sha,ref,write,guard,buildLocal,buildSql,localProof} from './replace-contract.mjs';
...
if(mode==='--inspect'){
 ...
 const inputs=[...built.completion.files,built.completion.catalog,built.completion.staged_catalog,...[F+'/publication-v1/stage-completion.json',F+'/baseline.json',built.baselineFile,W+'/changes.json',W+'/batch.json',W+'/acceptance-completion.json',N+'/inspection.json',...code,...runtime].map(ref)];guard(inputs);
