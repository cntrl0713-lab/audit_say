// --stage: 격리 stage에서 관찰 O1–O3 수정 10세트를 재검수(verified)·재게시(published)하고 공개본·암호화본·분류 카탈로그·전체 검증·DB 준비 검사를 수행한다.
// --install: stage 결과를 정본에 원자적으로 기록하고 정본 검사를 수행한다.
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/standard-new-verification-2026-09-14/fix-v4/publish.mjs --stage|--install
...
const F='cpa_uploader/analysis/reviews/standard-new-verification-2026-09-14/fix-v4',W=F+'/v3',P=F+'/publication-v1',stage=P+'/stage';
...
const canonical={...publicationPaths(),catalog:path.resolve('cpa_uploader/data/learning-question-classifications.json')},baseline=read(F+'/baseline.json'),accepted=read(W+'/acceptance-completion.json'),changes=read(W+'/changes.json');
...
const ids=changes.changed_sets;assert.equal(ids.length,10);
...
function catalog(bankFile,sourceFile,prefix){const sets=read(bankFile),reviewFile=prefix+'-classification-review.json';const review={source_file:sourceFile,source_file_sha256:ref(bankFile).sha256,entries:read(W+'/classification-review.json').entries};write(reviewFile,review);const topics=read(W+'/evidence-catalog.json').topics,compiled=compileLearningCatalog(sets,review.entries,topics);const file=prefix+'-catalog.json';write(file,{schema_version:1,source_file:sourceFile,source_file_sha256:ref(bankFile).sha256,public_content_hash:contentHash(sets.map(compilePublicQuestionSet)),review_file:reviewFile,review_file_sha256:ref(reviewFile).sha256,topics,classifications:compiled.classifications});return file;}
const evidence=accepted.batch.file,note=F+'/authorization.md; '+F+'/decision-o3-v3.md; '+evidence;
if(mode==='--stage'){
 guard();assert(env.CPA_QUESTION_V3_ENCRYPTION_KEY,'암호화 키 필요');fs.mkdirSync(stage,{recursive:true});
 fs.copyFileSync(W+'/evidence-bank.json',stage+'/authoring.json',fs.constants.COPYFILE_EXCL);fs.copyFileSync(canonical.ledger,stage+'/promotions.json',fs.constants.COPYFILE_EXCL);
 ...
 assert.equal(final,fs.readFileSync(W+'/evidence-bank.json','utf8'),'재검수·재게시 후 원문은 수락한 수정본과 같아야 한다');
 ...
 const added=nextLedger.entries.slice(priorLedger.entries.length);assert.equal(added.length,ids.length*2);
 ...
 write(P+'/install-completion.json',{status:'canonical_installed_and_validated',completed_at:new Date().toISOString(),files:Object.values(canonical).map(f=>ref(path.relative(process.cwd(),f).split(path.sep).join('/'))),changed_sets:ids,counts:done.counts,new_ledger_entries:done.new_ledger_entries,model_api_calls:0,db_applied:false});
}
