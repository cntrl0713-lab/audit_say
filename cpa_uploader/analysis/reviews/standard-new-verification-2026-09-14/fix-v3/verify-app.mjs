// 운영 DB 조회 경로와 운영 앱 /curriculum·/quiz 응답에서 B1·B2 수정 2세트의 7개 학습 단위가 새 릴리스로 그대로 보이는지 확인한다. 쓰기 없음.
// 바뀐 채점기준은 공개 응답에 없으므로 비공개 내용은 독립 검증기(verification.json)의 정본 대조로 확인한다.
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/standard-new-verification-2026-09-14/fix-v3/verify-app.mjs
...
const F='cpa_uploader/analysis/reviews/standard-new-verification-2026-09-14/fix-v3',O=F+'/db-publication-v1';
...
const done=read(O+'/completion.json'),changes=read(F+'/changes.json'),bank=read('cpa_uploader/data/cpa_question_sets_v3.authoring.json'),verification=read(O+'/verification.json');
...
const rows=changes.changed_sets.flatMap(id=>bank.find(s=>s.id===id).subquestions.map(q=>{const uid=learningUnitId(id,'standard',q.id),u=units.find(x=>x.id===uid);assert(u,'학습 단위 없음: '+uid);
 assert.equal(u.subquestions[0].prompt,q.prompt);assert.equal(u.subquestions.length,1);assert.equal(u.shared_context.facts.length,0);assert.equal(u.subquestions[0].max_points,q.criteria.reduce((a,c)=>a+c.max_points,0));
 const json=JSON.stringify(u);for(const k of ['"model_answer"','"criteria"','"requirements"'])assert(!json.includes(k));
 return{id:uid,max_points:u.subquestions[0].max_points};}));
const pages=[];
for(const route of ['/curriculum','/quiz']){
 ...
 pages.push({route,status:response.status,units_present:rows.filter(r=>has(r.id)).length,body_sha256:createHash('sha256').update(html).digest('hex')});
}
for(const p of pages)assert.equal(p.units_present,rows.length,p.route+' 누락');
const out={status:'production_app_verified',checked_at:new Date().toISOString(),release_id:done.release_id,learning_units:units.length,changed_units:rows,pages,public_answer_fields_excluded:true,private_criteria_verified_by:ref(O+'/verification.json'),writes:0,model_api_calls:0,database_completion:ref(O+'/completion.json'),note:'바뀐 채점기준은 공개 응답에 없고 발문은 그대로다. 비공개 채점기준은 독립 검증기가 정본과 대조했다. 브라우저 조작·사용자 답안 제출은 하지 않았다.'};
