// --stage: 격리 stage에서 수정 2세트를 재검수(verified)·재게시(published)하고 공개본·암호화본·분류 카탈로그·전체 검증·DB 준비 검사를 수행한다.
// --install: stage 결과를 정본에 원자적으로 기록하고 정본 검사를 수행한다.
//   node --env-file=.env.local --import tsx cpa_uploader/analysis/reviews/standard-new-verification-2026-09-14/fix-v3/publish.mjs --stage|--install
...
const F='cpa_uploader/analysis/reviews/standard-new-verification-2026-09-14/fix-v3',P=F+'/publication-v1',stage=P+'/stage';
...
const ids=changes.changed_sets;assert.equal(ids.length,2);
...
if(mode==='--stage'){
 ...
 const added=nextLedger.entries.slice(priorLedger.entries.length);assert.equal(added.length,4);
 ...
 write(P+'/install-completion.json',{status:'canonical_installed_and_validated',completed_at:new Date().toISOString(),files:Object.values(canonical).map(f=>ref(path.relative(process.cwd(),f).split(path.sep).join('/'))),changed_sets:ids,counts:done.counts,new_ledger_entries:4,model_api_calls:0,db_applied:false});
}
