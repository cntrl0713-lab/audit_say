...
export const F='cpa_uploader/analysis/reviews/standard-new-verification-2026-09-14/fix-v2';
...
export function buildLocal(){
 ...
 const document=fs.readFileSync(STAGE+'/authoring.json','utf8'),sets=JSON.parse(document),ids=read(F+'/changes.json').changed_sets;
 ...
 const indices=sets.map((s,i)=>ids.includes(s.id)?i:-1).filter(i=>i>=0);assert.equal(indices.length,21);
 ...
 const metadata={applicability:{},...catalog,source_file_hash:readiness.source_file_hash,bank_content_hash:readiness.bank_content_hash,public_content_hash:readiness.public_content_hash,evidence:F+'/batch.json',source_validation:'external-importer',actor_user_id:null};
