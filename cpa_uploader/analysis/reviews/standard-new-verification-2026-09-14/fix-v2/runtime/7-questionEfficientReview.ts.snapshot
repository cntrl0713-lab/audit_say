...
export interface GradingManifest {
    ...
    budget_usd: number | null; budget_enforcement: 'provider_limit' | 'not_specified';
...
export interface EfficientValidationContext {
    ...
    /** Historical generated evidence may resolve only through archives frozen in the same manifest. */
    preservedAuxiliaryInputs?: Map<string, { original: ReviewFile; archived: ReviewFile; collection: ReviewFile }>;
}
...
/** Generated analysis is historical review context, not a live question source.
 * Keep the original receipt and bytes intact when that generated view is rebuilt.
 * Primary bank/policy/runtime/observation/source reads never use this resolver. */
export function readPreservedAuxiliaryInput(identity: ReviewFile, inputs: ReviewFile[],
    context: EfficientValidationContext, root: string, protectedFiles: ReadonlySet<string> = new Set()): string {
    const relative = path.relative(root, path.resolve(root, identity.file));
    const registered = inputs.some(i => i.file === identity.file && i.sha256 === identity.sha256);
    const allowedGeneratedFile = identity.file === 'cpa_uploader/analysis/question-elements/question-elements.json';
    const isProtected = allowedGeneratedFile && [...protectedFiles].some(file => path.resolve(root, file) === path.resolve(root, identity.file));
    if (!registered || !allowedGeneratedFile || isProtected) return readFile(identity, context, root);
    assert(relative && !relative.startsWith('..') && !path.isAbsolute(relative), 'Evidence must stay inside the repository');
    const current = path.resolve(root, identity.file);
    if (fs.existsSync(current) && sha256(fs.readFileSync(current)) === identity.sha256) return readFile(identity, context, root);
    const collections = inputs.filter(i => /^cpa_uploader\/raw\/collections\/[A-Za-z0-9_-]+\/manifest\.json$/u.test(i.file));
    const matches: { archived: ReviewFile; collection: ReviewFile }[] = [];
    for (const collection of collections) {
        const manifest = JSON.parse(readFile(collection, context, root)) as {
            version?: number; mode?: string; entries?: { original_path?: string; sha256?: string; archived_path?: string }[];
        };
        if (manifest.version !== 1 || manifest.mode !== 'byte_preserving_copy' || !Array.isArray(manifest.entries)) continue;
        for (const entry of manifest.entries) {
            if (entry.original_path !== identity.file || entry.sha256 !== identity.sha256) continue;
            assert(typeof entry.archived_path === 'string' && /^cpa_uploader\/raw\/materials\/[A-Za-z0-9_-]+\/[a-f\d]{16}\/[A-Za-z0-9_.-]+$/u.test(entry.archived_path), 'Unsafe preserved evidence path');
            const archived = inputs.find(i => i.file === entry.archived_path && i.sha256 === identity.sha256);
            assert(archived, 'Preserved auxiliary bytes must be frozen in the same manifest');
            matches.push({ archived, collection });
        }
    }
    assert(matches.length, `Changed evidence: ${identity.file}; no manifest-bound preserved auxiliary input`);
    assert(new Set(matches.map(m => m.archived.file)).size === 1, 'Ambiguous preserved auxiliary input');
    const match = matches[0];
    const text = readFile(match.archived, context, root);
    context.preservedAuxiliaryInputs ??= new Map();
    context.preservedAuxiliaryInputs.set(identity.file + ':' + identity.sha256, { original: identity, ...match });
    return text;
}
...
function originalRuntimeSnapshots(originFile: ReviewFile, origin: GradingManifest,
    context: EfficientValidationContext, root: string): EfficientReviewBatch['runtime_snapshots'] {
    const indexPath = (file: ReviewFile) => path.resolve(root, path.dirname(file.file), 'runtime-snapshots.json');
    const sibling = origin.inputs.filter(input => path.resolve(root, input.file) === indexPath(originFile));
    if (sibling.length) {
        assert.equal(sibling.length, 1, 'Exactly one original sibling runtime snapshot index required');
        // A present but invalid sibling must never fall back to another execution.
        return readJson(sibling[0], context, root);
    }

    // An unchanged resumed execution may explicitly inherit an earlier index.
    // Only hash-verified reuse ancestry authorizes that directory; an arbitrary
    // index in inputs, or a snapshot covering only the grading files, is not proof.
    const pending = [...(origin.reused_observations ?? []).map(reuse => reuse.origin_manifest)];
    const visited = new Set<string>(), candidates = new Map<string, EfficientReviewBatch['runtime_snapshots']>();
    while (pending.length) {
        const identity = pending.pop()!;
        const ancestor = readJson<GradingManifest>(identity, context, root);
        const file = path.resolve(root, identity.file);
        if (visited.has(file)) continue;
        visited.add(file);
        assert.equal(ancestor.version, 1); assert.equal(ancestor.artifact_type, 'efficient_grading_manifest');
        const candidatePath = indexPath(identity);
        const inherited = origin.inputs.filter(input => path.resolve(root, input.file) === candidatePath);
        if (inherited.length) {
            assert.equal(inherited.length, 1, 'Exactly one inherited runtime snapshot index identity required');
            const registered = ancestor.inputs.filter(input => path.resolve(root, input.file) === candidatePath);
            assert.equal(registered.length, 1, 'Inherited runtime snapshot index missing from ancestor manifest');
            same(inherited[0], registered[0], 'Inherited runtime snapshot index identity differs');
            const snapshots = readJson<EfficientReviewBatch['runtime_snapshots']>(inherited[0], context, root);
            assert(Array.isArray(snapshots), 'Original execution runtime snapshot index must be an array');
            if (snapshots.length === origin.code_files.length && origin.code_files.every(code => {
                const matches = snapshots.filter(snapshot => snapshot.runtime_file === code.file);
                return matches.length === 1 && matches[0].sha256 === code.sha256;
            })) candidates.set(candidatePath, snapshots);
        }
        pending.push(...(ancestor.reused_observations ?? []).map(reuse => reuse.origin_manifest));
    }
    assert.equal(candidates.size, 1, 'Exactly one linked ancestor runtime snapshot index must match all original code identities');
...
export function validateReusableEfficientObservation(reuse: ReusedGradingObservation, manifest: GradingManifest,
    ...
    if (!origin) {
        ...
        const snapshots = originalRuntimeSnapshots(reuse.origin_manifest, origin, context, root);
...
function validateBatch(file: ReviewFile, context: EfficientValidationContext, root: string): ValidatedBatch {
    ...
    // The first batch used a $20 provider limit; later batches must record their
    // actual policy instead of claiming that historical limit was configured.
    if (manifest.budget_enforcement === 'not_specified') assert.equal(manifest.budget_usd, null, 'Unspecified budget must be null');
    else {
        assert.equal(manifest.budget_enforcement, 'provider_limit', 'Unknown budget enforcement');
        assert(typeof manifest.budget_usd === 'number' && Number.isFinite(manifest.budget_usd) && manifest.budget_usd > 0,
            'Provider budget must be a positive amount');
    }
    ...
    const bank = readJson<QuestionSetV3[]>(manifest.bank, context, root);
    const protectedFiles = new Set([manifest.bank.file, manifest.classifications.file, manifest.policy.file,
        file.file, batch.authorization.evidence.file, batch.grading_manifest.file,
        ...manifest.code_files.map(c => c.file), ...batch.runtime_snapshots.map(c => c.file),
        ...batch.observations.map(o => o.file), ...bank.flatMap(s => s.source_refs.map(r => r.file))]);
    for (const input of manifest.inputs) readPreservedAuxiliaryInput(input, manifest.inputs, context, root, protectedFiles);
    ...
    for (const review of batch.agent_reviews) {
        ...
        assert(review.evidence.length > 0); for (const input of review.evidence) readPreservedAuxiliaryInput(input, manifest.inputs, context, root, protectedFiles);
