...
export async function main(argv=process.argv.slice(2)):Promise<void>{
 ...
 const candidates=rows.filter(row=>row.unit_id===args['--unit-id']&&!row.error&&row.response!==null&&row.response!==undefined&&[1,2].includes(row.attempt));
 if(candidates.length!==1)throw Error('Exactly one original successful response is required; no selection among different valid responses');
 ...
 const schema=reviewChunkSchema(unit),baseInstructions=extractProductionInstructions(fs.readFileSync(reviewFile,'utf8'));
 const previous=prior.attempt===2?rows.slice(0,rows.indexOf(prior)).reverse().find(row=>row.unit_id===prior.unit_id&&row.attempt===1):undefined;
 if(prior.attempt===2&&!previous?.error)throw Error('Original successful retry lacks its immediately preceding validation error');
 const instructions=baseInstructions+(prior.attempt===2?' 직전 응답 검증 오류를 고치되 판정을 임의로 pass로 바꾸지 말라: '+previous!.error:'');
 ...
 const record:Record<string,unknown>={started_at:new Date().toISOString(),mode:execute?'one_actual_model_request':'local_preflight',set_id:set.id,plan_id:entry.plan_id,unit_id:unit.id,model,transport:execute?'model':'none',mock:false,max_input_chars:budget,
  ...
  original_log:{file:args['--chunks'],sha256:fileHash(args['--chunks']),line:rows.indexOf(prior)+1,attempt:prior.attempt,response_hash:jsonHash(prior.response),runtime_file:runtimeFile,runtime_hash:fileHash(runtimeFile),preceding_error:previous?.error||null,preceding_error_row_hash:previous?jsonHash(previous):null},
  instructions,base_instructions_hash:hash(baseInstructions),
  ...
  purpose:'Repeat the sole valid original semantic judgment with its exact attempt-specific production instructions, fixed model, input/schema, full bank, source and code. The original instruction hash must match after reconstructing any preceding validation-error instruction. Valid nonpass findings remain unchanged; one diagnostic call, no retry, injection, or receipt promotion.'};
