...
const [manifestFile,lockFile,label,...excluded]=process.argv.slice(2);
...
const runner=path.join(root,'resume-semantic.ts');
...
const fixed=[{file:manifestFile,sha256:lock.manifest_sha256},lock.comparison_bank,...lock.code_files,...lock.source_files,
 ...[lockFile,runner,process.argv[1]].map(file=>({file,sha256:hash(file)}))];
...
try{
 for(const entry of manifest.entries.filter(entry=>!excluded.includes(entry.plan_id))){
  ...
  for(let attempt=1;attempt<=2;attempt++){
   ...
   if(summary.status==='receipt_created')break;
