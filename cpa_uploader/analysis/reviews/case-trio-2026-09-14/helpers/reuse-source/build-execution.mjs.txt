...
write(out + '/policy.json', {scope: ref(out + '/scope.json'), model: 'gpt-5.6-luna', grading_point_tolerance: 1,
  ...
  reuse_decision: '신규 6사례·18물음만 검토하고 각 사례의 모범·적용 가능한 대표 부분·오답을 전체 사례 단위로 통합하여 실측한다. 기존 은행 문항·과거 실측을 새 검증 분모에 넣지 않는다.'});
...
const inputFiles = [...new Set([...target.flatMap(set => set.source_refs.map(source => source.file)), ...evidenceFiles,
  R + '/helpers/build-execution.mjs', R + '/helpers/representative-qa.mjs', R + '/helpers/provenance.json', R + '/helpers/provenance-v2.json', bankFile, catalogFile, R + '/changed-sets-v1.json',
