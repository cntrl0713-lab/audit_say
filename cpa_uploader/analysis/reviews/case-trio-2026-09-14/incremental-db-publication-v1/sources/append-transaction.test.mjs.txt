...
const added=Array.from({length:6},(_,i)=>({id:`new-${i+1}`}));
...
begin
 ...
 if v_mode='missing-addition' then delete from public.cpa_question_bank_release_items where release_id='${newId}' and set_id='new-6'; end if;
...
test('generated transaction commits the exact source append and preserves existing versions/classifications',async()=>{
 ...
 try{
  ...
  assert.equal(receipt.release_id,newId);assert.equal(receipt.set_count,8);
