...
const P=N+'/preparation-v1',O=R+'/db-publication-v2';
...
const preserved=sourceProvenance.files.map(row=>({file:row.preserved_file,sha256:row.sha256}));guard(preserved);
...
if(mode==='--prepare'){
 ...
 const inputs=[...built.completion.files,...[R+'/publication-v2/stage-completion.json',R+'/integration-baseline-v2.json',built.baselineFile,R+'/integration-baseline-v2/catalog.json',R+'/changed-sets-v1.json',R+'/sealed-v1/batch.json',R+'/sealed-v1/readiness.json',R+'/concurrent-reuse-validation-v2.json',N+'/source-provenance.json',...code,...runtime].map(ref),...preserved];guard(inputs);
 ...
 const installed=read(R+'/publication-v2/install-completion.json');assert.equal(installed.status,'canonical_installed_and_validated');guard(installed.files);
 ...
 const inputs=[...prep.inputs,prep.pack,prep.sql,prep.proof,prep.readiness,ref(P+'/preparation.json'),ref(R+'/publication-v2/install-completion.json'),...installed.files];guard(inputs);
