...
const D='cpa_uploader/analysis/reviews/case-deepening-2026-09-14';
...
const ids=read(D+'/changed-sets-v1.json'),candidate=read(D+'/candidate-v2.json');
const baseline=read(D+'/integration-baseline-v2/bank.json');
assert.equal(ids.length,6);assert.equal(new Set(ids).size,ids.length);
assert(ids.every(id=>!baseline.some(set=>set.id===id)&&candidate.some(set=>set.id===id)),'Target must be a new ID in the reviewed candidate');
assert.equal(candidate.length,baseline.length+ids.length);
for(const old of baseline)assert.deepEqual(candidate.find(set=>set.id===old.id),old,'Unrelated baseline content changed');
...
if(mode==='--stage'){
 assert(process.execArgv.includes('--env-file=.env.local'),'Invoke stage with node --env-file=.env.local --import tsx');
 assert(process.env.CPA_QUESTION_V3_ENCRYPTION_KEY,'Encryption key must be loaded before preparing publication; its value is never recorded');
 assert(!fs.existsSync(out));assert.equal(ref(canonical.authoring).sha256,read(D+'/integration-baseline-v2.json').bank.sha256,'Canonical changed during review');
 assert.equal(ref(canonical.catalog).sha256,read(D+'/integration-baseline-v2.json').catalog.sha256,'Canonical classifications changed during review');
 ...
 assert.equal(originals.find(r=>r.name==='authoring').sha256,read(D+'/integration-baseline-v2.json').bank.sha256,'Canonical changed while creating backups');
 assert.equal(originals.find(r=>r.name==='catalog').sha256,read(D+'/integration-baseline-v2.json').catalog.sha256,'Catalog changed while creating backups');
 for(const r of originals)assert.equal(ref(r.backup).sha256,r.sha256,'Backup changed while copying: '+r.file);
 const immutable=[D+'/candidate-v2.json',D+'/classification-v2.json',D+'/changed-sets-v1.json',D+'/integration-baseline-v2.json',D+'/integration-baseline-v2/bank.json',D+'/sealed-v1/batch.json',D+'/sealed-v1/readiness.json'].map(ref);
 ...
 fs.copyFileSync(D+'/candidate-v2.json',stage+'/authoring.json');fs.copyFileSync(canonical.ledger,stage+'/promotions.json');
 const newIds=ids;
 ...
 run('05-catalog',[C+'/rebind-final-catalog.ts','--bank',stage+'/authoring.json','--review',D+'/classification-v2.json','--output',stage+'/classification-review.json','--catalog-output',stage+'/catalog.json'],stagedEnv,guard);
 ...
 withPublicationLock(canonical.authoring,()=>{
  ...
  try{
   ...
   run('08-canonical-catalog',[C+'/rebind-final-catalog.ts','--bank',canonical.authoring,'--review',D+'/classification-v2.json','--output',out+'/canonical-classification-review.json','--catalog-output',out+'/canonical-catalog.json'],env,guard);
