...
export const R='cpa_uploader/analysis/reviews/case-deepening-2026-09-14';
export const N=R+'/incremental-db-publication-v2',STAGE=R+'/publication-v2/stage';
...
export function buildLocal(){
 const completion=read(R+'/publication-v2/stage-completion.json');assert.equal(completion.status,'validated_isolated_published_stage');guard(completion.files);
 const baselineFile=R+'/integration-baseline-v2/bank.json',baselineDocument=fs.readFileSync(baselineFile,'utf8'),baseline=JSON.parse(baselineDocument);
 ...
 assert.equal(ids.length,6);assert.equal(new Set(ids).size,6);assert.equal(added.length,6);
 assert.deepEqual(sets.slice(0,baseline.length),baseline,'Existing baseline objects or order changed');assert.deepEqual(added.map(s=>s.id),ids,'Only the six reviewed IDs may be appended');
 ...
 assert.equal(read(R+'/integration-baseline-v2.json').bank.sha256,sha(baselineDocument));assert.equal(read(R+'/sealed-v1/readiness.json').ready,true);
