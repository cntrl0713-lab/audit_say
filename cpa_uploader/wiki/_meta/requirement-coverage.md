...
[[source-catalog]]의 고유 원자료 단위 7852개를 먼저 탐색한다. 주제별 카탈로그는 기준서·이론·문제연습·기출의 실제 파일·위치·판본·은행 인용 연결을 표시한다. 직접 인용 연결이 없는 단위에서 목표 후보를 찾되, 다른 표현·파일로 출제된 기존 목표가 있는지 실제 발문과 대조한다. [[source-authoring-design]]의 두 경로와 계획서로 목표·조건·예외·답안 범위·기존 차이를 명시한다.
...
| [[source-catalog-topic-04]] | 1344 | 32 | 1312 |
...
| [[source-catalog-topic-16]] | 704 | 44 | 660 |
...
정본 requirement 1444개와 criterion 1882개의 직접 대응은 각 [[topic-map]] → 세트 색인의 ‘학습목표·채점명제와 핵심 조건’ 및 ‘요구사항과 직접 근거’ 표에서 찾는다. ID는 세트·물음 안에서 해석한다. 실제 발문과 조건을 함께 보고 동일 명제 반복과 범위 누락을 검토한다.
...
- 탐색 절 189개: 공백 후보 0, 낮은 유사도 37, 유사 문구 탐지 152
