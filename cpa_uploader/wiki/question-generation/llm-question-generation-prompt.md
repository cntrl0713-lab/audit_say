...
생성 응답의 출처 ID·파일·위치·인용·해시는 제공된 패킷과 일치해야 한다. 생성 뒤 구조·원문 검증과 선택한 경로의 의미검토·실제 채점 검증을 거치며 생성기가 검수 완료로 상태를 올리지 않는다.

현재 기본 경로는 [공통 비용 통제 계약](../../../.agents/skills/audit-question-review/references/cost-controlled-verification.md)에 따른 agent의 전수 내용·출처·배점 검토와 실제 Luna 대표 채점이며, 사용자 승인 범위에서 `--efficient-review` 증거로 승급한다. agent 검토는 사람의 직접 확인과 구분한다. 95%·±1점은 채점 일관성에만 적용하고 기반 자료·모범답안·배점·QA 기대값의 미해결 오류는 허용하지 않는다.

**기준별 전수검사 경로를 선택한 경우**에는 기존의 별도 의미검수·실제 사례 채점·점수 재현과 `--review` receipt 계약을 따른다. 이 경로에서 의미검수만 pass이고 grading.status=not_run인 receipt로는 승급할 수 없으며, 신규 verified 승급에는 검증을 통과한 receipt와 실제 사람 검수 근거가 모두 필요하다.

이 템플릿은 [확정 수정 정책](../../../docs/plans/주제-01-03-검토에-따른-수정-결정.md)을 반영한다. 실행 생성기와 채점 코드의 지원 여부는 별도 확인한다. 적용 연도·판본을 입력에 포함하고 2027년 동일 적용 가정을 다른 연도로 확장하지 않는다.
