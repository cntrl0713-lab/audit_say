...
- 문제 세트 목록은 OX 학습 순서의 주제별 표로 묶고, 각 행의 왼쪽 세트부터 오른쪽 세트 순서로 읽는다. 모든 세트의 직접 링크와 전체 제목을 유지하며 주제 내 정본 배열 순서를 바꾸지 않는다.
- `log.md`: 위키 생성·갱신 이력. 400줄 제한에 가까워지면 가장 오래된 기록을 문구·순서를 바꾸지 않고 `log-archive-<시작일>-to-<종료일>.md` 보관본으로 옮기고 log.md 첫머리에서 wikilink로 연결한다. 보관본은 기록의 상대 링크가 그대로 유효하도록 log.md와 같은 폴더에 두며, log.md처럼 frontmatter·index·콘텐츠 페이지 수 대상이 아니지만 줄 수 제한·링크 검사와 log.md 연결 검사를 받는다.
...
- 새 `linked_question_set` 출력은 같은 학습 유형으로 작성하고 유형별 물음 수·사례 분량은 [학습 단위 계약](../../docs/물음별-학습-단위와-분류-계약.md#새-초안과-학습-화면)을 따른다. 사례형은 부모 사실이 필요하고 사례 지문 아래에는 사례형 물음만 배치한다. 기준서형은 `shared_context.facts=[]`이며 학습·제출은 한 물음씩 독립적으로 구성한다.
...
- 빌드는 생성 페이지를 check와 같은 기준(CRLF/LF·생성 날짜 차이 무시)으로 기존 파일과 비교해 신규이거나 내용이 바뀐 페이지만 쓴다. 생성 날짜만 다른 페이지는 기존 바이트와 날짜를 유지하므로 따로 되돌리지 않으며, `log.md` 자동 기록에 실제로 쓴 페이지 수가 남는다.
- 빌드는 `log.md`에 자동 기록하고, lint·check의 실제 결과는 작업자가 기록한다. 자동 기록의 변경 없음 표현이 실제 변경 범위(원자료 추가, 정본 변경 등)와 다르면 자동 기록을 고치지 말고 작업자 기록으로 실제 범위를 덧붙인다. 과거 로그는 연결된 보관본에 유지한다.
