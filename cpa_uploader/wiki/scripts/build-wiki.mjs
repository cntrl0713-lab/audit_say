...
export function buildWiki({ repoDir = path.resolve(scriptDir, '../../..'), date = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  // Group the complete directory by study topic; read each row left to right.
  // Every set keeps its direct link and full title without a one-line-per-set index.
  const questionIndex = table(['주제', '문제 세트', '문제 세트'], studyTopics.flatMap((topic) => {
    const sets = orderedBank.filter((set) => set.classification.topic_id === topic.id);
    const label = (set) => set ? `[[${set.id}]] — ${inline(set.title)}` : '';
    return Array.from({ length: Math.ceil(sets.length / 2) }, (_, index) => [
      index === 0 ? `${topic.id}. [[${topic.slug}]]` : '', label(sets[index * 2]), label(sets[index * 2 + 1]),
    ]);
  }));
