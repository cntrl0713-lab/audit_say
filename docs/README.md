...
사례형 갱신: [수정 요청서](case-question-edit-notes-2026-09-14.md)(문제별 지시와 반영 현황) · [병합 종합문제 설계](사례형-병합-종합문제-설계.md)(남은 대상의 묶음 후보와 설계 방침).

공식 출처의 등록 범위: [공식 전문 등록 결손 조사](reports/공식-전문-등록-결손-조사-2026-09-21.md). `cpa_uploader/data/official/`이 기준서별로 어느 문단을 담고 있는지와 끊기는 구간, 그 결손이 의도된 발췌인지, 다음 제작 후보가 어디서 막히는지를 2026-09-21에 대조한 기록이다. 보강은 기존 파일 수정이 아니라 새 등록 단위로 처리한다.
...
2026-09-14 정리: 작업이 끝난 계획 전달 문서 20개를 저장소에서 지웠다. 2026-09-11 서브에이전트 배정서 16개, 대체된 KICPA 채용공고 알림 원안 계획서, 끝난 문항 전수검증 재개 요구서 3개다. 과거 보고서·검토 기록의 해당 링크는 당시 기록으로 두며, 내용은 커밋 `868e6471`까지의 git 기록에서 볼 수 있다.
