...
for(const [name,data]of Object.entries({'13-cases':cases,'13-offline':{offline,boundaries,deployment},'13-public':sets.map(compilePublicQuestionSet),'13-run-metadata':metadata}))fs.writeFileSync(`${dir}/${name}.json`,JSON.stringify(data,null,2)+'\n');console.log(JSON.stringify(metadata));
if(process.argv.includes('--live')){const out=`${dir}/13-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
