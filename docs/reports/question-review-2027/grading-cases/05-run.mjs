...
for(const s of bank)for(const variant of ['full','paraphrase','omission','contrary','empty']){
 ...
 vs.push(q.criteria.map((c,i)=>variant==='empty'||variant==='omission'&&i===0?n:variant==='contrary'?x(parts[i]):m(parts[i])));}
...
fs.writeFileSync(`${dir}/05-cases.json`,JSON.stringify(cases,null,2)+'\n');fs.writeFileSync(`${dir}/05-public.json`,JSON.stringify(bank.map(compilePublicQuestionSet),null,2)+'\n');fs.writeFileSync(`${dir}/05-offline.json`,JSON.stringify({offline,boundaries},null,2)+'\n');
...
fs.writeFileSync(`${dir}/05-run-metadata.json`,JSON.stringify(meta,null,2)+'\n');console.log(JSON.stringify(meta));
if(process.argv.includes('--live')){const out=`${dir}/05-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').split('\n').filter(Boolean).map(JSON.parse):[];
