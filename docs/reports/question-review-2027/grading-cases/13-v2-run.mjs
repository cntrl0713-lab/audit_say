// Internal synthetic review. node --env-file=.env.local .../13-run.mjs [--live]
...
const bank=JSON.parse(fs.readFileSync(file)),sets=bank.filter(s=>s.id.startsWith('pilot-13'));
const inputs=JSON.parse(fs.readFileSync(`${dir}/13-inputs.json`,'utf8').replace(/^\uFEFF/,''));
...
function add(s,variant,answers,vs,id,live=true){const expected={subquestions:s.subquestions.map((q,i)=>({subquestion_id:q.id,verdicts:q.criteria.map((c,j)=>({criterion_id:c.id,...vs[i][j]}))}))};cases.push({id:id||`${s.id}-${variant}`,set_id:s.id,variant,answers,expected,expected_score:expected.subquestions.flatMap(q=>q.verdicts).filter(v=>v.verdict==='met').length,live,basis:'13-inputs.json + reviewed KGA610/620 criterion contract; independent expectations before execution'});}
...
edge('pilot-13-003','sub1','13-wrong-signatories','내부감사인으로부터 외부감사인의 지시를 따른다는 서면동의를 받는다. 내부감사인으로부터 기업이 보조업무에 관여하지 않는다는 서면동의를 받는다. 기업 대표자로부터 내부감사인이 특정 기밀을 유지한다는 서면동의를 받는다. 기업 대표자로부터 내부감사인의 객관성 위협을 외부감사인에게 알린다는 서면동의를 받는다.',[not,not,not,not,not]);
edge('pilot-13-002','sub1','13-expert-evaluation-swapped','감사인측 전문가의 적격성, 역량 및 객관성을 평가한다.',[not,not,not]);
edge('pilot-13-004','sub2','13-manager-expert-swapped','경영진측 전문가의 업무가 재무제표 작성목적에 적합한지 평가한다.',[not]);
edge('pilot-13-005','sub2','13-only-direct-increase','외부감사인의 직접 수행업무를 늘린다.',[not]);
edge('pilot-13-003','sub1','13-only-domestic-prohibition','우리나라 회계감사기준에 따른 감사와 그룹감사의 부문감사에서는 직접적 보조를 활용할 수 없다.',[not,not,not,not,met('우리나라 회계감사기준에 따른 감사와 그룹감사의 부문감사에서는 직접적 보조를 활용할 수 없다.')]);
edge('pilot-13-001','sub1','13-both-responsibilities-one-sentence','두 경우 모두 감사의견에 대한 전적인 책임은 외부감사인에게 그대로 남는다.',[met('두 경우 모두 감사의견에 대한 전적인 책임은 외부감사인에게 그대로 남는다.'),met('두 경우 모두 감사의견에 대한 전적인 책임은 외부감사인에게 그대로 남는다.')]);
// Explicit signatory and object substitution contradict the revised contract; omissions still use not_met.
for(const c of cases.filter(c=>['13-wrong-signatories','13-manager-expert-swapped'].includes(c.id)))for(const q of c.expected.subquestions)if(c.answers[q.subquestion_id])for(const v of q.verdicts)if(v.criterion_id!=='crit7'){v.verdict='contradicted';v.quote=c.answers[q.subquestion_id];}
// A reason that explicitly says the necessary use condition was not met implies non-use.
for(const c of cases.filter(c=>c.set_id==='pilot-13-005'&&(c.id.endsWith('-omission')||c.id.endsWith('sub1-omit-crit1')))){
 const v=c.expected.subquestions.find(q=>q.subquestion_id==='sub1').verdicts[0];v.verdict='met';v.quote=c.answers.sub1;c.expected_score++;
 c.basis+='; the surviving reason explicitly establishes failure of a necessary use condition, so non-use is implied under the accepted policy';
}
...
const hashes=Object.fromEntries([file,'lib/questionV3.ts','lib/questionV3Grading.ts','lib/ai/openaiStructured.ts',`${dir}/13-inputs.json`].map(p=>[p,sha(p)])),model=process.env.CPA_GRADING_MODEL||'gpt-5.6-luna';
...
const live=cases.filter(c=>c.live&&c.variant!=='empty'&&['pilot-13-003','pilot-13-004','pilot-13-005'].includes(c.set_id));
...
for(const [name,data]of Object.entries({'13-v2-cases':cases,'13-v2-offline':{offline,boundaries,deployment},'13-v2-public':sets.map(compilePublicQuestionSet),'13-v2-run-metadata':metadata}))fs.writeFileSync(`${dir}/${name}.json`,JSON.stringify(data,null,2)+'\n');console.log(JSON.stringify(metadata));
if(process.argv.includes('--live')){const out=`${dir}/13-v2-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
...
for(const c of live.filter(c=>['pilot-13-005-omission','pilot-13-005-sub1-omit-crit1','13-wrong-signatories','13-manager-expert-swapped'].includes(c.id)))for(const a of [2,3])await run(c,a);
