...
const sets=JSON.parse(fs.readFileSync(`${dir}/17-public.json`));
const original=JSON.parse(fs.readFileSync(`${dir}/17-offline.json`)).offline;const updated=JSON.parse(fs.readFileSync(`${dir}/17-v4-offline.json`)).offline;const latest=JSON.parse(fs.readFileSync(`${dir}/17-v5-offline.json`)).offline;const offline=[...latest,...updated.filter(r=>r.result.question_set_id!=='pilot-17-002'),...original.filter(r=>!['pilot-17-001','pilot-17-002'].includes(r.result.question_set_id))];
...
try{
  ...
  fs.writeFileSync(`${dir}/17-ui-results.json`,JSON.stringify({scope:'actual QuizClient + actual globals.css; auth and server action mocked; no live auth/database E2E',records},null,2)+'\n');
