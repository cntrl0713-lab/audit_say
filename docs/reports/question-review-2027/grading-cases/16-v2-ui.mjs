...
const sets=JSON.parse(fs.readFileSync(`${dir}/16-v2-public.json`));
const offline=JSON.parse(fs.readFileSync(`${dir}/16-v2-offline.json`)).offline;
...
try{
  ...
  for(const width of [1440,390]){
    ...
    for(const s of sets){
      ...
      const maxLength=await page.locator('#sub1-answer').getAttribute('maxlength');assert.equal(maxLength,'5000');
      for(const q of s.subquestions)await page.locator(`#${q.id}-answer`).fill(`${s.id}/${q.id}\n합성 답안`);
      const overflow=await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth);
      assert.equal(overflow,false);
      await page.screenshot({path:`${dir}/${s.id}-${width}-solving-final.png`,fullPage:true});
      ...
      assert.equal(submitted.id,s.id);assert.deepEqual(Object.keys(submitted.answers),['sub1','sub2']);
      ...
      await page.screenshot({path:`${dir}/${s.id}-${width}-review-final.png`,fullPage:true});
      ...
      assert.ok((await page.locator('body').innerText()).includes('채점 완료'));
      await page.evaluate(()=>window.failRefresh=false);
      await page.getByRole('button',{name:'프로필 다시 불러오기'}).click();
      await page.getByRole('button',{name:'프로필 다시 불러오기'}).waitFor({state:'detached'});
      const refreshLosesResult=!(await page.locator('body').innerText()).includes('채점 완료');
      records.push({set_id:s.id,width,prompts:true,result_ownership:true,model_answers:true,horizontal_overflow:false,retry_reset:true,service_failure_preserves_answers:true,blank_html_submission_blocked:blankBlocked,profile_refresh_failure_returns_to_solving:refreshLosesResult,textarea_maxlength:maxLength});
    }
    await page.close();
  }
  console.log(JSON.stringify(records));
}finally{
  fs.writeFileSync(`${dir}/16-v2-ui-results.json`,JSON.stringify({scope:'actual QuizClient + actual globals.css; auth and server action mocked; no live auth/database E2E',records},null,2)+'\n');
