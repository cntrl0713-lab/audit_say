...
const set=JSON.parse(fs.readFileSync('cpa_uploader/data/cpa_question_sets_v3.authoring.json')).find(s=>s.id==='pilot-14-002');
...
const out=`${dir}/14-followup-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
...
fs.writeFileSync(`${dir}/14-followup-contract.json`,JSON.stringify(meta,null,2)+'\n');
const selected=process.argv.includes('--probe')?cases.filter(c=>c.id==='14-wrong-report-direction'||c.variant==='followup-boundary'):cases;
