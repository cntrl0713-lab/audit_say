...
const sets=JSON.parse(fs.readFileSync(`${dir}/14-public.json`));
const offline=JSON.parse(fs.readFileSync(`${dir}/14-offline.json`)).offline;
...
try{
  ...
  for(const width of [1440,390]){
    ...
    for(const s of sets){
      ...
      const maxLength=await page.locator('#sub1-answer').getAttribute('maxlength');
  ...
  fs.writeFileSync(`${dir}/14-ui-results.json`,JSON.stringify({scope:'actual QuizClient + actual globals.css; auth and server action mocked; no live auth/database E2E',records},null,2)+'\n');
