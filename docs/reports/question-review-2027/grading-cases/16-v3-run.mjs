// Internal synthetic review. node --env-file=.env.local .../16-run.mjs [--live]
...
const inputs=JSON.parse(fs.readFileSync(`${dir}/16-inputs.json`,'utf8').replace(/^\uFEFF/,''));
for(const [key,value] of Object.entries(inputs))if(value.reuse)inputs[key]=structuredClone(inputs[value.reuse]);
...
function add(s,variant,answers,vs,id,live=true){const expected={subquestions:s.subquestions.map((q,i)=>({subquestion_id:q.id,verdicts:q.criteria.map((c,j)=>({criterion_id:c.id,...vs[i][j]}))}))};cases.push({id:id||`${s.id}-${variant}`,set_id:s.id,variant,answers,expected,expected_score:expected.subquestions.flatMap(q=>q.verdicts).filter(v=>v.verdict==='met').length,live,basis:'16-inputs.json + reviewed KGA701/706/710/720 criterion contract; independent expectations before execution'});}
...
function e(s,q,id,answer,verdicts){edge('pilot-16-'+s,q,'16-'+id,answer,verdicts.map(v=>v==='met'?met(answer):v==='contradicted'?contra(answer):not));}
e('003','sub1','other-information-no-comparison','기타정보와 재무제표 간 중요한 불일치를 고려한다.',['not_met','not_met','not_met']);
e('003','sub1','other-information-wrong-paragraph','기타사항문단은 재무제표에 공시되지 않은 사항으로 감사와 감사인의 책임 및 감사보고서의 이해에 관련된다.',['not_met','not_met','not_met']);
e('003','sub2','refusal-no-correction-request','경영진이 수정 거부 시 지배기구와 그 사실을 커뮤니케이션한다.',['not_met','not_met','not_met']);
e('003','sub2','agreement-only','경영진이 수정에 동의했다는 사실만 확인한다.',['not_met','not_met','not_met']);
e('004','sub1','names-only','강조사항문단, 기타사항문단',['not_met','not_met','not_met','not_met']);
e('004','sub2','emphasis-no-location','재무제표에 표시되거나 공시된 정보만 언급한다.',['not_met','not_met','not_met','not_met']);
e('004','sub2','emphasis-location-only','강조할 사항과 그 상세 내용이 있는 재무제표 관련 공시의 위치를 명확히 언급한다.',['not_met','not_met','not_met','not_met']);
e('004','sub2','emphasis-other-qualified','다른 사항으로 한정의견을 표명하더라도 강조된 해당 사항 때문에 감사의견이 변형되는 것은 아님을 나타낸다.',['not_met','not_met','not_met','met']);
e('004','sub2','emphasis-always-unmodified','강조사항문단을 넣는 감사보고서는 다른 변형의견 사유가 있어도 반드시 적정의견이어야 한다.',['not_met','not_met','not_met','contradicted']);
e('005','sub2','intro-separate-opinion-only','핵심감사사항에 대해 별도의 의견을 제공하지 않는다는 점을 도입부에 기술한다.',['not_met','not_met']);
e('005','sub2','intro-whole-only','해당 사항은 전체 재무제표의 감사 관점에서 전체 감사의견 형성 시 다루어졌다고 도입부에 기술한다.',['not_met','not_met']);
e('006','sub1','kam-reference-valid','변형의견 사항은 핵심감사사항 단락에 개별 기술할 수 없다. 다만 그 성격상 핵심감사사항이며 한정의견근거 단락에서 보고하고 핵심감사사항 단락에는 해당 근거 단락을 참조한다.',['met']);
e('006','sub1','kam-replaces-modified','한정의견 표명 대신 핵심감사사항 단락에 이 사항을 기술하면 된다.',['contradicted']);
e('006','sub1','kam-reference-banned','변형의견 사항은 본질적으로 핵심감사사항이 아니므로 그 단락에서는 의견근거 단락을 참조하는 것까지 금지된다.',['contradicted']);
e('007','sub2','intro-most-only','핵심감사사항은 감사인의 전문가적 판단으로 당기 재무제표감사에서 가장 유의적인 사항이라는 점을 도입부에 기술한다.',['not_met','not_met']);
e('007','sub2','intro007-whole-only','핵심감사사항을 재무제표 전체 감사의 관점에서 전체 감사의견 형성 시 다루었다고 기술한다.',['not_met','not_met']);
e('007','sub1','risk-repeat','높게 평가된 중요왜곡표시위험 분야. 유의적 위험으로 식별된 분야. 높은 중요왜곡표시위험.',['met','not_met']);
e('002','sub1','management-without-auditor-judgment','추정불확실성이 높은 회계추정치를 포함하여 경영진의 유의적인 판단이 수반된 재무제표 분야를 고려한다.',['not_met','not_met','not_met']);
e('005','sub1','management005-without-auditor-judgment','추정불확실성이 높은 회계추정치를 포함하여 경영진의 유의적인 판단이 수반된 재무제표 분야를 고려한다.',['not_met','not_met','not_met']);
assert.equal(new Set(cases.map(c=>c.id)).size,cases.length);
for(const c of cases)c.live=c.id==='16-intro007-whole-only';
...
for(const kind of ['invented','cross-question']){const j=structuredClone(c.expected);j.subquestions[0].verdicts[0].quote=kind==='invented'?'답안에 전혀 없는 허위 인용':c.answers.sub2;const r=applyQuestionSetJudgment(s,c.answers,j);assert.equal(r.score,c.expected_score-1);boundaries.push({kind,set:s.id,passed:true});}}
...
const hashes=Object.fromEntries([file,'lib/questionV3.ts','lib/questionV3Grading.ts','lib/ai/openaiStructured.ts',`${dir}/16-inputs.json`].map(p=>[p,sha(p)])),model=process.env.CPA_GRADING_MODEL||'gpt-5.6-luna';
...
const metadata={at:new Date().toISOString(),fingerprint,model,hashes,base_question_answer_combinations:70,cases:cases.length,initial_live_calls:live.length,estimated_input_tokens:Math.ceil(live.reduce((n,c)=>n+(prompts[c.id]?.input.length||0),0)/1.5),max_output_tokens_per_call:8000,estimate_note:'Korean characters/1.5 approximation; every first-run mismatch or error repeated twice',deployment,offline_mismatches:offline.filter(r=>r.differences.length).map(r=>r.id)};
for(const [name,data]of Object.entries({'16-v3-cases':cases,'16-v3-offline':{offline,boundaries,deployment},'16-v3-public':sets.map(compilePublicQuestionSet),'16-v3-run-metadata':metadata}))fs.writeFileSync(`${dir}/${name}.json`,JSON.stringify(data,null,2)+'\n');console.log(JSON.stringify(metadata));
if(process.argv.includes('--live')){const out=`${dir}/16-v3-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
...
const repeat=live.filter(c=>done.some(r=>r.id===c.id&&r.attempt===1&&r.fingerprint===fingerprint&&(r.error||r.differences?.length||r.raw_differences?.length||c.id.includes('without-auditor-judgment'))));for(let i=0;i<repeat.length;i+=3)await Promise.all(repeat.slice(i,i+3).map(async c=>{for(const a of [2,3])await run(c,a);}));}
