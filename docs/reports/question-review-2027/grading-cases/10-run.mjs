// Internal synthetic review. node --env-file=.env.local .../10-run.mjs [--live]
...
const bank=JSON.parse(fs.readFileSync(file)),sets=bank.filter(s=>s.id.startsWith('pilot-10'));
const inputs=JSON.parse(fs.readFileSync(`${dir}/10-inputs.json`,'utf8').replace(/^\uFEFF/,''));
...
function add(s,variant,answers,vs,id,live=true){const expected={subquestions:s.subquestions.map((q,i)=>({subquestion_id:q.id,verdicts:q.criteria.map((c,j)=>({criterion_id:c.id,...vs[i][j]}))}))};cases.push({id:id||`${s.id}-${variant}`,set_id:s.id,variant,answers,expected,expected_score:expected.subquestions.flatMap(q=>q.verdicts).filter(v=>v.verdict==='met').length,live,basis:'10-inputs.json + reviewed KGA520/530 criterion contract; independent expectations before execution'});}
...
for(const v of ['full','paraphrase','omission','contrary','empty']){const answers={},vs=[];for(const q of s.subquestions){const inp=inputs[`${s.id}/${q.id}`],t=inp[v]||inp.full;answers[q.id]=(v==='empty'?[]:v==='omission'?(inp.omission||t.slice(1)):t).join('\n');vs.push(q.criteria.map((c,j)=>v==='empty'||(v==='omission'&&j===0)?not:v==='contrary'?{verdict:'contradicted',quote:t[j]}:met(t[j])));}add(s,v,answers,vs);}
...
for(const c of cases){
 if(c.id==='pilot-10-001-omission'||c.id==='pilot-10-001-subq2-omit-crit3'){
  const q=c.expected.subquestions.find(q=>q.subquestion_id==='subq2');q.verdicts[0]=met(c.answers.subq2);q.verdicts[0].criterion_id='crit3';c.expected_score++;
 }
}
edge('pilot-10-002','sub1','10-data-reliability-no-factors','기대치 도출 데이터의 신뢰성을 평가한다.',[not,not,not,not]);
edge('pilot-10-002','sub2','10-inquiry-without-evidence','경영진에게 질문한다.',[not,not]);
edge('pilot-10-003','sub1','10-missing-document-replacement','②는 서류가 없어도 정상으로 처리하고 새 항목으로 교체한다.',[not,{verdict:'contradicted',quote:'②는 서류가 없어도 정상으로 처리하고 새 항목으로 교체한다.'},{verdict:'contradicted',quote:'②는 서류가 없어도 정상으로 처리하고 새 항목으로 교체한다.'}]);
edge('pilot-10-004','sub2','10-purpose-without-risk-limit','표본 왜곡표시를 모집단으로 투영한다. 목적은 모집단 왜곡표시 규모의 개괄적 관점을 얻는 것이다.',[met('표본 왜곡표시를 모집단으로 투영한다.'),not]);
edge('pilot-10-004','sub2','10-projection-wrong-risk-direction','표본 왜곡표시를 모집단으로 투영한다. 규모의 개괄적 관점을 얻으며 투영하면 수용할 수 없는 표본위험이 모두 사라진다.',[met('표본 왜곡표시를 모집단으로 투영한다.'),{verdict:'contradicted',quote:'규모의 개괄적 관점을 얻으며 투영하면 수용할 수 없는 표본위험이 모두 사라진다.'}]);
edge('pilot-10-005','sub1','10-unequal-known-probability','모든 표본단위가 추출될 기회를 갖게 하되 각 단위의 추출확률이 반드시 같을 필요는 없다.',[met('모든 표본단위가 추출될 기회를 갖게 하되 각 단위의 추출확률이 반드시 같을 필요는 없다.')]);
...
const hashes=Object.fromEntries([file,'lib/questionV3.ts','lib/questionV3Grading.ts','lib/ai/openaiStructured.ts',`${dir}/10-inputs.json`].map(p=>[p,sha(p)])),model=process.env.CPA_GRADING_MODEL||'gpt-5.6-luna';
...
const live=cases.filter(c=>c.live&&c.variant!=='empty'&&(!process.argv.includes('--only-set-001')||c.set_id==='pilot-10-001'));
...
for(const [name,data]of Object.entries({'10-cases':cases,'10-offline':{offline,boundaries,deployment},'10-public':sets.map(compilePublicQuestionSet),'10-run-metadata':metadata}))fs.writeFileSync(`${dir}/${name}.json`,JSON.stringify(data,null,2)+'\n');console.log(JSON.stringify(metadata));
if(process.argv.includes('--live')){const out=`${dir}/10-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
