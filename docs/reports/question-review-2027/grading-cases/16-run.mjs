...
for(const [name,data]of Object.entries({'16-cases':cases,'16-offline':{offline,boundaries,deployment},'16-public':sets.map(compilePublicQuestionSet),'16-run-metadata':metadata}))fs.writeFileSync(`${dir}/${name}.json`,JSON.stringify(data,null,2)+'\n');console.log(JSON.stringify(metadata));
if(process.argv.includes('--live')){const out=`${dir}/16-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
