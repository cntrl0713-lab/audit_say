...
const sets=JSON.parse(fs.readFileSync(`${dir}/07-v2-public.json`));
const offline=JSON.parse(fs.readFileSync(`${dir}/07-v2-offline.json`)).offline;
...
try{
  ...
  for(const width of [1440,390]){
    ...
    for(const s of sets){
      ...
      const maxLength=await page.locator('#'+s.subquestions[0].id+'-answer').getAttribute('maxlength');assert.equal(maxLength,'5000');
  ...
  fs.writeFileSync(`${dir}/07-v2-ui-results.json`,JSON.stringify({scope:'actual QuizClient + actual globals.css; auth and server action mocked; no live auth/database E2E',records},null,2)+'\n');
