...
const sets=JSON.parse(fs.readFileSync(`${dir}/19-public.json`));
const offline=JSON.parse(fs.readFileSync(`${dir}/19-offline.json`)).offline;
...
try{
  ...
  fs.writeFileSync(`${dir}/19-ui-results.json`,JSON.stringify({scope:'actual QuizClient + actual globals.css; auth and server action mocked; no live auth/database E2E',records},null,2)+'\n');
