...
for(let i=0;i<lines.length;i++)edge(s.id,q.id,s.id+'-'+q.id+'-only-'+q.criteria[i].id,lines[i],lines.map((t,j)=>j===i?met(t):(s.id==='pilot-18-002'&&q.id==='sub1'&&i===3&&j===0)?met(lines[i]):not));
}
...
edge('pilot-18-002','sub1','18-transition-extra-no-documentation','일반 감사기준서 준수를 위해 필요한 추가 절차를 설계·수행한다.',[met('일반 감사기준서 준수를 위해 필요한 추가 절차를 설계·수행한다.'),not,not,not]);
...
for(const [name,data]of Object.entries({'18-cases':cases,'18-offline':{offline,boundaries,deployment},'18-public':sets.map(compilePublicQuestionSet),'18-run-metadata':metadata}))fs.writeFileSync(`${dir}/${name}.json`,JSON.stringify(data,null,2)+'\n');console.log(JSON.stringify(metadata));
if(process.argv.includes('--live')){const out=`${dir}/18-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
