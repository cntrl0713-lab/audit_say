...
const bank=JSON.parse(fs.readFileSync(paths[0]));const topic=bank.filter(s=>s.classification.topic_id==='02');
...
const cases=JSON.parse(fs.readFileSync(`${dir}/02-cases.json`));let checks=[];
...
fs.writeFileSync(`${dir}/02-boundaries.json`,JSON.stringify({hashes,authoring_public_encrypted_equal:true,checks,observations,working_tree:execFileSync('git',['status','--short'],{encoding:'utf8'}),limitations:'server action uses isolated auth/quota/grading/progress adapters; real database side effects not executed'},null,2)+'\n');
