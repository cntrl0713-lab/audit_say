...
const sets=JSON.parse(fs.readFileSync(`${dir}/11-v2-public.json`));
const offline=JSON.parse(fs.readFileSync(`${dir}/11-v2-offline.json`)).offline;
...
try{
  ...
  fs.writeFileSync(`${dir}/11-ui-results.json`,JSON.stringify({scope:'actual QuizClient + actual globals.css; auth and server action mocked; no live auth/database E2E',records},null,2)+'\n');
