...
e('007','sub2','intro-whole-only','핵심감사사항을 재무제표 전체 감사의 관점에서 전체 감사의견 형성 시 다루었다고 기술한다.',['not_met','not_met']);
...
for(const [name,data]of Object.entries({'16-v2-cases':cases,'16-v2-offline':{offline,boundaries,deployment},'16-v2-public':sets.map(compilePublicQuestionSet),'16-v2-run-metadata':metadata}))fs.writeFileSync(`${dir}/${name}.json`,JSON.stringify(data,null,2)+'\n');console.log(JSON.stringify(metadata));
if(process.argv.includes('--live')){const out=`${dir}/16-v2-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
