...
const sets=JSON.parse(fs.readFileSync(`${dir}/07-public.json`));
const offline=JSON.parse(fs.readFileSync(`${dir}/07-offline.json`)).offline;
...
try{
  ...
  fs.writeFileSync(`${dir}/07-ui-results.json`,JSON.stringify({scope:'actual QuizClient + actual globals.css; auth and server action mocked; no live auth/database E2E',records},null,2)+'\n');
