...
const live=cases.filter(c=>c.live&&c.variant!=='empty'&&c.set_id==='pilot-15-003');
...
for(const [name,data]of Object.entries({'15-v4-cases':cases,'15-v4-offline':{offline,boundaries,deployment},'15-v4-public':sets.map(compilePublicQuestionSet),'15-v4-run-metadata':metadata}))fs.writeFileSync(`${dir}/${name}.json`,JSON.stringify(data,null,2)+'\n');console.log(JSON.stringify(metadata));
if(process.argv.includes('--live')){const out=`${dir}/15-v4-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
...
const prior=['15-wrong-opinion-name','15-conditions-without-opinions','pilot-15-003-paraphrase','15-mapped-conclusions-only'];for(const c of live.filter(c=>prior.includes(c.id)))for(const a of [2,3])await run(c,a);
