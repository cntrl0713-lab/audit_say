...
for(const [name,data]of Object.entries({'15-cases':cases,'15-offline':{offline,boundaries,deployment},'15-public':sets.map(compilePublicQuestionSet),'15-run-metadata':metadata}))fs.writeFileSync(`${dir}/${name}.json`,JSON.stringify(data,null,2)+'\n');console.log(JSON.stringify(metadata));
if(process.argv.includes('--live')){const out=`${dir}/15-live.jsonl`,done=fs.existsSync(out)?fs.readFileSync(out,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse):[];
