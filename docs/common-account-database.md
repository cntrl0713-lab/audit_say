...
2026-09-10. 운영 DB 변경·기존 계정 삭제를 실행하지 않은 구현 기록이다.
...
이 마이그레이션은 기존 Auth 회원을 삭제하지 않는다. 기존 회원의 닉네임은 CTA → CPA → Auth 메타데이터 순서로 가져오고, 형식이 다르거나 이미 사용 중이면 새 이름을 부여한다. 기존 회원을 유지하여 전환한다면 적용 전후 매핑을 확인한다. 사용자 승인 범위의 기존 계정 초기화는 결제 정산과 관리자 재생성을 확인한 별도 전환 작업이다.
