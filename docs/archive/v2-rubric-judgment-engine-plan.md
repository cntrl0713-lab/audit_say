# 계획서: 루브릭 판정 엔진 도입 (v2 보관본)

> 보관 문서: v2 테이블·루브릭·홀리스틱 채점 엔진을 전제로 작성된 과거 계획입니다.
> 현재 v3 구현 지침이나 전환 계획이 아니며, 아래의 v2 파일·테이블 명칭을 현재 런타임에
> 적용하지 않습니다.
...
- 기존 외부 AI 모델·재시도·지연 구조: 유지 (판정 품질 미달 실측 시에만 상위 모델 검토 — 이연).
