...
- 매일 한국시간 **08:30 이상 18:30 미만**에만 KICPA를 요청한다. 예약 실행은 정각 혼잡을 피한 08:37~18:22, 15분 간격이다. 지연 실행·수동 실행·페이지 이동·재시도·리다이렉트에도 요청 직전 시간을 검사한다.

GitHub Actions의 15분 예약은 15분 내 감지를 보장하지 않는다. 지연·누락이 가능하고 공개 저장소는 60일간 활동이 없으면 예약 실행이 비활성화될 수 있다. 저장소 공개 여부와 사용 요금제에 따라 실행 비용도 확인해야 한다. [GitHub 예약 실행 문서](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule)
