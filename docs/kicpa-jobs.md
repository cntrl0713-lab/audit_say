...
| HTML/JSON 파서·시간 제한 | 합성 자료로 검증. 게시판 외곽 페이지 HTTP 응답은 확인했으나 실제 목록 구조·원문 링크·파서 실데이터 검증은 미완료 |
| 발송 처리 | 업체 공통 인터페이스와 오프라인 가짜 발송기 검증 |
| 실제 발송 | 기본 발송기에서 차단. 환경변수만 바꿔도 실행 불가 |
| 운영 DB·앱 배포 | 배포 진행 중. 적용 SQL·배포 식별자·운영 검증 결과는 완료 후 추가하는 배포 기록(receipt)에서 확인 |
| 실제 파서·수집 예약·메시지 | 실제 목록 파서 미검증, 수집과 발송은 비활성 유지. 이번 배포로 활성화하지 않음 |
...
2026-09-09 밤의 로컬 검증은 한국시간 22시 이후여서 KICPA 요청 없이 수행했다. 별도 허용시간 내 확인에서는 게시판 외곽 페이지(wrapper)와 정책 페이지의 HTTP 응답을 확인했다. **실제 목록 구조·개별 공고의 원문 링크·파서 실데이터 검증은 아직 완료하지 않았다.** `tests/fixtures/kicpa-jobs`는 합성 자료이며 실제 사이트 구조의 증거가 아니다. 검증된 설정과 명시적인 활성화 조건이 없으면 실제 수집을 거부한다. 합성 설정은 검증 플래그를 켜도 운영 수집에 사용할 수 없다.

허용 시간에 실제 목록 HTML 또는 목록을 제공하는 JSON 응답에서 게시글 번호·제목·회사명·게시일·원문 링크와 공지 표시·빈 목록·페이지 이동을 확인해야 한다. 본문이나 연락처 등의 상세 정보는 추출하지 않는다. 목록이 최신순으로 안정적으로 정렬되는지 확인한 뒤 이미 수집한 구간에서 탐색을 종료하는 최적화를 검토한다. 현재 구현은 설정된 목록의 마지막 페이지까지 검증하므로, 운영 전 페이지 수와 요청량을 확인해야 한다. 페이지 순환·한도 초과·구조 변경은 일부 목록으로 기준을 초기화하지 않고 해당 게시판 저장을 중단한다.

## 데이터와 접근 권한

운영 배포에서는 웹을 먼저 배포한 뒤, 공통 계정 스키마가 준비된 상태에서 다음 채용 마이그레이션 3개를 순서대로 하나의 트랜잭션으로 적용할 계획이다. 기존 공통 계정 마이그레이션을 다시 실행하거나 이전 SQL 파일을 재작성하지 않는다. 현재는 적용·배포 진행 단계이며, 각 단계의 완료 여부는 완료 후 추가하는 배포 기록(receipt)을 기준으로 확인한다.
...
1. 한국시간 허용 구간에 KICPA 실제 목록과 원문 링크를 확인하고 요약 네 필드만 추출하는 파서 설정·페이지 범위를 확정한다.
2. 수집 비활성 상태에서 웹 배포와 기본·요약 필드·가입 회차 마이그레이션 적용을 완료하고 운영 검증 결과를 배포 기록으로 남긴다. 이번 배포와 별도로 실제 파서·최초 기준 적재·후속 신규 감지를 검증한 뒤 수집 예약 활성화를 진행한다.
