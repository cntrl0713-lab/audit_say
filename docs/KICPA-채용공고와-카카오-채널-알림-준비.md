...
기준일: 2026-09-10. 사용자 제공 원안 계획서는 2026-09-14 정리로 저장소에서 지웠고 커밋 `868e6471`까지의 git 기록에 남아 있다. 실제 구현 기준은 이 문서다.
...
- 매일 한국시간 **08:30 이상 18:30 미만**에만 KICPA를 요청한다. 예약 실행은 정각 혼잡을 피한 08:32~18:27, 5분 간격(`02·07·12·17·22·27·32·37·42·47·52·57분`)이다. 지연 실행·수동 실행·페이지 이동·재시도·리다이렉트에도 요청 직전 시간을 검사한다.
...
| 운영 DB·앱 배포 | 2026-09-10 운영 배포·SQL 3개 적용 및 HTTP·권한 검증 완료. [배포 기록](KICPA-채용-기능-운영-배포-기록-2026-09-10.md) 참고 |
| 수집 예약·메시지 | 자동 수집 연결 및 검증은 [수집 운영 기록](KICPA-자동-수집-연결-기록-2026-09-10.md) 참고. 메시지 발송은 계속 비활성 |
...
2026-09-09 밤의 로컬 검증은 한국시간 22시 이후여서 KICPA 요청 없이 수행했다. 이후 외곽 페이지와 정책 페이지 확인을 거쳐 2026-09-10 허용시간에 실제 목록 구조·원문 링크·페이지 이동·빈 목록을 확인했다. 상세 근거는 [수집 운영 기록](KICPA-자동-수집-연결-기록-2026-09-10.md)에 남겼다. `tests/fixtures/kicpa-jobs`의 공고 값은 합성 자료이며 실제 사이트 내용과 구별한다. 검증된 설정과 명시적인 활성화 조건이 없으면 실제 수집을 거부한다. 합성 설정은 검증 플래그를 켜도 운영 수집에 사용할 수 없다.
...
2026-09-10 운영 배포에서는 웹을 먼저 배포한 뒤, 공통 계정 스키마가 준비된 상태에서 다음 채용 마이그레이션 3개를 순서대로 하나의 트랜잭션으로 적용했다. 기존 공통 계정 마이그레이션을 다시 실행하거나 이전 SQL 파일을 재작성하지 않았다. 적용 버전과 운영 검증 결과는 [배포 기록](KICPA-채용-기능-운영-배포-기록-2026-09-10.md)을 기준으로 확인한다.
...
4. [알림톡 심사 준비안](수습-신입-CPA-채용공고-알림톡-심사-준비안.md)을 실제 승인 형식으로 확정하고 신청한다. 승인 가능성을 현재 보장하지 않는다.
