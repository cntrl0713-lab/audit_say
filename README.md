...
- `/pricing`: 30일 9,900원 PRO 구독·토스 카드 등록·추천인 닉네임 확인
- `/mypage/subscription`: 자동 갱신 해지·이용 기간·결제 내역·추천 보상 확인

결제·추천인은 CTA Tax Law와 같은 정책으로 구현하며 감사 서비스 안에서 별도로 관리합니다. 추천받은 회원의 첫 결제 후 30일간 환불이 없으면 양쪽에 30일을 지급합니다. 현재 학습 횟수·오답노트 접근 정책은 유지합니다. 운영 활성화에 필요한 DB 마이그레이션, 감사 전용 토스 키, 결제 worker와 웹훅 설정은 [결제·추천인 도입 안내](docs/결제-추천인-도입.md)를 따릅니다. 코드 추가만으로 운영 결제가 활성화되지는 않습니다.
