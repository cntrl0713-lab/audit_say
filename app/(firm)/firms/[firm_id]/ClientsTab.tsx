    ...
    return (
        <>
            ...
            <div className="mb-4 flex flex-wrap gap-1.5" aria-label="감사대상회사 정렬">
                {(
                    [
                        { key: 'revenue', label: '감사대상회사 매출액순' },
                        { key: 'operating_profit', label: '감사대상회사 영업이익순' },
                        { key: 'name', label: '이름순' },
                    ] as const
                ).map((option) => (
                    <Chip
                        key={option.key}
                        href={base + buildFilterQuery(query, { sort: option.key })}
            ...
            {result.rows.length === 0 ? (
                <EmptyState
                    title="조건에 맞는 감사대상회사가 없습니다."
                    description="선택한 회계법인과 사업연도 안에서만 검색합니다."
                />
            ) : (
                <>
                    ...
                    <DataTable
                        columns={[
                            { key: '0', label: '회사명', align: 'left', priority: 'always' },
                            { key: '1', label: '시장', align: 'right', priority: 'wide' },
                            { key: '2', label: '업종', align: 'right', priority: 'wide' },
                            { key: '3', label: '매출액', align: 'right', priority: 'always' },
                            { key: '4', label: '영업이익', align: 'right', priority: 'wide' },
                            { key: '5', label: '의견', align: 'right', priority: 'always' },
                            { key: '6', label: 'KAM', align: 'right', priority: 'wide' },
                        ]}
                        rows={result.rows.map((row) => [
                            ...
                            <span key="4">{formatKrw(row.operating_profit)}</span>,
                            <span key="5">
                                <OpinionBadge opinion={row.adt_opinion} />
                            </span>,
                            <span key="6">{formatNumber(row.kam_count)}</span>,
                        ])}
                    />
                    <Pagination
                        page={result.page}
