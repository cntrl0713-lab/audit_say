...
export default function WorkforceTab({ periods, year, tenure, headcounts }: { periods: FirmAnnualSummary[]; year: number; tenure: FirmCpaTenureRow[]; headcounts: FirmHeadcountRow[] }) {
    return <>
        ...
        {periods.length ? <div className="space-y-8">{periods.map(current => <AnnualPeriodCard key={current.source_rcept_no} current={current} tenure={rowsForReceipt(tenure, current.source_rcept_no)} headcounts={rowsForReceipt(headcounts, current.source_rcept_no)} />)}</div> : <EmptyState title="보고기간 정보 미확보" description="선택한 연도에 시작한 사업보고서가 아직 적재되지 않았거나 검토 보류 중입니다." />}
    </>;
}

function AnnualPeriodCard({ current, tenure, headcounts }: { current: FirmAnnualSummary; tenure: FirmCpaTenureRow[]; headcounts: FirmHeadcountRow[] }) {
    ...
    const fields = [
        ...
        { label: '이사 1인당 직원 수', value: current.employee_per_director, format: (n: number) => formatDecimal(n, 1, '명') },
    ...
    return (
        <>
            ...
            <Footnote>인원은 보고기간 말 기준입니다. 전 임직원·공인회계사·등록회계사는 서로 다른 모집단입니다. 1인당 매출액·인건비의 분모는 전 임직원이며, 이사 1인당 직원 수는 전 임직원 ÷ 이사 수입니다.</Footnote>
            <Footnote>이사 평균 보수는 공시 보수 집계 인원으로 나눕니다. 집계 인원이 5명 미만이거나 미확보이면 평균을 표시하지 않습니다.</Footnote>
            {counts.some(row => row.note) ? <Footnote>{counts.filter(row => row.note).map(row => row.label + ': ' + row.note).join(' · ')}</Footnote> : null}
            <section className="mt-8 space-y-3"><h4 className="text-lg font-medium">부문별 인력·매출</h4>
                <DataTable columns={[{ key: 'segment', label: '부문' }, { key: 'people', label: '전 임직원', align: 'right' }, { key: 'revenue', label: '매출액', align: 'right' }]} rows={(['audit', 'tax', 'advisory', 'other'] as const).map(segment => [SEGMENT_LABEL[segment], formatNumber(current[`employee_${segment}`], '명'), formatKrw(current[`revenue_${segment}`])])} />
            </section>
        </>
    );
}
