...
const secret = process.env.CPA_QUESTION_V3_ENCRYPTION_KEY || process.env.GOOGLE_API_KEY || '';

if (!fs.existsSync(authoringPath)) throw new Error(`authoring 문제은행을 찾을 수 없습니다: ${authoringPath}`);
if (!secret) throw new Error('CPA_QUESTION_V3_ENCRYPTION_KEY 또는 GOOGLE_API_KEY가 필요합니다.');
...
const errors = sets.flatMap((set, index) => validateQuestionSetV3(set, { verifySourceQuotes: true })
...
fs.writeFileSync(
    publicPath,
    `${JSON.stringify(sets.map(compilePublicQuestionSet), null, 2)}\n`,
    'utf8',
);
