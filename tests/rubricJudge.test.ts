...
// ─────────────────────────────────────────────
test('scoreFromVerdicts - 307번(ordered, all 단일) 실데이터 만점 검산', () => {
...
test('scoreFromVerdicts - 117번(all+best_n 혼합, 3개 sub) 실데이터 만점 검산', () => {
...
test('scoreFromVerdicts - 117번 best_n sub만 초과 충족 시에도 만점 (sub1·sub2는 부분 누락)', () => {
    const p117 = getCpaProblem(117);
...
test('scoreFromVerdicts - 134번(all 2개 sub, 비균등 배점) 실데이터 만점 검산', () => {
