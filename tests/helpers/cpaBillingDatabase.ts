...
export async function value<T>(db: PGlite, sql: string, params: unknown[] = []): Promise<T> {
  return (await db.query<{value:T}>(`select ${sql} as value`, params)).rows[0].value;
}
export async function createBillingDatabase() {
  ...
  try {
    ...
    await db.exec(readFileSync(new URL('../../supabase/migrations/20260913010000_cpa_billing.sql', import.meta.url), 'utf8'));
    return db;
