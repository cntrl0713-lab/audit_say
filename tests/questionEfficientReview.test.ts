...
check('cached evidence JSON cannot be mutated through a later consumer', f => {
    const context = createEfficientValidationContext();
    createEfficientReviewReceipt(f.flush(), f.sets[0], context, f.root);
    const cached = context.jsonFiles?.get(path.resolve(f.root, f.manifest.bank.file));
    assert(cached); const bank = cached.value as QuestionSetV3[];
    assert(Object.isFrozen(bank)); assert(Object.isFrozen(bank[0].subquestions[0].criteria[0]));
    assert.throws(() => { bank[0].title = 'mutated cached evidence'; }, TypeError);
    assert.doesNotThrow(() => assertEfficientEvidenceUnchanged(context));
});
...
check('reuse selects the original manifest sibling snapshot despite an earlier ancestor index', f => {
